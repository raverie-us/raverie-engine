...
inline bool ContainsRequiredTags(const TagList& requiredTags,
                                 const TagList& tags)
{
  for (TagList::range range = requiredTags.All(); !range.Empty();
       range.PopFront())
  {
...
inline bool ContainsRequiredTagsCaseInsensitive(const TagList& requiredTags,
                                                const TagList& tags)
{
  for (TagList::range range = requiredTags.All(); !range.Empty();
       range.PopFront())
  {
    ...
    bool found = false;
    for (TagList::range tagRange = tags.All(); !tagRange.Empty();
         tagRange.PopFront())
    {
