...
void ZilchShaderGenerator::Initialize()
{
  ...
  mSamplerAttributeValues["TextureAddressingXClamp"] =
      SamplerSettings::AddressingX(TextureAddressing::Clamp);
  mSamplerAttributeValues["TextureAddressingXRepeat"] =
      SamplerSettings::AddressingX(TextureAddressing::Repeat);
  mSamplerAttributeValues["TextureAddressingXMirror"] =
      SamplerSettings::AddressingX(TextureAddressing::Mirror);
  mSamplerAttributeValues["TextureAddressingYClamp"] =
      SamplerSettings::AddressingY(TextureAddressing::Clamp);
  mSamplerAttributeValues["TextureAddressingYRepeat"] =
      SamplerSettings::AddressingY(TextureAddressing::Repeat);
  mSamplerAttributeValues["TextureAddressingYMirror"] =
      SamplerSettings::AddressingY(TextureAddressing::Mirror);
  mSamplerAttributeValues["TextureFilteringNearest"] =
      SamplerSettings::Filtering(TextureFiltering::Nearest);
  mSamplerAttributeValues["TextureFilteringBilinear"] =
      SamplerSettings::Filtering(TextureFiltering::Bilinear);
  mSamplerAttributeValues["TextureFilteringTrilinear"] =
      SamplerSettings::Filtering(TextureFiltering::Trilinear);
  mSamplerAttributeValues["TextureCompareModeDisabled"] =
  ...
  mSamplerAttributeValues["TextureCompareFuncNever"] =
      SamplerSettings::CompareFunc(TextureCompareFunc::Never);
  mSamplerAttributeValues["TextureCompareFuncAlways"] =
      SamplerSettings::CompareFunc(TextureCompareFunc::Always);
  mSamplerAttributeValues["TextureCompareFuncLess"] =
      SamplerSettings::CompareFunc(TextureCompareFunc::Less);
  mSamplerAttributeValues["TextureCompareFuncLessEqual"] =
  ...
  mSamplerAttributeValues["TextureCompareFuncEqual"] =
      SamplerSettings::CompareFunc(TextureCompareFunc::Equal);
  mSamplerAttributeValues["TextureCompareFuncNotEqual"] =
      SamplerSettings::CompareFunc(TextureCompareFunc::NotEqual);
...
void ZilchShaderGenerator::InitializeSpirV()
{
  ...
  nameSettings.mAllowedFieldAttributes.Insert(PropertyAttributes::cGroup,
                                              AttributeInfo());
  nameSettings.mAllowedFieldAttributes.Insert(PropertyAttributes::cRange,
                                              AttributeInfo());
  nameSettings.mAllowedFieldAttributes.Insert(PropertyAttributes::cSlider,
                                              AttributeInfo());
  ...
  EventConnect(&mFragmentsProject,
               ...
               &ZilchShaderGenerator::OnZilchFragmentTypeParsed,
               this);
  EventConnect(&mFragmentsProject,
               Events::TranslationError,
               &ZilchShaderGenerator::OnZilchFragmentTranslationError,
               this);
  EventConnect(&mFragmentsProject,
               Events::ValidationError,
               &ZilchShaderGenerator::OnZilchFragmentValidationError,
               this);
}
...
LibraryRef BuildWrapperLibrary(ZilchShaderIRLibraryRef fragmentsLibrary)
{
  ...
  String wrapperLibraryName =
      BuildString(fragmentsLibrary->mZilchLibrary->Name, "Wrapper");
    ...
    if (shaderTypeMeta->mFragmentType != FragmentType::None)
    {
      ...
      forRange(ShaderIRFieldMeta * field, shaderTypeMeta->mFields.All())
      {
        ...
        if (field->ContainsAttribute("PropertyInput"))
        {
          ...
          String propertyTypeName = lookupMap.FindValue(
              field->mZilchType->Name, field->mZilchType->Name);
          BoundType* type =
          ...
          if (type != nullptr)
          {
            ...
            // alignment.
            size_t pad =
                (byteAlignment - fragmentSize % byteAlignment) % byteAlignment;
            fragmentSize += pad;
      ...
      BoundType* boundType = builder.AddBoundType(shaderTypeMeta->mZilchName,
      ...
      // type.
      boundType->Add(new MetaResource(
          (Resource*)shaderTypeMeta->mZilchType->Location.CodeUserData));
      ...
      forRange(ShaderPropertyInfo & shaderProperty, shaderProperties.All())
      {
        ...
        if (shaderProperty.mIsTexture)
        {
          ...
          // Set default value.
          Texture* texture =
              shaderProperty.mShaderField->mDefaultValueVariant.Get<Texture*>();
          if (texture != nullptr)
        ...
        else
        {
          ...
          // Set default value.
          shaderProperty.mShaderField->mDefaultValueVariant.CopyStoredValueTo(
              defaultElement);
        }

        // Create property.
        GetterSetter* getterSetter =
            builder.AddBoundGetterSetter(boundType,
                                         shaderProperty.mName,
                                         shaderProperty.mBoundType,
                                         setter,
                                         getter,
                                         MemberOptions::None);
        ...
        getterSetter->NameLocation =
            shaderProperty.mShaderField->mZilchType->NameLocation;
        getterSetter->Location =
            shaderProperty.mShaderField->mZilchType->Location;
        ...
        forRange(ShaderIRAttribute & shaderAttribute,
                 shaderProperty.mShaderField->mAttributes.All())
        {
          if (zeroAttributes->IsValidPropertyAttribute(
                  shaderAttribute.mAttributeName))
          {
            Attribute* attribute =
                getterSetter->AddAttribute(shaderAttribute.mAttributeName);
            forRange(ShaderIRAttributeParameter & attributeParam,
                     shaderAttribute.mParameters.All())
            {
              Zilch::AttributeParameter& param =
                  attributeParam.GetZilchAttributeParameter();
...
LibraryRef ZilchShaderGenerator::BuildFragmentsLibrary(
    Module& dependencies,
    ...
    StringParam libraryName)
{
  ...
  forRange(Resource * resource, fragments.All())
  {
    ...
    mFragmentsProject.AddCodeFromString(
        fragment->mText, fragment->GetOrigin(), resource);
  }
  ...
  {
    ZilchShaderIRLibraryRef internalDependency =
        GetInternalLibrary(dependentLibrary);
    internalDependencies->Append(internalDependency);
  }

  ZilchShaderIRLibraryRef fragmentsLibrary =
      mFragmentsProject.CompileAndTranslate(internalDependencies,
                                            mFrontEndTranslator);
  ...
  forRange(BoundType * boundType, library->BoundTypes.Values())
  {
    ...
    if (shaderTypeMeta->ContainsAttribute("CoreVertex") &&
        shaderTypeMeta->ContainsAttribute("Vertex"))
      fragmentTypes.Insert(boundType->Name, ZilchFragmentType::CoreVertex);
    else if (shaderTypeMeta->ContainsAttribute("RenderPass") &&
             shaderTypeMeta->ContainsAttribute("Pixel"))
      fragmentTypes.Insert(boundType->Name, ZilchFragmentType::RenderPass);
    else if (shaderTypeMeta->ContainsAttribute("PostProcess") &&
             shaderTypeMeta->ContainsAttribute("Pixel"))
...
bool ZilchShaderGenerator::Commit(ZilchCompileEvent* e)
{
  ...
  forRange(ResourceLibrary * library, e->mModifiedLibraries.All())
  {
    ...
    if (pendingLibrary)
    {
      ZilchShaderIRLibraryRef internalPendingLibrary =
          mPendingToPendingInternal.FindValue(pendingLibrary, nullptr);
  ...
  ErrorIf(!mPendingToPendingInternal.Empty(),
          "We created a new library but it was not given to commit");
...
void ZilchShaderGenerator::MapFragmentTypes()
{
  ...
  {
    ZilchShaderIRLibraryRef fragmentLibrary =
        GetInternalLibrary(wrapperLibrary);
    forRange(BoundType * boundType, wrapperLibrary->BoundTypes.Values())
    {
      ZilchShaderIRType* shaderType =
          fragmentLibrary->FindType(boundType->Name);
      ShaderIRTypeMeta* shaderTypeMeta = shaderType->mMeta;
...
bool ZilchShaderGenerator::BuildShaders(
    ShaderSet& shaders,
    ...
    Array<ShaderDefinition>* compositeShaderDefs)
{
  ...
  pipelineDescription.mToolPasses.PushBack(
      new SpirVSpecializationConstantPass());
  ...
  for (size_t startIndex = 0; startIndex < totalShaderCount;
       startIndex += compositeBatchCount)
  {
    ...
    size_t endIndex =
        Math::Min(startIndex + compositeBatchCount, totalShaderCount);
    for (size_t i = startIndex; i < endIndex; ++i)
    {
      ...
      // CoreVertex
      ErrorIf(fragmentsLibrary->FindType(shader->mCoreVertex) == nullptr,
              "No fragment.");
      shaderDef.mFragments.PushBack(
          fragmentsLibrary->FindType(shader->mCoreVertex));

      // Composition
      if (composites.ContainsKey(shader->mComposite))
      {
        forRange(String fragmentName,
                 composites[shader->mComposite].mFragmentNames.All())
        {
          ErrorIf(fragmentsLibrary->FindType(fragmentName) == nullptr,
                  "No fragment.");
          shaderDef.mFragments.PushBack(
              fragmentsLibrary->FindType(fragmentName));
        }
      }
      else
      {
      ...
      // ApiPerspective
      ZilchShaderIRType* apiPerspectiveOutput =
          fragmentsLibrary->FindType("ApiPerspectiveOutput");
      ErrorIf(apiPerspectiveOutput == nullptr,
              "Missing fragment from core library.");
      shaderDef.mFragments.PushBack(apiPerspectiveOutput);

      // RenderPass
      if (shader->mRenderPass.Empty() == false)
      {
        ErrorIf(fragmentsLibrary->FindType(shader->mRenderPass) == nullptr,
                "No fragment.");
        shaderDef.mFragments.PushBack(
            fragmentsLibrary->FindType(shader->mRenderPass));
      }
      ...
      ZilchShaderIRCompositor::ShaderStageDescription& vertexInfo =
          shaderDef.mResults[FragmentType::Vertex];
      ZilchShaderIRCompositor::ShaderStageDescription& geometryInfo =
          shaderDef.mResults[FragmentType::Geometry];
      ZilchShaderIRCompositor::ShaderStageDescription& pixelInfo =
          shaderDef.mResults[FragmentType::Pixel];

      shaderProject.AddCodeFromString(
          vertexInfo.mShaderCode, vertexInfo.mClassName, nullptr);
      shaderProject.AddCodeFromString(
          geometryInfo.mShaderCode, geometryInfo.mClassName, nullptr);
      shaderProject.AddCodeFromString(
          pixelInfo.mShaderCode, pixelInfo.mClassName, nullptr);
    ...
    EventConnect(&shaderProject,
                 Zilch::Events::CompilationError,
                 &ZilchShaderGenerator::OnZilchFragmentCompilationError,
                 this);
    ZilchShaderIRLibraryRef shaderLibrary = shaderProject.CompileAndTranslate(
    ...
    for (size_t i = entryStartIndex; i < shaderEntries.Size(); ++i)
    {
      ...
      ZilchShaderIRType* vertexShader =
          shaderLibrary->FindType(entry.mVertexShader);
      ZilchShaderIRType* geometryShader =
          shaderLibrary->FindType(entry.mGeometryShader);
      ZilchShaderIRType* pixelShader =
          shaderLibrary->FindType(entry.mPixelShader);
      ErrorIf(vertexShader == nullptr || pixelShader == nullptr,
              "Invalid shader entry");

      bool success = true;
      Array<TranslationPassResultRef> vertexPipelineResults;
      success &= CompilePipeline(
          vertexShader, pipelineDescription, vertexPipelineResults);

      Array<TranslationPassResultRef> geometryPipelineResults;
      if (geometryShader != nullptr)
        success &= CompilePipeline(
            geometryShader, pipelineDescription, geometryPipelineResults);

      Array<TranslationPassResultRef> pixelPipelineResults;
      success &= CompilePipeline(
          pixelShader, pipelineDescription, pixelPipelineResults);

      if (!success)
        return false;

      entry.mVertexShader =
          vertexPipelineResults.Back()->mByteStream.ToString();
      if (geometryShader != nullptr)
        entry.mGeometryShader =
            geometryPipelineResults.Back()->mByteStream.ToString();
...
bool ZilchShaderGenerator::CompilePipeline(
    ZilchShaderIRType* shaderType,
                                                    ...
                                                    AnyParam value)
{
  ...
  ZilchShaderIRType* shaderType =
      GetCurrentInternalProjectLibrary()->FindType(fragmentName);
  if (shaderType == nullptr)
    return shaderInput;
  ...
  {
    shaderInput.mTranslatedInputName =
    ...
    forRange(ShaderIRAttribute & attribute, fieldMeta->mAttributes.All())
    {
      if (mSamplerAttributeValues.ContainsKey(attribute.mAttributeName))
        SamplerSettings::AddValue(
            shaderInput.mSamplerSettings,
            mSamplerAttributeValues[attribute.mAttributeName]);
    }
  }
  else
  {
    shaderInput.mTranslatedInputName = BuildString(
        "Material.", GenerateSpirVPropertyName(inputName, fragmentName));
    // SPIR-V doesn't allow boolean uniforms so we convert boolean inputs from
    // Zilch to integers.
    if (type == ShaderInputType::Bool)
    {
      shaderInput.mTranslatedInputName =
          BuildString(shaderInput.mTranslatedInputName, "_Boolean");
...
void ZilchShaderGenerator::OnZilchFragmentCompilationError(
    Zilch::ErrorEvent* event)
{
...
void ZilchShaderGenerator::OnZilchFragmentTypeParsed(Zilch::ParseEvent* event)
{
  ...
  if (status.Failed())
    event->BuildingProject->Raise(
        status.mLocation, ErrorCode::GenericError, status.Message.c_str());

  Status nameStatus;
  ZilchFragmentManager::GetInstance()->ValidateRawName(
      nameStatus, boundType->TemplateBaseName, boundType);

  if (nameStatus.Failed())
    event->BuildingProject->Raise(boundType->NameLocation,
                                  ErrorCode::GenericError,
                                  nameStatus.Message.c_str());
}

void ZilchShaderGenerator::OnZilchFragmentTranslationError(
    TranslationErrorEvent* event)
{
  String fullMessage = event->GetFormattedMessage(Zilch::MessageFormat::Python);
  ZilchFragmentManager::GetInstance()->DispatchScriptError(
...
void ZilchShaderGenerator::OnZilchFragmentValidationError(
...
ZilchShaderGenerator::GetInternalLibrary(LibraryRef library)
{
  if (library == nullptr)
    return nullptr;
...
ZilchShaderIRLibraryRef
ZilchShaderGenerator::GetCurrentInternalLibrary(LibraryRef library)
{
  return mCurrentToInternal.FindValue(library, nullptr);
}

ZilchShaderIRLibraryRef
ZilchShaderGenerator::GetPendingInternalLibrary(LibraryRef library)
{
  return mPendingToPendingInternal.FindValue(library, nullptr);
}

ZilchShaderGenerator::InternalLibraryRange
ZilchShaderGenerator::GetCurrentInternalLibraries()
{
...
{
  return GetInternalLibrary(
      ZilchManager::GetInstance()->mCurrentFragmentProjectLibrary);
}
