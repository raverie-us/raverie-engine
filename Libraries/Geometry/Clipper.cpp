...
void Clean(ContourArray* contours, f32 angleTolerance, f32 distanceTolerance)
{
  ...
  for (size_t i = 0; i < contours->Size(); ++i)
  {
    ...
    for (size_t j = 0; contour.Size() >= 3 && j < contour.Size();)
    {
      ...
      if (Math::Abs(sint) < angleTolerance || distance < distanceTolerance)
      {
        contour.EraseAt(j);
      }
      else
...
bool Operate(const OperationInput& input, ContourArray* results)
{
