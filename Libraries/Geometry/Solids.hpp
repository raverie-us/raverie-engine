...
/// 2)
const uint cPointPointEdge[8][3] = {{1, 4, 3},
                                    {0, 5, 2},
                                    {3, 6, 1},
                                    {2, 7, 0},
                                    {5, 0, 7},
...
const uint cEdgeFaceBase[6][4] = {{5, 9, 6, 1},
                                  {4, 11, 7, 3},
                                  {8, 9, 10, 11},
                                  {0, 1, 2, 3},
                                  {2, 6, 10, 7},
...
const uint cEdgeFace[6][4] = {{9, 6, 1, 5},
                              {4, 3, 7, 11},
                              {11, 10, 9, 8},
                              {0, 1, 2, 3},
                              {7, 2, 6, 10},
...
const uint cPointFaceSign[6][4] = {{1, 2, 5, 6},
                                   {0, 3, 4, 7}, // X-Axis
                                   {4, 7, 5, 6},
                                   {0, 3, 1, 2}, // Y-Axis
                                   {3, 7, 2, 6},
...
const uint cOctahedronFaces[8][3] = {{0, 2, 4},
                                     {0, 4, 3},
                                     {0, 5, 2},
                                     {0, 3, 5},
                                     {1, 4, 2},
...
const uint cIcosahedronFaces[20][3] = {
    {0, 1, 4},  {1, 0, 6},  {0, 4, 8},  {5, 2, 8},  {4, 5, 8},
...
const Vec3 cTruncatedCube[24] = {
    Vec3(sto, one, one),    Vec3(sto, one, -one),   Vec3(sto, -one, one),
    Vec3(sto, -one, -one),  Vec3(-sto, one, one),   Vec3(-sto, one, -one),
    Vec3(-sto, -one, one),  Vec3(-sto, -one, -one), Vec3(one, sto, one),
    Vec3(one, sto, -one),   Vec3(one, -sto, one),   Vec3(one, -sto, -one),
    Vec3(-one, sto, one),   Vec3(-one, sto, -one),  Vec3(-one, -sto, one),
    Vec3(-one, -sto, -one), Vec3(one, one, sto),    Vec3(one, one, -sto),
    Vec3(one, -one, sto),   Vec3(one, -one, -sto),  Vec3(-one, one, sto),
    Vec3(-one, one, -sto),  Vec3(-one, -one, sto),  Vec3(-one, -one, -sto)};
#undef sto

const Vec3 cTruncatedOctahedron[24] = {
    Vec3(zero, one, two),   Vec3(zero, one, -two),  Vec3(zero, -one, two),
    Vec3(zero, -one, -two), Vec3(zero, two, one),   Vec3(zero, two, -one),
    Vec3(zero, -two, one),  Vec3(zero, -two, -one), Vec3(one, zero, two),
    Vec3(one, zero, -two),  Vec3(-one, zero, two),  Vec3(-one, zero, -two),
    Vec3(one, two, zero),   Vec3(one, -two, zero),  Vec3(-one, two, zero),
    Vec3(-one, -two, zero), Vec3(two, zero, one),   Vec3(two, zero, -one),
    Vec3(-two, zero, one),  Vec3(-two, zero, -one), Vec3(two, one, zero),
    Vec3(two, -one, zero),  Vec3(-two, one, zero),  Vec3(-two, -one, zero)};
...
const Vec3 cRhombicuboctahedron[24] = {
    Vec3(one, one, ost),    Vec3(one, one, -ost),   Vec3(one, -one, ost),
    Vec3(one, -one, -ost),  Vec3(-one, one, ost),   Vec3(-one, one, -ost),
    Vec3(-one, -one, ost),  Vec3(-one, -one, -ost), Vec3(one, ost, one), //
    Vec3(one, ost, -one),   Vec3(one, -ost, one),   Vec3(one, -ost, -one),
    Vec3(-one, ost, one),   Vec3(-one, ost, -one),  Vec3(-one, -ost, one),
    Vec3(-one, -ost, -one), Vec3(ost, one, one), //
    Vec3(ost, one, -one),   Vec3(ost, -one, one),   Vec3(ost, -one, -one),
    Vec3(-ost, one, one),   Vec3(-ost, one, -one),  Vec3(-ost, -one, one),
    Vec3(-ost, -one, -one)};
...
const Vec3 cTruncatedCuboctahedron[48] = {
    Vec3(one, ost, ott),    Vec3(one, ost, -ott),   Vec3(one, -ost, ott),
    Vec3(one, -ost, -ott),  Vec3(-one, ost, ott),   Vec3(-one, ost, -ott),
    Vec3(-one, -ost, ott),  Vec3(-one, -ost, -ott), Vec3(one, ott, ost),
    Vec3(one, ott, -ost),   Vec3(one, -ott, ost),   Vec3(one, -ott, -ost),
    Vec3(-one, ott, ost),   Vec3(-one, ott, -ost),  Vec3(-one, -ott, ost),
    Vec3(-one, -ott, -ost), Vec3(ost, one, ott),    Vec3(ost, one, -ott),
    Vec3(ost, -one, ott),   Vec3(ost, -one, -ott),  Vec3(-ost, one, ott),
    Vec3(-ost, one, -ott),  Vec3(-ost, -one, ott),  Vec3(-ost, -one, -ott),
    Vec3(ost, ott, one),    Vec3(ost, ott, -one),   Vec3(ost, -ott, one),
    Vec3(ost, -ott, -one),  Vec3(-ost, ott, one),   Vec3(-ost, ott, -one),
    Vec3(-ost, -ott, one),  Vec3(-ost, -ott, -one), Vec3(ott, one, ost),
    Vec3(ott, one, -ost),   Vec3(ott, -one, ost),   Vec3(ott, -one, -ost),
    Vec3(-ott, one, ost),   Vec3(-ott, one, -ost),  Vec3(-ott, -one, ost),
    Vec3(-ott, -one, -ost), Vec3(ott, ost, one),    Vec3(ott, ost, -one),
    Vec3(ott, -ost, one),   Vec3(ott, -ost, -one),  Vec3(-ott, ost, one),
    Vec3(-ott, ost, -one),  Vec3(-ott, -ost, one),  Vec3(-ott, -ost, -one)};
#undef ott
#undef ost
...
const Vec3 cIcosidodecahedron[30] = {
    Vec3(zero, zero, tau),  Vec3(zero, zero, -tau), Vec3(zero, tau, zero),
    Vec3(zero, -tau, zero), Vec3(tau, zero, zero),  Vec3(-tau, zero, zero),

    Vec3(hlf, tot, hot),    Vec3(hlf, tot, -hot),   Vec3(hlf, -tot, hot),
    Vec3(hlf, -tot, -hot),  Vec3(-hlf, tot, hot),   Vec3(-hlf, tot, -hot),
    Vec3(-hlf, -tot, hot),  Vec3(-hlf, -tot, -hot),

    Vec3(tot, hot, hlf),    Vec3(tot, hot, -hlf),   Vec3(tot, -hot, hlf),
    Vec3(tot, -hot, -hlf),  Vec3(-tot, hot, hlf),   Vec3(-tot, hot, -hlf),
    Vec3(-tot, -hot, hlf),  Vec3(-tot, -hot, -hlf),

    Vec3(hot, hlf, tot),    Vec3(hot, hlf, -tot),   Vec3(hot, -hlf, tot),
    Vec3(hot, -hlf, -tot),  Vec3(-hot, hlf, tot),   Vec3(-hot, hlf, -tot),
    Vec3(-hot, -hlf, tot),  Vec3(-hot, -hlf, -tot)};
#undef hot
#undef tot
#undef hlf
...
const Vec3 cTruncatedDodecahedron[60] = {
    Vec3(zero, oot, TpT),   Vec3(zero, oot, -TpT),  Vec3(zero, -oot, TpT),
    Vec3(zero, -oot, -TpT),

    Vec3(TpT, zero, oot),   Vec3(TpT, zero, -oot),  Vec3(-TpT, zero, oot),
    Vec3(-TpT, zero, -oot),

    Vec3(oot, TpT, zero),   Vec3(oot, -TpT, zero),  Vec3(-oot, TpT, zero),
    Vec3(-oot, -TpT, zero),

    Vec3(oot, tau, ttt),    Vec3(oot, tau, -ttt),   Vec3(oot, -tau, ttt),
    Vec3(oot, -tau, -ttt),  Vec3(-oot, tau, ttt),   Vec3(-oot, tau, -ttt),
    Vec3(-oot, -tau, ttt),  Vec3(-oot, -tau, -ttt),

    Vec3(ttt, oot, tau),    Vec3(ttt, oot, -tau),   Vec3(ttt, -oot, tau),
    Vec3(ttt, -oot, -tau),  Vec3(-ttt, oot, tau),   Vec3(-ttt, oot, -tau),
    Vec3(-ttt, -oot, tau),  Vec3(-ttt, -oot, -tau),

    Vec3(tau, ttt, oot),    Vec3(tau, ttt, -oot),   Vec3(tau, -ttt, oot),
    Vec3(tau, -ttt, -oot),  Vec3(-tau, ttt, oot),   Vec3(-tau, ttt, -oot),
    Vec3(-tau, -ttt, oot),  Vec3(-tau, -ttt, -oot),

    Vec3(tau, two, tsq),    Vec3(tau, two, -tsq),   Vec3(tau, -two, tsq),
    Vec3(tau, -two, -tsq),  Vec3(-tau, two, tsq),   Vec3(-tau, two, -tsq),
    Vec3(-tau, -two, tsq),  Vec3(-tau, -two, -tsq),

    Vec3(tsq, tau, two),    Vec3(tsq, tau, -two),   Vec3(tsq, -tau, two),
    Vec3(tsq, -tau, -two),  Vec3(-tsq, tau, two),   Vec3(-tsq, tau, -two),
    Vec3(-tsq, -tau, two),  Vec3(-tsq, -tau, -two),

    Vec3(two, tsq, tau),    Vec3(two, tsq, -tau),   Vec3(two, -tsq, tau),
    Vec3(two, -tsq, -tau),  Vec3(-two, tsq, tau),   Vec3(-two, tsq, -tau),
    Vec3(-two, -tsq, tau),  Vec3(-two, -tsq, -tau)};
#undef tsq
#undef ttt
#undef TpT
#undef oot
...
#define tpt (real(2.0) + tau) /* Two Plus Tau */
const Vec3 cTruncatedIcosahedron[60] = {
    Vec3(zero, one, tht), // abc
    Vec3(zero, one, -tht),  Vec3(zero, -one, tht), Vec3(zero, -one, -tht),

    Vec3(one, tht, zero), // bca
    Vec3(one, -tht, zero),  Vec3(-one, tht, zero), Vec3(-one, -tht, zero),
...
const Vec3 cRhombicosidodecahedron[60] = {
    Vec3(one, one, tcu),    Vec3(one, one, -tcu),   Vec3(one, -one, tcu),
    Vec3(one, -one, -tcu),  Vec3(-one, one, tcu),   Vec3(-one, one, -tcu),
    Vec3(-one, -one, tcu),  Vec3(-one, -one, -tcu),

    Vec3(tcu, one, one),    Vec3(tcu, one, -one),   Vec3(tcu, -one, one),
    Vec3(tcu, -one, -one),  Vec3(-tcu, one, one),   Vec3(-tcu, one, -one),
    Vec3(-tcu, -one, one),  Vec3(-tcu, -one, -one),

    Vec3(one, tcu, one),    Vec3(one, tcu, -one),   Vec3(one, -tcu, one),
    Vec3(one, -tcu, -one),  Vec3(-one, tcu, one),   Vec3(-one, tcu, -one),
    Vec3(-one, -tcu, one),  Vec3(-one, -tcu, -one),

    Vec3(tsq, tau, ttt),    Vec3(tsq, tau, -ttt),   Vec3(tsq, -tau, ttt),
    Vec3(tsq, -tau, -ttt),  Vec3(-tsq, tau, ttt),   Vec3(-tsq, tau, -ttt),
    Vec3(-tsq, -tau, ttt),  Vec3(-tsq, -tau, -ttt),

    Vec3(ttt, tsq, tau),    Vec3(ttt, tsq, -tau),   Vec3(ttt, -tsq, tau),
    Vec3(ttt, -tsq, -tau),  Vec3(-ttt, tsq, tau),   Vec3(-ttt, tsq, -tau),
    Vec3(-ttt, -tsq, tau),  Vec3(-ttt, -tsq, -tau),

    Vec3(tau, ttt, tsq),    Vec3(tau, ttt, -tsq),   Vec3(tau, -ttt, tsq),
    Vec3(tau, -ttt, -tsq),  Vec3(-tau, ttt, tsq),   Vec3(-tau, ttt, -tsq),
    Vec3(-tau, -ttt, tsq),  Vec3(-tau, -ttt, -tsq),

    Vec3(tpt, zero, tsq),   Vec3(tpt, zero, -tsq),  Vec3(-tpt, zero, tsq),
    Vec3(-tpt, zero, -tsq),

    Vec3(tsq, tpt, zero),   Vec3(tsq, -tpt, zero),  Vec3(-tsq, tpt, zero),
    Vec3(-tsq, -tpt, zero),

    Vec3(zero, tsq, tpt),   Vec3(zero, tsq, -tpt),  Vec3(zero, -tsq, tpt),
    Vec3(zero, -tsq, -tpt)};
