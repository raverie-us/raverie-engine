...
// ObbObb test
//                          Primary Axes
const uint cObbAxes[15] = {
    65536,  // One's X                 0x00010000
...
struct PointInfo
{
  ...
  PointInfo(void) : Point(Vec3::cZero), Projection(real(0.0))
  {
    //
  }
  ...
  *currPointIndex = initIndex;
  ...
  for (uint i = 0; i < maxPoints; ++i)
  {
    ...
    *currPointIndex = initIndex;
    ...
    ErrorIf(*currPointIndex == initIndex,
            "Intersection - Something went wrong"
                             ...
                             Manifold& manifold)
{
  ...
  const Vec3* aCenter;
  const Vec3* bCenter;
  const Vec3* aHalfExtents;
  const Vec3* bHalfExtents;
  ...
  if (axisCase < 4)
  {
    aIndex = 0;
    bIndex = 1;
    aAxes[0] = &(obbOneAxes[0]);
    bAxes[0] = &(obbTwoAxes[0]);
    aAxes[1] = &(obbOneAxes[1]);
    bAxes[1] = &(obbTwoAxes[1]);
    aAxes[2] = &(obbOneAxes[2]);
    bAxes[2] = &(obbTwoAxes[2]);
    bBasis = &obbTwoBasis;
    aCenter = &obbOneCenter;
    bCenter = &obbTwoCenter;
    aHalfExtents = &obbOneHalfExtents;
    bHalfExtents = &obbTwoHalfExtents;
  }
  else
  {
    aIndex = 1;
    bIndex = 0;
    aAxes[0] = &(obbTwoAxes[0]);
    bAxes[0] = &(obbOneAxes[0]);
    aAxes[1] = &(obbTwoAxes[1]);
    bAxes[1] = &(obbOneAxes[1]);
    aAxes[2] = &(obbTwoAxes[2]);
    bAxes[2] = &(obbOneAxes[2]);
    bBasis = &obbOneBasis;
    aCenter = &obbTwoCenter;
    bCenter = &obbOneCenter;
    aHalfExtents = &obbTwoHalfExtents;
    bHalfExtents = &obbOneHalfExtents;
    aNormal *= real(-1.0);
  }
  ...
  if (pointCount <= maxPoints)
  {
    ...
    for (uint i = 0; i < pointCount; ++i)
    {
      ...
      if (cGeometrySafeChecks)
      {
        ...
        ErrorIf(dot < cObbObbNormalCheck,
                "Intersection - Normal is not "
