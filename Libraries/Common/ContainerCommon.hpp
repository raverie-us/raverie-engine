...
struct ZeroSharedTemplate Pair
{
  ...
  explicit Pair(const type0& value0) : first(value0), second()
  {
  }

  Pair(const type0& value0, const type1& value1) : first(value0), second(value1)
  {
  }
  ...
  Pair(MoveReference<Pair> other) : first(other->first), second(other->second)
  {
  }
...
struct IteratorRange
{
  ...
  IteratorRange(iterator pbegin, iterator pend) : begin(pbegin), end(pend)
  {
  }
...
struct IteratorTypedRange
{
  ...
  IteratorTypedRange(iterator pbegin, iterator pend) : begin(pbegin), end(pend)
  {
  }
  ...
  IteratorTypedRange& All()
  {
    return *this;
  }
  const IteratorTypedRange& All() const
  {
    return *this;
  }
...
struct ConstPointerRange
{
  ...
  ConstPointerRange(iterator pbegin, iterator pend) : begin(pbegin), end(pend)
  {
  }
  ...
  ConstPointerRange& All()
  {
    return *this;
  }
  const ConstPointerRange& All() const
  {
    return *this;
  }
...
struct PointerRange
{
  ...
  // Construct a range with two pointers.
  PointerRange(iterator pbegin, iterator pend) : begin(pbegin), end(pend)
  {
  }

  // Construct a range with a pointer and a size.
  PointerRange(iterator pbegin, size_t size) : begin(pbegin), end(pbegin + size)
  {
  }
...
template <typename type>
inline void DeleteOp(type* pointer)
{
  delete pointer;
}

template <typename keytype, typename type>
inline void DeleteOp(Pair<const keytype, type>& entry)
{
  delete entry.second;
}

template <typename keytype, typename type>
inline void DeleteOp(Pair<keytype, type>& entry)
{
  delete entry.second;
}
