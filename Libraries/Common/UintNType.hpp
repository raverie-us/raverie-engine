...
public:
  ...
  UintN& operator=(MoveReference<UintN> rhs)
  {
    mValue = rhs->mValue;
    return *this;
  }
  /// Assignment Operator
  UintN& operator=(value_type rhs)
  {
    mValue = Clamp(rhs);
    return *this;
  }
  ...
  UintN& operator+=(const UintN& rhs)
  {
    mValue = Clamp(mValue + rhs.mValue);
    return *this;
  }
  UintN& operator-=(const UintN& rhs)
  {
  ...
  bool operator==(const UintN& rhs) const
  {
    return mValue == rhs.mValue;
  }
  bool operator!=(const UintN& rhs) const
  ...
  bool operator>(const UintN& rhs) const
  {
    return IsGreaterThan<WrapAware>(rhs);
  }
  bool operator<(const UintN& rhs) const
  {
    return IsLessThan<WrapAware>(rhs);
  }
  bool operator>=(const UintN& rhs) const
  {
