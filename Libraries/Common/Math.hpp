...
ZeroShared inline Vec4 ToVector4(Vec3Param v, real w = real(0.0))
{
  return Vec4(v.x, v.y, v.z, w);
}

// Convert from a IntVec2 to a Vec2
ZeroShared inline Vec2 ToVec2(IntVec2 v)
{
  return Vec2(real(v.x), real(v.y));
}

// Convert from a Vec2 to a IntVec2 standard float to int conversion
ZeroShared inline IntVec2 ToIntVec2(Vec2Param vec2)
{
  return IntVec2(int(vec2.x), int(vec2.y));
}
ZeroShared inline IntVec2 ToIntVec2(Vec3Param vec3)
{
