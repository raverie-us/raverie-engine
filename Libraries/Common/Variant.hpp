...
public:
  ...
  /// Destroys the stored value currently being managed, if any, and zeroes out
...
private:
  ...
  /// stored value should exist (may be valid, destructed, or zeroed)
  ...
  /// buffer) Zeroes the variant's heap buffer (memsets it all to 0)
  void InternalZeroHeapBuffer();

  /// (Assumes the variant has a large stored type and has not allocated a heap
  /// buffer) Allocates the variant's heap buffer Does not zero the heap buffer
  ...
  /// allocating/zeroing/freeing) to be replaced with a value of the new stored
  /// type
  ...
  /// Alternatively zero-initializes if the stored type is missing a default
  /// constructor
