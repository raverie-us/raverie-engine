...
{
  ErrorIf(coefficientCount == 0,
          "Math - No coefficients passed for the "
          "polynomial.");
  ErrorIf(coefficients == nullptr,
          "Math - No coefficients passed for the "
          "polynomial.");
...
uint SolveCubic(real a0, real a1, real a2, real a3, real* roots)
{
  ...
  case 3:
    roots[2] = roots[2] -
               Math::EvaluateCubic(roots[2], a0, a1, a2, a3) /
                   Math::EvaluateQuadratic(roots[2], a1, two * a2, three * a3);
  case 2:
    roots[1] = roots[1] -
               Math::EvaluateCubic(roots[1], a0, a1, a2, a3) /
                   Math::EvaluateQuadratic(roots[1], a1, two * a2, three * a3);
  case 1:
    roots[0] = roots[0] -
               Math::EvaluateCubic(roots[0], a0, a1, a2, a3) /
...
uint SolveQuartic(real a0, real a1, real a2, real a3, real a4, real* roots)
{
  ...
  if (q >= real(0.0))
  {
    ...
    rootCount += SolveQuadratic(
        z + u, -v, real(1.0), roots + (roots == nullptr ? 0 : rootCount));
  }
  else
  {
    rootCount = SolveQuadratic(z + u, v, real(1.0), roots);
  ...
  for (uint i = 0; i < cRootIterations; ++i)
  {
    ...
    case 4:
      roots[3] =
          roots[3] - Math::EvaluateQuartic(roots[3], a0, a1, a2, a3, a4) /
                         Math::EvaluateCubic(
                             roots[3], a1, two * a2, three * a3, four * a4);
    case 3:
      roots[2] =
          roots[2] - Math::EvaluateQuartic(roots[2], a0, a1, a2, a3, a4) /
                         Math::EvaluateCubic(
                             roots[2], a1, two * a2, three * a3, four * a4);
    case 2:
      roots[1] =
          roots[1] - Math::EvaluateQuartic(roots[1], a0, a1, a2, a3, a4) /
                         Math::EvaluateCubic(
                             roots[1], a1, two * a2, three * a3, four * a4);
    case 1:
      roots[0] =
          roots[0] - Math::EvaluateQuartic(roots[0], a0, a1, a2, a3, a4) /
                         Math::EvaluateCubic(
                             roots[0], a1, two * a2, three * a3, four * a4);
      break;
