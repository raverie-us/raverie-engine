...
// For debugging, this is so the crash handler can be called within visual
// studio. This can't know about the engine though, so it has to take a callback
// that calls run on the engine...
...
struct CrashInfo
{
  ...
  // In zero these have a date/time-stamp which is set in the
  // CrashStartCallback.
  String mDumpName;
  String mLogName;
  String mStackName;
  ...
  // Make sure to set mModuleName to the name of your program if you set
  // mStripModules to true!
  bool mStripModules;
  String mModuleName;
};
...
struct CrashHandler
{
  // Enable the crash handler to start catching hardware exceptions. When a
  // crash happens, mRunCrashHandlerCallback will be called to control all of
  // the crash handler logic.
  static void Enable();
  ...
  // call in the platform specific __try __except exceptions handlers for
  // safety.
