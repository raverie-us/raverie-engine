...
#define SerializeEnumName(enumName, variable)                                  \
  stream.EnumField(enumName::EnumName,                                         \
                   #variable,                                                  \
                   (uint&)variable,                                            \
                   ZilchTypeId(enumName::Enum))
...
#define SerializeEnumNameDefault(enumName, variable, defaultValue)             \
  if (!stream.EnumField(enumName::EnumName,                                    \
                        #variable,                                             \
                        (uint&)variable,                                       \
                        ZilchTypeId(enumName::Enum)))                          \
...
void SerializeBits(Serializer& stream,
                   BitFieldType& field,
                   const cstr enumNames[],
                   uint mask = 0,
                   uint defaults = uint(-1))
{
  ...
  else // Could this be done in a better way?
  {
    ...
    if (stream.GetMode() == SerializerMode::Loading)
    {
      ...
      // the new values. To do this clear the bits we are saving then "or" in the
      // serialized data.
