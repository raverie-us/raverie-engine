...
class OptionSet {
	...
	struct Option {
		...
		Option(plcob pb_, ScString description_="") :
			opType(SC_TYPE_BOOLEAN), pb(pb_), description(description_) {
		}
		Option(plcoi pi_, ScString description_) :
			opType(SC_TYPE_INTEGER), pi(pi_), description(description_) {
		}
		Option(plcos ps_, ScString description_) :
	...
	typedef ScMap<ScString, Option> OptionMap;
	OptionMap nameToDef;
	ScString names;
	ScString wordLists;
...
public:
	...
	void DefineProperty(const char *name, plcob pb, ScString description="") {
		nameToDef[name] = Option(pb, description);
		AppendName(name);
	}
	void DefineProperty(const char *name, plcoi pi, ScString description="") {
		nameToDef[name] = Option(pi, description);
		AppendName(name);
	}
	void DefineProperty(const char *name, plcos ps, ScString description="") {
