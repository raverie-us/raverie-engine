...
static ScString GetRestOfLine(LexAccessor &styler, int start, bool allowSpace) {
	ScString restOfLine;
	int i =0;
...
static ScVector<ScString> Tokenize(const ScString &s) {
	// Break into space separated tokens
	ScString word;
	ScVector<ScString> tokens;
...
struct PPDefinition {
	...
	ScString key;
	ScString value;
	PPDefinition(int line_, const ScString &key_, const ScString &value_) :
...
struct OptionsCPP {
	...
	ScString foldExplicitStart;
	ScString foldExplicitEnd;
...
class LexerCPP : public ILexer {
	...
	PPStates vlls;
	ScVector<PPDefinition> ppDefineHistory;
	WordList keywords;
	WordList keywords2;
	WordList keywords3;
	WordList keywords4;
	WordList ppDefinitions;
	ScMap<ScString, ScString> preprocessorDefinitionsStart;
	OptionsCPP options;
	OptionSetCPP osCPP;
	SparseState<ScString> rawStringTerminators;
...
public:
	...
	void EvaluateTokens(ScVector<ScString> &tokens);
	bool EvaluateExpression(const ScString &expr, const ScMap<ScString, ScString> &preprocessorDefinitions);
};
		...
		if (*wordListN != wlNew) {
			...
			if (n == 4) {
				...
				for (int nDefinition = 0; nDefinition < ppDefinitions.len; nDefinition++) {
					...
					if (cpEquals) {
						ScString Name(cpDefinition, cpEquals - cpDefinition);
						ScString val(cpEquals+1);
						preprocessorDefinitionsStart[Name] = val;
					} else {
						ScString Name(cpDefinition);
						ScString val("1");
						preprocessorDefinitionsStart[Name] = val;
					}
				}
...
void SCI_METHOD LexerCPP::Lex(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
	...
	ScVector<PPDefinition>::iterator itInvalid = std::find_if(ppDefineHistory.begin(), ppDefineHistory.end(), After(lineCurrent-1));
	if (itInvalid != ppDefineHistory.end()) {
		ppDefineHistory.erase(itInvalid, ppDefineHistory.end());
		definitionsChanged = true;
	}

	ScMap<ScString, ScString> preprocessorDefinitions = preprocessorDefinitionsStart;
	for (ScVector<PPDefinition>::iterator itDef = ppDefineHistory.begin(); itDef != ppDefineHistory.end(); ++itDef) {
		preprocessorDefinitions[itDef->key] = itDef->value;
	}

	ScString rawStringTerminator = rawStringTerminators.ValueAt(lineCurrent-1);
	SparseState<ScString> rawSTNew(lineCurrent);
				...
				if (sc.atLineEnd) {
					...
					if (options.trackPreprocessor) {
						if (sc.Match("ifdef") || sc.Match("ifndef")) {
							...
							int i = isIfDef ? 5 : 6;
							ScString restOfLine = GetRestOfLine(styler, sc.currentPos + i + 1, false);
							bool foundDef = preprocessorDefinitions.find(restOfLine) != preprocessorDefinitions.end();
							preproc.StartSection(isIfDef == foundDef);
						} else if (sc.Match("if")) {
							ScString restOfLine = GetRestOfLine(styler, sc.currentPos + 2, true);
							...
							if (options.updatePreprocessor && !preproc.IsInactive()) {
								ScString restOfLine = GetRestOfLine(styler, sc.currentPos + 6, true);
								if (restOfLine.find(")") == ScString::npos) {	// Don't handle macros with arguments
									ScVector<ScString> tokens = Tokenize(restOfLine);
									ScString key;
									ScString value("1");
									if (tokens.size() >= 1) {
										key = tokens[0];
...
void LexerCPP::EvaluateTokens(ScVector<ScString> &tokens) {
	...
	ScVector<ScString>::iterator itBracket = std::find(tokens.begin(), tokens.end(), "(");
	ScVector<ScString>::iterator itEndBracket = std::find(tokens.begin(), tokens.end(), ")");
	while ((itBracket != tokens.end()) && (itEndBracket != tokens.end()) && (itEndBracket > itBracket)) {
		ScVector<ScString> inBracket(itBracket + 1, itEndBracket);
	...
	for (size_t j=0; (j+1)<tokens.size();) {
		if (setNegationOp.Contains(tokens[j][0])) {
			...
			if (tokens[j] == "!")
				isTrue = !isTrue;
			ScVector<ScString>::iterator itInsert =
...
bool LexerCPP::EvaluateExpression(const ScString &expr, const ScMap<ScString, ScString> &preprocessorDefinitions) {
	...
	for (;;) {
		if (setWord.Contains(*cp)) {
			...
			ScMap<ScString, ScString>::const_iterator it = preprocessorDefinitions.find(word);
