      ...
      forRange(Property * property, component.StoredType->GetProperties())
      {
            ...
            property->Set != nullptr)
        {
          if (event->Testing)
          {
            ...
            if (MetaDisplay* display =
                    metaObjectDrop->HasInherited<MetaDisplay>())
              displayString = display->GetName(instance);
            else
              displayString = metaObjectDrop->Name;

            event->Result = String::Format("Set %s.%s to %s",
                                           component.StoredType->Name.c_str(),
                                           property->Name.c_str(),
                                           displayString.c_str());
          }
          else
...
bool DropOnObjectViewport(MetaDropEvent* event,
                          Viewport* viewport,
                          Space* space)
{
  Handle instance = event->Instance;
  ...
  if (Archetype* archetype = instance.Get<Archetype*>())
  {
    ...
    {
      event->Result =
          String::Format("Create instance of %s", archetype->Name.c_str());
    }
    else
    {
      Z::gEditor->Tools->mCreationTool->CreateWithViewport(
          viewport, dropPosition, archetype);
    }
    return true;
  }
  ...
  if (SpriteSource* spriteSource = instance.Get<SpriteSource*>())
  {
    if (event->Testing)
    {
      event->Result = String::Format("Create sprite object from %s",
                                     spriteSource->Name.c_str());
    }
    else
    {
      Cog* object = Z::gEditor->Tools->mCreationTool->CreateWithViewport(
          viewport, dropPosition, CoreArchetypes::Sprite);
      if (object)
      {
  ...
  if (Mesh* mesh = instance.Get<Mesh*>())
  {
    ...
    {
      event->Result =
          String::Format("Create mesh object from %s", mesh->Name.c_str());
    }
    else
    {
      Cog* object = Z::gEditor->Tools->mCreationTool->CreateWithViewport(
          viewport, dropPosition, CoreArchetypes::Transform);
      if (object)
      {
  ...
  if (SoundCue* soundCue = instance.Get<SoundCue*>())
  {
    if (event->Testing)
    {
      event->Result = String::Format("Create sound emitter object from %s",
                                     soundCue->Name.c_str());
    }
    else
    {
                ...
                Cog* droppedOn)
{
  ...
  // other Resources
  if (droppedType->IsA(ZilchTypeId(Mesh)) ||
      droppedType->IsA(ZilchTypeId(SpriteSource)))
  {
