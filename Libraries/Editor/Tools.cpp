...
Frustum SelectToolFrustumEvent::GetFrustum()
{
  return mFrustum;
}
...
bool SameRootCompare(Cog* toSelect, Cog* hitCog)
{
  if (toSelect == nullptr || hitCog == nullptr)
    return false;

  return toSelect->FindRoot() == hitCog->FindRoot();
}
