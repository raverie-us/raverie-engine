...
void ManipulatorTool::ComputeWorldToolRect(Array<Handle>::range& range,
  Vec3Param toolPosition, bool includeChildren, bool childPass)
{
  Aabb aabb;
...
//******************************************************************************
void ManipulatorTool::ComputeLocalToolRect(Array<Handle>::range& range, Vec3Param toolNormal,
  Vec3Param pointOnToolPlane, Aabb* aabbOut, bool includeChildren, bool childPass)
{
  bool valid = false;
  Array<Aabb> obbs;
...
void ManipulatorTool::TestMouseMove(ViewportMouseEvent* event)
{
  ...
  for(uint i = 0; i <= 4; ++i)
  {
    ...
    Intersection::Type result =
      Intersection::RayObb(mouseRay.Start, mouseRay.Direction, middles[i], boxSize, obbBasis);
  ...
  for(uint i = 0; i < 4; ++i)
  {
    ...
    Intersection::Type result =
      Intersection::RayObb(mouseRay.Start, mouseRay.Direction, corner[i], boxSize, obbBasis);
