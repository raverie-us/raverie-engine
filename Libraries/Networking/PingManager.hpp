...
public:
  ...
  /// Acquires the next random incremental identifier (used to prevent
  /// predictive message spam attacks).
  uint AcquireNextRandomIncrementalId();

  /// Create a host ping request and sends a host ping message to the specified
  ...
  /// Returns true if successful, else false.
  bool SendHostPong(const IpAddress& theirIpAddress,
                    uint pingId,
                    uint sendAttemptId,
                    uint theirManagerId,
                    BitStream& pongData);
  ...
  bool ReceivePeerMessage(IpAddress const& theirIpAddress,
  ...
  bool ReceiveLinkMessage(IpAddress const& thierIpAddress,
                          Message& linkMessage) override;
  ...
  typedef UniquePointer<Callback2<bool, IpAddress const&, NetHostPingData&>>
      PingReceivedCallback;
  typedef UniquePointer<
      Callback3<void, IpAddress const&, NetHostPongData&, PendingHostPing&>>
      PongReceivedCallback;

  /// Sets the PingTimeout callback. This happens when a ping request has
  /// expired by living longer than its lifetime.
  void SetPingTimeoutCallback(PendingPingCallback callback);
  /// Sets the CancelledCallback. This happens when one ping overwrite another
  /// and is canceled before starting the next one.
  void SetPingCancelledCallback(PendingPingCallback callback);
  /// Sets the PingCallback. This happens when a ping manager receives a generic
  /// ping from any ping manager.
  void SetPingCallback(PingReceivedCallback callback);
  /// Sets the PongCallback. This happens when the ping manager receives a pong
  /// that is specifically for this manager.
  ...
  uint mManagerId;    ///< The id of this ping manager. Used to associate pings
                      ///< with this
  uint mNextRandomId; ///< Next random incremental identifier.
  PendingPingCallback
      mPingTimeoutCallback; ///< Callback for when a pending host ping expires.
  PendingPingCallback
      mPingCancelledCallback; ///< Callback for when a pending ping is canceled.
  ...
  float mPingInterval; ///< Controls how often the peer sends a host ping
                       ///< message when discovering or refreshing hosts via
                       ///< ping messages.
};
