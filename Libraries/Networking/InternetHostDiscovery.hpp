...
public:
  ...
  virtual bool ReceiveLinkMessage(IpAddress const& theirIpAddress,
  ...
  virtual void
  HandleNetPeerSentConnectResponse(NetPeerSentConnectResponse* event) override;
  ...
  virtual void HandleNetPeerReceivedConnectResponse(
      NetPeerReceivedConnectResponse* event) override;
  /// Handles the event where we sent a connect request. Returns true if we
  /// handled it, false if we did not.
  virtual void
  HandleNetPeerSentConnectRequest(NetPeerSentConnectRequest* event) override;
  /// Handles the event where we received a connect response. Returns true if we
  /// handled, false if we did not.
  virtual void HandleNetPeerReceivedConnectRequest(
      NetPeerReceivedConnectRequest* event) override;
  ...
  virtual void
  HandleCancelSingleHostRequest(SingleHostRequest& singleHostRequest);
  ...
  virtual bool HandlePing(IpAddress const& theirIpAddress,
                          NetHostPingData& netHostPingData) override;
  ...
  void ReceiveNetHostRecordList(IpAddress const& theirIpAddress,
                                Zero::Message const& message);
  ...
  IpAddress
      mMasterServerConnectionIp; ///< [Client/Server] Used to determine the
