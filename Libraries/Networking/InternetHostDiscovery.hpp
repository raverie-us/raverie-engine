...
public:
  ...
  // Data
  //
  unsigned int mCurrentMasterServerIndex; ///< [Client/Server] Used to poll master server in the order that they were placed in the array.
  IpAddress    mMasterServerConnectionIp; ///< [Client/Server] Used to determine the master server that it did end up connecting to when discovering hosts.
  float        mInternetHostListTimer;    ///< [Client/Server] Used to keep track of how much time has passed since internet host list request was requested.
};
