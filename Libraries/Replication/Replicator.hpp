...
public:
  ...
  /// locally, else false
  bool HasReplicaByEmplaceContext(const EmplaceContext& emplaceContext,
  ...
  ReplicaSet
  GetReplicasByEmplaceContext(const EmplaceContext& emplaceContext) const;
  /// Returns the replica in the specified emplace context if it is known
  /// locally, else nullptr
  Replica* GetReplicaByEmplaceContext(const EmplaceContext& emplaceContext,
                                      EmplaceId emplaceId) const;
  ...
  /// locally
  size_t
  GetReplicaCountByEmplaceContext(const EmplaceContext& emplaceContext) const;
  ...
  /// else false
  bool EmplaceReplicas(const ReplicaArray& replicas,
  ...
  bool SpawnReplicas(const ReplicaArray& replicas,
                     const Route& route = Route::All);
  ...
  bool CloneReplicas(const ReplicaArray& replicas,
                     const Route& route = Route::All);
  ...
  /// Returns true if successful, else false
  bool ForgetReplicas(const ReplicaArray& replicas,
                      const Route& route = Route::All);
  ...
  bool DestroyReplicas(const ReplicaArray& replicas,
                       const Route& route = Route::All);
  ...
  GetReplicaChannelType(const String& replicaChannelTypeName) const;
  ReplicaChannelType*
  GetReplicaChannelType(const String& replicaChannelTypeName);
  ...
  ReplicaChannelType*
  AddReplicaChannelType(ReplicaChannelTypePtr replicaChannelType);
  ...
  GetReplicaPropertyType(const String& replicaPropertyTypeName) const;
  ReplicaPropertyType*
  GetReplicaPropertyType(const String& replicaPropertyTypeName);
  ...
  ReplicaPropertyType*
  AddReplicaPropertyType(ReplicaPropertyTypePtr replicaPropertyType);
  ...
  virtual bool SerializeReplicas(const ReplicaArray& replicas,
                                 ReplicaStream& replicaStream) = 0;
  ...
  /// else false
  virtual bool DeserializeReplicas(const ReplicaStream& replicaStream,
                                   ReplicaArray& replicas) = 0;
  ...
  virtual void
  OnReplicaChannelPropertyChange(TimeMs timestamp,
  ...
  virtual void ClientOnConnectRequest(ReplicatorLink* link,
  ...
  virtual void
  ClientOnDisconnectNotice(ReplicatorLink* link,
  ...
  virtual Pair<bool, BitStream> ServerOnConnectRequest(
  ...
  virtual void ServerOnConnectResponse(ReplicatorLink* link,
                                       ConnectResponseData& connectResponseData)
  {
  }
  /// [Server] Called after receiving a connect confirmation
  virtual void ServerOnConnectConfirmation(ReplicatorLink* link,
                                           BitStream& connectConfirmationData)
  {
  }
  ...
  /// this
  virtual void
  ServerOnDisconnectNotice(ReplicatorLink* link,
                           DisconnectNoticeData& disconnectNoticeData,
                           TransmissionDirection::Enum direction)
  {
  }
  ...
  static bool
  ShouldIncludeAccurateTimestampOnInitialization(const ReplicaArray& replicas);
  static bool ShouldIncludeAccurateTimestampOnUninitialization(
      const ReplicaArray& replicas);
  static bool
  ShouldIncludeAccurateTimestampOnChange(ReplicaChannel* replicaChannel);
  ...
  /// Returns true if successful, else false
  bool HandleEmplace(const ReplicaArray& replicas,
                     const EmplaceContext& emplaceContext,
                     TimeMs timestamp);
  ...
  /// Returns true if successful, else false
  bool RouteSpawn(const ReplicaArray& replicas,
                  const Route& route,
                  TimeMs timestamp);
  ...
  /// Returns true if successful, else false
  bool RouteClone(const ReplicaArray& replicas,
                  const Route& route,
                  TimeMs timestamp);
  ...
  /// Returns true if successful, else false
  bool RouteForget(const ReplicaArray& replicas,
                   const Route& route,
                   TimeMs timestamp);
  ...
  /// Returns true if successful, else false
  bool RouteDestroy(const ReplicaArray& replicas,
                    const Route& route,
                    TimeMs timestamp);

  /// Routes a replica channel change
  /// Returns true if successful, else false
  bool RouteChange(ReplicaChannel* replicaChannel,
                   const Route& route,
                   TimeMs timestamp);
  /// Serializes a replica channel change
  /// Returns true if successful, else false
  bool SerializeChange(ReplicaChannel* replicaChannel,
                       Message& message,
                       TimeMs timestamp);
  ...
  public:
    /// Constructor
    ItemCacher(Replicator* replicator,
               ReplicatorMessageType::Enum replicatorMessageType) :
        ItemCache(),
    ...
    bool SerializeItems(item_id_range items, Message& message)
    {
      ...
      forRange(value_type & item, items)
      {
        ...
        // Write item
        if (!SerializeUnknownBasicVariant(
                SerializeDirection::Write, bitStream, item.first)) // Unable?
        {
          Assert(false);
          return false;
        }
    ...
    bool DeserializeItems(const Message& message)
    {
      ...
      while (bitStream.GetBitsUnread())
      {
        ...
        // Read item
        Item item;
        if (!SerializeUnknownBasicVariant(SerializeDirection::Read,
    ...
    bool SendItems(ReplicatorLink* replicatorLink, Message& message)
    {
      ...
      Status status;
      replicatorLink->LinkPlugin::Send(
          status, message, true, replicatorLink->GetCommandChannelId());
    ...
    ReplicatorMessageType::Enum
        mReplicatorMessageType; /// Replicator message type
  };
  ...
  EmplaceIdStores
      mEmplaceIdStores;   /// Emplace ID stores mapped by emplace context
  ReplicaSet mReplicaSet; /// Locally known live replicas
  CreateMap
      mCreateMap; /// Locally known created replicas mapped by create context
  ReplicaMap mReplicaMap; /// Locally known live replicas mapped by replica type
  EmplaceMap
      mEmplaceMap; /// Locally known emplaced replicas mapped by emplace context
