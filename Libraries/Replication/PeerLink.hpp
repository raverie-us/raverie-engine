...
public:
  ...
  MessageReceiptId SendInternal(Status& status,
                                MoveReference<Message> message,
                                bool reliable,
                                MessageChannelId channelId = 0,
                                bool receipt = false,
...
protected:
  ...
  /// Called before a message is sent
  /// Return true to continue sending the message, else false
  virtual bool OnMessageSend(OutMessage& message)
  {
    return true;
  }
  /// Called after a sent message is receipted
  /// Return true to continue receipting the message, else false
  virtual bool OnMessageReceipt(OutMessage& message, Receipt::Enum receipt)
  {
    return true;
  }
  /// Called after a message is received
  /// Return true to continue receiving the message, else false
  virtual bool OnMessageReceive(Message& message)
  {
    return true;
  }
  ...
  /// Called before a plugin message is sent
  /// Return true to continue sending the message, else false
  virtual bool OnPluginMessageSend(OutMessage& message)
  {
    return true;
  }
  ...
  /// Called before an incoming channel is opened
  /// Return true to continue opening the incoming channel, else false
  virtual bool OnIncomingChannelOpen(MessageChannelId channelId)
  {
    return true;
  }
  /// Called before an incoming channel is closed
