...
void DrawArc(Vec3Param center, Vec3Param normal, Vec3Param axis, real radiusA, real radiusB,
  real minAngle, real maxAngle, bool drawBoundaries, ByteColor color, real radianStepSize, bool leftHanded)
{
  Vec3 xAxis = axis;
...
void DrawHinge(Joint* joint, AnchorAtom& anchors, Mat3Param basis0, Mat3Param basis1, uint defaultPerspective)
{
  ...
  if(perspective & JointDebugDrawConfigFlags::ObjectAPerspective)
  {
    ...
    DrawArc(worldAnchors.mWorldPoints[0], normal, axis.Normalized(),
      size, size, min, max, drawBoundaries, Color::Red, real(1.0) / detail, false);
  }
  if(perspective & JointDebugDrawConfigFlags::ObjectBPerspective)
  {
    ...
    DrawArc(worldAnchors.mWorldPoints[1], normal, axis.Normalized(),
      size, size, min, max, drawBoundaries, Color::Blue, real(1.0) / detail, true);
  }
}
