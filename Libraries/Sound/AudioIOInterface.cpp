...
bool AudioIOInterface::Initialize(bool initOutput, bool initInput)
{
  ...
  if (initOutput &&
      StreamInfoList[StreamTypes::Output].mStatus != StreamStatus::Initialized)
  {
    StreamInfo& outputInfo = StreamInfoList[StreamTypes::Output];
    outputInfo.mStatus = AudioIO.InitializeStream(StreamTypes::Output,
                                                  &outputInfo.mErrorMessage);
  ...
  if (initInput &&
      StreamInfoList[StreamTypes::Input].mStatus != StreamStatus::Initialized)
  {
    StreamInfo& inputInfo = StreamInfoList[StreamTypes::Input];
    inputInfo.mStatus =
        AudioIO.InitializeStream(StreamTypes::Input, &inputInfo.mErrorMessage);
...
bool AudioIOInterface::StopStreams(bool stopOutput, bool stopInput)
{
  ...
  {
    StreamInfoList[StreamTypes::Output].mStatus =
        AudioIO.StopStream(StreamTypes::Output,
                           &StreamInfoList[StreamTypes::Output].mErrorMessage);
  ...
  if (stopInput)
  {
    StreamInfoList[StreamTypes::Input].mStatus = AudioIO.StopStream(
        StreamTypes::Input, &StreamInfoList[StreamTypes::Input].mErrorMessage);
...
StreamStatus::Enum
AudioIOInterface::GetStreamStatus(StreamTypes::Enum whichStream)
{
  return StreamInfoList[whichStream].mStatus;
}

const String&
AudioIOInterface::GetStreamErrorMessage(StreamTypes::Enum whichStream)
{
...
void AudioIOInterface::GetInputDataThreaded(Array<float>& buffer,
...
void AudioIOInterface::SetOutputLatencyThreaded(AudioLatency::Enum newLatency)
{
  ...
  InitializeRingBuffer(OutputRingBuffer,
                       MixedOutputBuffer,
                       OutputBufferSizePerLatency[mOutputStreamLatency]);
...
{
  unsigned size =
      GetBufferSize(AudioIO.GetStreamSampleRate(StreamTypes::Output),
...
void AudioIOInterface::InitializeRingBuffer(RingBuffer& ringBuffer,
                                            float* buffer,
                                            unsigned size)
{
...
void AudioIOInterface::GetMixedOutputSamples(float* outputBuffer,
                                             const unsigned frames)
{
  ...
  unsigned samplesNeeded =
      frames * StreamInfoList[StreamTypes::Output].mChannels;
  ...
  if (samples < samplesNeeded)
    memset(
        outputBuffer + samples, 0, (samplesNeeded - samples) * sizeof(float));

  // Signal the semaphore for the mix thread
  MixThreadSemaphore.Increment();
}

void AudioIOInterface::SaveInputSamples(const float* inputBuffer,
                                        unsigned frames)
{
  // Note: remember this function is called asynchronously from the audio device
  // thread
...
{
  if (outputBuffer)
    ((AudioIOInterface*)data)
        ->GetMixedOutputSamples(outputBuffer, framesPerBuffer);
  if (inputBuffer)
