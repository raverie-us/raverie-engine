...
ZilchDefineType(Range<T>, builder, type)
{
  ...
  ZilchFullBindMethod(builder,
                      ...
                      ZilchNoNames);
  ZilchFullBindMethod(
      builder, type, &ZilchSelf::Reset, ZilchNoOverload, "Reset", ZilchNoNames);
}
...
public:
  ...
  void MoveNext()
  {
    if (this->IsEmpty())
    {
      ...
      if (ExecutableState::CallingState)
        ExecutableState::CallingState->ThrowException(
            "The range reached the end, but then an attempt was made to make "
...
#  define ZilchDefineRange(RangeType)                                          \
    ZilchDefineExternalBaseType(                                               \
        RangeType, ZZ::TypeCopyMode::ReferenceType, builder, type)             \
    {                                                                          \
      ZilchBindDefaultCopyDestructor();                                        \
      ZilchFullBindGetterSetter(builder,                                       \
                                type,                                          \
                                &ZZ::RangeAdapter<ZilchSelf>::All,             \
                                ZilchNoOverload,                               \
                                ZilchNoSetter,                                 \
                                ZilchNoOverload,                               \
                                "All");                                        \
      ...
      ZilchFullBindMethod(builder,                                             \
                          type,                                                \
                          &ZZ::RangeAdapter<ZilchSelf>::MoveNext,              \
                          ZilchNoOverload,                                     \
                          "MoveNext",                                          \
                          ZilchNoNames);                                       \
    }
#  define ZilchInitializeRange(RangeType) ZilchInitializeExternalType(RangeType)
#  define ZilchInitializeRangeAs(RangeType, Name)                              \
    ZilchInitializeExternalTypeAs(RangeType, Name)
} // namespace Zilch
