...
// BaseClassInitializerMustComeFirst
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "To make the order of operations very clear, the base class initializer "
...
// BaseClassMemberSameName
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The base class '%s' has a member of the same name '%s'. This is not "
...
// BinaryOperatorRightOperandNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The right operand is invalid or not found for the '%s' operator '%s'.";
  error.Name = "BinaryOperatorRightOperandNotFound";
  error.Reason =
      "When an operator between two operands, such as the '+' operator, has a "
...
// BlockCommentNotComplete
{
  ...
  error.Reason =
      "Commonly, users accidentally delete or mistype the end of a block "
...
// CastTypeNotFound
{
  ...
  error.Reason =
      "Casting allows us to convert one type to another. The 'as' operator was "
...
// ClassBodyNotComplete
{
  ...
  error.Reason =
      "In order for the compiler to understand where a class definition starts "
...

  {
    ...
    example.ExplanationOfFix =
        "In the error case, we started the definition of the class 'Animal' "
...
// ClassBodyNotFound
{
  ...
  error.Reason =
      "The compiler couldn't find the opening scope ({) for the class "
...
// ClassNameNotFound
{
  ...
  error.Reason =
      "Most likely the class name was invalid due to unnecessary symbols or to "
...

  {
    ...
    example.FixedCode = "class Animal\r\n{\r\n}";
    example.ExplanationOfFix =
        "In the error case, we forgot to add the name of the class after the "
...
// CompositionCycleDetected
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "A cycle of composition was detected. Eg. Type A includes Type B, Type B "
...
// DoWhileConditionalExpressionNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "Do while statement conditional expression is not properly closed.";
...
// ForEachRangeExpressionNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The foreach loop expects a range expression after the 'in' keyword.";
...
// FunctionArgumentListNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "Function declaration '%s' is missing the argument list opening '('.";
...

  {
    ...
    example.FixedCode =
        "class Animal\r\n{\r\n  function TestFn()\r\n  {\r\n  }\r\n}";
    example.ExplanationOfFix =
        "Added an empty argument list, but an argument list nonetheless.";
  }
}
...
// FunctionBodyNotFound
{
  ...
  error.Reason =
      "The compiler couldn't find the opening scope ({) for the function "
...

  {
    ...
    example.FixedCode = "class Animal\r\n{\r\n  function TestFn() : Real\r\n  "
                        "{\r\n    return 1.0;\r\n  }\r\n}";
    example.ExplanationOfFix =
        "Adding the rest of the function, such as the braces and return call.";
  }
}
...
// FunctionCallNamedArgumentNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The right hand side of a named argument '%s' is missing or invalid.";
...
// FunctionReturnTypeNotFound
{
  ...
  error.Reason =
      "':' was used after the parameter list, which signifies to the compiler "
...

  {
    ...
    example.ExplanationOfFix =
        "Fixed by removing the incomplete return type syntax.";
  }
}
...
// IndexerNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "An index operator was not closed with ']' or its contents were invalid.";
...
// InvalidBinaryOperation
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The binary '%s' operator '%s' is not valid with '%s' and '%s'.";
...
// InvalidEscapeInStringLiteral
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "An invalid escape sequence '\\%c' was used in a string literal.";
...
// LocalCreateMustBeValueType
{
  ...
  error.Reason =
      "The keyword 'local' is used to differentiate whether we are allocating "
...
// LocalVariableReferenceNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The variable '%s' could not be found. If the variable was defined "
...
// MemberIdMustBeUsedOnAMember
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The expression passed to memberid(...) must be accessing a member.";
...
// MemberVariableTypesCannotBeInferred
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The type of the member variable '%s' can only be inferred when being "
...
// NativeTypesRequireConstructors
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The native type '%s' has no constructors and cannot be created.";
...
// ParameterTypeNotFound
{
...

  {
    ...
    example.FixedCode = "class Animal\r\n{\r\n  function TestFn(count : "
                        "Int)\r\n  {\r\n  }\r\n}";
    example.ExplanationOfFix =
        "Simply add the type of the parameter and all is good.";
  }
}
...
// ParameterTypeSpecifierNotFound
{
...

  {
    ...
    example.FixedCode = "class Animal\r\n{\r\n  function TestFn(count : "
                        "Int)\r\n  {\r\n  }\r\n}";
    example.ExplanationOfFix =
        "Adding the type specifier, ':', and a type to the parameter 'count'.";
  }
}
...
// ParsingNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "Parsing could not be completed (we ran into something we didn't "
...
// PropertyDeclarationNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The declaration of the property '%s' must be closed by an ending '}'.";
...
// PropertyDelegateRequiresGetOrSet
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The property delegate operator '@' requires the property to have either "
...
// ReferenceToUndefinedTypeWithSimilarMember
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "It appears you are trying to reference the member '%s', and if so you "
...
// ReturnValueNotFound
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The return statement is expected to return a '%s' value, since the "
...
// SendsEventStatementNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "In a 'sends %s' statement, we expect to see 'sends EventName : "
...
// StructsCanOnlyContainValueTypes
{
  ...
  error.Reason =
      "A struct is a value type, which means it must be trivially copyable in "
...
// ThrowTypeMustDeriveFromException
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The exception we're throwing must derive from the 'Exception' type.";
...
// TimeoutSecondsExpectedIntegerLiteral
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "The timeout statement only accepts Integer literals for seconds.";
...
// TimeoutSecondsNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "Timeout statement's seconds is not properly closed with a ')'.";
...
// UnaryOperatorOperandNotFound
{
  ...
  error.Reason =
      "The operators such as negation '-' and logical-not '!' must have an "
...
// UnidentifiedSymbol
{
...

  {
    ErrorExample& example = error.Examples.PushBack();
    example.ErrorCode =
        "function #IsEmpty() : Bool32\r\n{\r\n  return mCount == 0;\r\n}";
    example.FixedCode =
        "function IsEmpty() : Bool32\r\n{\r\n  return mCount == 0;\r\n}";
    example.ExplanationOfFix =
        "In the error case, a # sign was introduced before the name of the "
...
// VariableInitializationNotComplete
{
  ...
  error.Reason =
      "This error occurred because the compiler was trying to parse a variable "
...

  {
    ...
    example.FixedCode =
        "var cat : Animal = new Cat();\r\nvar dog : Animal = new Dog();";
    example.ExplanationOfFix =
        "In the error case, we forgot to put a semicolon (;) at the end of the "
...
// VariableInitialValueNotFound
{
...

  {
    ...
    example.ExplanationOfFix =
        "In the error case, the variable 'lives' was going to be initialized, "
...
// VariableMustBeInitialized
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "Variable declaration '%s' is missing the initializer expression.";
  error.Name = "VariableMustBeInitialized";
  error.Reason =
      "All variables must be initialized for safety, and rather than letting "
...

  {
    ...
    example.FixedCode = "var lives : Int32 = 9;";
    example.ExplanationOfFix =
        "In the error case, the variable 'lives' did not have an initializer "
...
// VariableNameNotFound
{
  ...
  error.Reason =
      "One of the most common reasons for this error is that a keyword was "
...

  {
    ...
    example.FixedCode = "var lives : Int32 := 9;";
    example.ExplanationOfFix =
        "In the error case, we forgot to name the variable. We fixed it simply "
...
// VariableTypeNotFound
{
  ...
  error.Reason =
      "Mosty likely the type was just left out or the type contained some "
...

  {
    ...
    example.FixedCode =
        "var lives : Int32 := 9;\r\n\r\n/* OR */\r\n\r\nvar lives := 9;";
    example.ExplanationOfFix =
        "In the error case, the type of lives was left out yet the type "
...
// WhileConditionalExpressionNotComplete
{
  ErrorInfo& error = this->Errors.PushBack();
  error.Error =
      "While statement conditional expression is not properly closed.";
