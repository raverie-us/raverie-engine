...
public:
  ...
  // documentation string, etc) Note: No Zilch classes should be created before
  // this occurs
  ZilchSetup(SetupFlags::Type flags = SetupFlags::None);

  // Shuts down the shared global memory manager and releases any static
  // libraries Note: No Zilch classes should be created after this occurs
  ~ZilchSetup();

  // The setup is a singleton, which means two may not exist at the same time
  // However, you can create ZilchSetup and destroy it, then create another
  // ZilchSetup On destruction, this Instance will be cleared to null
