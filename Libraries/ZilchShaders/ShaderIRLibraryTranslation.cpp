...
{
  // This function should never be called via zilch. This is a function that cannot (or hasn't yet)
  // been given an actual implementation and is only used for binding purposes.
...
void ResolvePrimitiveGet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();

  // Since this isn't actually a vector, just return the left operand (ignore the index)
...
void ResolvePrimitiveSet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();

  // Since this isn't actually a vector, the "index" result is just the left operand
  IZilchShaderIR* selfInstance = translator->WalkAndGetResult(memberAccessNode->LeftOperand, context);
...
void ResolveVectorGet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ...
  IZilchShaderIR* accessChainOp = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, componentType->mPointerType, selfInstance, indexOperand, context);

  context->PushIRStack(accessChainOp);
}

void ResolveVectorSet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  // Get the 'this' vector type and component type
  Zilch::Type* zilchVectorType = memberAccessNode->LeftOperand->ResultType;
  ZilchShaderIRType* vectorType = translator->FindType(zilchVectorType, memberAccessNode->LeftOperand);
...
void ResolveMatrixGet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ResolveVectorGet(translator, functionCallNode, memberAccessNode, context);
}

void ResolveMatrixSet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
...
void TranslatePrimitiveDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::Type* zilchResultType, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void TranslatePrimitiveDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  TranslatePrimitiveDefaultConstructor(translator, fnCallNode->ResultType, context);
}

void TranslateBackupPrimitiveConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
...
void TranslateCompositeDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::Type* zilchResultType, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ZilchShaderIRType* resultType = translator->FindType(zilchResultType, nullptr);
...
void TranslateCompositeDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  TranslateCompositeDefaultConstructor(translator, fnCallNode->ResultType, context);
}

void TranslateBackupCompositeConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ZilchShaderIRType* resultType = translator->FindType(fnCallNode->ResultType, fnCallNode);

  // Create the op for construction but don't add it to the current block yet, we need to walk all arguments first
  ZilchShaderIROp* constructOp = translator->BuildIROpNoBlockAdd(OpType::OpCompositeConstruct, resultType, context);
...
void TranslateMatrixDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::Type* zilchResultType, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ZilchShaderIRType* resultType = translator->FindType(zilchResultType, nullptr);
  ...
  // @JoshD: Leave this out for now since it can produce a glsl translation error
  //constituent->mDebugResultName = "constituent";

  // Construct the composite but delay adding it to the stack until we've added all of the arguments to it
  ZilchShaderIROp* constructOp = translator->BuildIROpNoBlockAdd(OpType::OpCompositeConstruct, resultType, context);
  for(size_t i = 0; i < resultType->mComponents; ++i)
  {
...
void TranslateMatrixDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  TranslateMatrixDefaultConstructor(translator, fnCallNode->ResultType, context);
}

void TranslateMatrixFullConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  for(size_t i = 0; i < matrixType->mComponents; ++i)
  {
    ...
    // Now that we've finished constructing the parameters for the vector, actually add it
    // to the the current block and add it as a parameter to the matrix
...
ZilchShaderIROp* RecursivelyTranslateCompositeSplatConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchShaderIRType* type, ZilchShaderIROp* splatValueOp, ZilchSpirVFrontEndContext* context)
{
  // Terminate on the base case. Potentially need to handle translating the splat value op to the correct scalar type.
  ...
  // Construct the composite but delay adding it as an instruction until all of the sub-composites are created
  ...
  { 
    ...
    ZilchShaderIROp* constituentOp = RecursivelyTranslateCompositeSplatConstructor(translator, fnCallNode, staticTypeNode, componentType, splatValueOp, context);
    // @JoshD: Leave this out for now since this produces a glsl translation error
    //constituentOp->mDebugResultName = "constituent";
    constructOp->mArguments.PushBack(constituentOp);
  }
  // Now we can add the instruction to construct this composite since we've created all of the parameters
  currentBlock->mLines.PushBack(constructOp);
  return constructOp;
}

void TranslateCompositeSplatConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ZilchShaderIRType* resultType = translator->FindType(fnCallNode->ResultType, fnCallNode);
  ...
  ZilchShaderIROp* constructOp= RecursivelyTranslateCompositeSplatConstructor(translator, fnCallNode, staticTypeNode, resultType, splatValueOp, context);
...
{
  // A scalar component access on a scalar type is just the scalar itself (e.g. a.X => a)
...
void ResolveScalarSwizzle(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, StringParam memberName, ZilchSpirVFrontEndContext* context)
{
  ...
  // Vector swizzle doesn't exist on scalar types so just splat construct the relevant vector type
...
void ResolveVectorComponentAccess(ZilchSpirVFrontEnd* translator, ZilchShaderIROp* selfInstance, ZilchShaderIRType* componentType, byte componentName, ZilchSpirVFrontEndContext* context)
{
  ...
  // If the operand was a pointer type then we have to use an access chain to extract the sub-pointer
  if(selfInstance->IsResultPointerType())
  {
    ZilchShaderIROp* indexOp = translator->GetIntegerConstant(index, context);
    ZilchShaderIROp* accessChainOp = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, componentType->mPointerType, selfInstance, indexOp, context);
    context->PushIRStack(accessChainOp);
  }
  ...
  else
  {
    ...
    ZilchShaderIROp* accessChainOp = translator->BuildCurrentBlockIROp(OpType::OpCompositeExtract, componentType, selfInstance, indexLiteral, context);
    context->PushIRStack(accessChainOp);
  }
}
...
void ResolveVectorComponentAccess(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, byte componentName, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIRType* operandType = translator->FindType(memberAccessNode->LeftOperand->ResultType, memberAccessNode->LeftOperand);
...
void ResolveVectorSwizzle(ZilchSpirVFrontEnd* translator, IZilchShaderIR* selfInstance, ZilchShaderIRType* resultType, StringParam memberName, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* swizzleOp = translator->BuildCurrentBlockIROp(OpType::OpVectorShuffle, resultType, operandValueOp, operandValueOp, context);

  // For every swizzle element, figure out what index the sub-item is and add that as an argument
  for(size_t i = 0; i < memberName.SizeInBytes(); ++i)
  {
    byte memberValue = *(memberName.Data() + i);
...
void ResolveVectorSwizzle(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, StringParam memberName, ZilchSpirVFrontEndContext* context)
{
  // Figure out what the result type of this swizzle is
  ZilchShaderIRType* resultType = translator->FindType(memberAccessNode->ResultType, memberAccessNode);
...
void VectorBackupFieldResolver(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void ResolverVectorSwizzleSetter(ZilchSpirVFrontEnd* translator, IZilchShaderIR* selfInstance, ZilchShaderIROp* resultValue, ZilchShaderIRType* resultType, StringParam memberName, ZilchSpirVFrontEndContext* context)
{
  // To generate the new vector we need to perform vector shuffle which requires a value type.
  ...
  // from elements of the old and new vector using the shuffle instruction (parameters set below)
  ZilchShaderIROp* shuffleOp = translator->BuildCurrentBlockIROp(OpType::OpVectorShuffle, resultType, instanceValue, resultValue, context);

  // The shuffle operator picks components from the two vectors by index as if they were
  // laid out in one contiguous block of memory. By default assume that all components will
  // come from the same location in the original vector. To copy elements from the new vector 
...
void ResolverVectorSwizzleSetter(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchShaderIROp* resultValue, ZilchSpirVFrontEndContext* context)
{
  String memberName = memberAccessNode->Name;
  ZilchShaderIRType* resultType = translator->FindType(memberAccessNode->LeftOperand->ResultType, memberAccessNode->LeftOperand);
...
void VectorBackupPropertySetter(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchShaderIROp* resultValue, ZilchSpirVFrontEndContext* context)
{
  String memberName = memberAccessNode->Name;
...
bool MatrixElementAccessResolver(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context, Zilch::MatrixUserData& matrixUserData)
{
  ...
  ZilchShaderIROp* accessOp = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, resultType->mPointerType, operandResult, indexYConstant, indexXConstant, context);
  context->PushIRStack(accessOp);
  return true;
}

void MatrixBackupFieldResolver(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void TranslateQuaternionDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::Type* zilchResultType, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  ZilchShaderIROp* constructOp = translator->BuildIROp(context->GetCurrentBlock(), OpType::OpCompositeConstruct, quaternionType, vec4ConstructOp, context);
  context->PushIRStack(constructOp);
}

void TranslateQuaternionDefaultConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  TranslateQuaternionDefaultConstructor(translator, fnCallNode->ResultType, context);
}

void QuaternionBackupFieldResolver(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  ZilchShaderIROp* vec4Instance = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, vec4Type->mPointerType, selfInstance, vec4OffsetConstant, context);
...
void QuaternionBackupPropertySetter(ZilchSpirVFrontEnd* translator, Zilch::MemberAccessNode* memberAccessNode, ZilchShaderIROp* resultValue, ZilchSpirVFrontEndContext* context)
{
  String memberName = memberAccessNode->Name;
...
void ResolveQuaternionTypeCount(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
...
void ResolveQuaternionGet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  IZilchShaderIR* leftOperand = translator->WalkAndGetResult(memberAccessNode->LeftOperand, context);
  ZilchShaderIROp* selfInstance = translator->GetOrGeneratePointerTypeFromIR(leftOperand, context);
  ZilchShaderIRType* vec4Type = selfInstance->mResultType->mDereferenceType->GetSubType(0);
  ...
  // Generate the access chain to get the element within the vector
  IZilchShaderIR* accessChainOp = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, realType->mPointerType, vec4Instance, indexOperand, context);

  context->PushIRStack(accessChainOp);
}

void ResolveQuaternionSet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  IZilchShaderIR* leftOperand = translator->WalkAndGetResult(memberAccessNode->LeftOperand, context);
  ZilchShaderIROp* selfInstance = translator->GetOrGeneratePointerTypeFromIR(leftOperand, context);
  ZilchShaderIRType* vec4Type = selfInstance->mResultType->mDereferenceType->GetSubType(0);
  ...
  ZilchShaderIROp* vec4Instance = translator->BuildCurrentBlockIROp(OpType::OpAccessChain, vec4Type->mPointerType, selfInstance, offsetConstant, context);
...
void TranslateQuaternionSplatConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* subConstructOp = RecursivelyTranslateCompositeSplatConstructor(translator, fnCallNode, staticTypeNode, vec4Type, splatValueOp, context);

  // Construct the quaternion from the vec4
  ZilchShaderIROp* vec4Value = translator->GetOrGenerateValueTypeFromIR(subConstructOp, context);
  ZilchShaderIROp* quaternionConstructOp = translator->BuildCurrentBlockIROp(OpType::OpCompositeConstruct, quaternionType, vec4Value, context);
...
void TranslateBackupQuaternionConstructor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* fnCallNode, Zilch::StaticTypeNode* staticTypeNode, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* quaternionConstructOp = translator->BuildCurrentBlockIROp(OpType::OpCompositeConstruct, quaternionType, vec4ConstructOp, context);
  // Also mark this as the return of this tree
  context->PushIRStack(quaternionConstructOp);
}

void ResolveColor(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* /*functionCallNode*/, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
...
void RegisterColorsOps(ZilchSpirVFrontEnd* translator, ZilchShaderIRLibrary* shaderLibrary, TypeGroups& types)
{
  ...
  ZilchShaderIRType* colorsShaderType = translator->MakeStructType(shaderLibrary, colors->Name, colors, spv::StorageClass::StorageClassGeneric);
