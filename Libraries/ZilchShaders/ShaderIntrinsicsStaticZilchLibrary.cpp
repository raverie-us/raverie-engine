...
ShaderIntrinsicsStaticZilchLibrary*
    ShaderIntrinsicsStaticZilchLibrary::mInstance = nullptr;
...
void ShaderIntrinsicsStaticZilchLibrary::Parse(ZilchSpirVFrontEnd* translator)
{
  ...
  shaderLibrary->mDependencies->PushBack(
      ZilchShaderIRCore::GetInstance().GetLibrary());
  ...
  Zilch::BoundType* zilchSampledDepthImage2d =
      ZilchTypeId(Zilch::SampledDepthImage2d);
  Zilch::BoundType* zilchSampledImageCube =
      ZilchTypeId(Zilch::SampledImageCube);
  Zilch::BoundType* zilchStorageImage2d = ZilchTypeId(Zilch::StorageImage2d);

  // Create the sampler type
  ZilchShaderIRType* samplerType =
  ...
  CreateImageAndSampler(translator,
                        shaderLibrary,
                        core.RealType,
                        zilchImage2d,
                        zilchSampledImage2d,
  ...
  CreateImageAndSampler(translator,
                        ...
                        core.RealType,
                        zilchImageCube,
                        zilchSampledImageCube,
                        spv::DimCube,
                        0);
  CreateStorageImage(translator,
                     shaderLibrary,
                     core.RealType,
                     zilchStorageImage2d,
                     spv::Dim2D,
                     0,
                     spv::ImageFormatRgba32f);

  RegisterShaderIntrinsics(translator, shaderLibrary);

  // Register the template resolver for fixed array
  shaderLibrary->RegisterTemplateResolver("FixedArray[Type,Integer]",
                                          FixedArrayResolver);
  String runtimeArayResolverName =
      BuildString(SpirVNameSettings::mRuntimeArrayTypeName, "[Type]");
  shaderLibrary->RegisterTemplateResolver(runtimeArayResolverName,
                                          RuntimeArrayResolver);

  shaderLibrary->RegisterTemplateResolver("PointInput[Type]",
                                          GeometryStreamInputResolver);
  shaderLibrary->RegisterTemplateResolver("LineInput[Type]",
                                          GeometryStreamInputResolver);
  shaderLibrary->RegisterTemplateResolver("TriangleInput[Type]",
                                          GeometryStreamInputResolver);
  shaderLibrary->RegisterTemplateResolver("PointOutput[Type]",
                                          GeometryStreamOutputResolver);
  shaderLibrary->RegisterTemplateResolver("LineOutput[Type]",
                                          GeometryStreamOutputResolver);
  shaderLibrary->RegisterTemplateResolver("TriangleOutput[Type]",
                                          GeometryStreamOutputResolver);
...
void ShaderIntrinsicsStaticZilchLibrary::CreateImageAndSampler(
    ...
    int depthMode)
{
  ...
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(dimension)); // Dim
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(depthMode)); // Depth
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(0)); // Arrayed
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(0)); // MultiSampled
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(1)); // Sampled
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(spv::ImageFormatUnknown));
...
void ShaderIntrinsicsStaticZilchLibrary::CreateStorageImage(
    ZilchSpirVFrontEnd* translator,
    ZilchShaderIRLibrary* shaderLibrary,
    Zilch::BoundType* zilchSampledType,
    Zilch::BoundType* zilchImageType,
    ...
    int imageFormat)
{
  ...
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(0)); // MultiSampled
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(2)); // Sampled
  imageType->mParameters.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(imageFormat));
  translator->MakeShaderTypeMeta(imageType, nullptr);
}

void ShaderIntrinsicsStaticZilchLibrary::PopulateStageRequirementsData(
    ZilchShaderIRLibrary* shaderLibrary)
{
