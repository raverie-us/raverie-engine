...
void ZilchShaderIRCore::Parse(ZilchSpirVFrontEnd* translator)
{
  ...
  RegisterPrimitiveFunctions(
      translator, shaderLibrary, types, types.mRealVectorTypes[0]);
  RegisterPrimitiveFunctions(
      translator, shaderLibrary, types, types.mIntegerVectorTypes[0]);
  RegisterPrimitiveFunctions(
      translator, shaderLibrary, types, types.mBooleanVectorTypes[0]);

  // Add all static/instance functions for vector types
  RegisterVectorFunctions(
      translator, shaderLibrary, types, types.mRealVectorTypes);
  RegisterVectorFunctions(
      translator, shaderLibrary, types, types.mIntegerVectorTypes);
  RegisterVectorFunctions(
      translator, shaderLibrary, types, types.mBooleanVectorTypes);

  // Add all static/instance functions for matrix types (only float matrices
  // exist)
  RegisterMatrixFunctions(
      translator, shaderLibrary, types, types.mRealMatrixTypes);
  // Also the quaternion type
  RegisterQuaternionFunctions(
      translator, shaderLibrary, types, types.mQuaternionType);
...
void ZilchShaderIRCore::MakeMathTypes(ZilchSpirVFrontEnd* translator,
                                      ZilchShaderIRLibrary* shaderLibrary,
                                      TypeGroups& types)
{
  Zilch::Core& core = Zilch::Core::GetInstance();

  types.mVoidType = translator->MakeCoreType(shaderLibrary,
                                             ShaderIRTypeBaseType::Void,
                                             0,
                                             nullptr,
                                             core.VoidType,
                                             false);

  ZilchShaderIRType* floatType = translator->MakeCoreType(
      shaderLibrary, ShaderIRTypeBaseType::Float, 1, nullptr, core.RealType);
  ZilchShaderIRType* float2Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               2,
                               floatType,
                               core.Real2Type);
  ZilchShaderIRType* float3Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               3,
                               floatType,
                               core.Real3Type);
  ZilchShaderIRType* float4Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               4,
                               floatType,
  ...
  for (size_t y = 2; y <= 4; ++y)
  {
    for (size_t x = 2; x <= 4; ++x)
    {
      ...
      Zilch::BoundType* zilchMatrixType =
          core.GetLibrary()->BoundTypes.FindValue(matrixName, nullptr);
      ZilchShaderIRType* shaderMatrixType =
          translator->MakeCoreType(shaderLibrary,
                                   ShaderIRTypeBaseType::Matrix,
                                   y,
                                   basisType,
                                   zilchMatrixType);
  ...
  ZilchShaderIRType* intType = translator->MakeCoreType(
      shaderLibrary, ShaderIRTypeBaseType::Int, 1, nullptr, core.IntegerType);
  ZilchShaderIRType* int2Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               2,
                               intType,
                               core.Integer2Type);
  ZilchShaderIRType* int3Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               3,
                               intType,
                               core.Integer3Type);
  ZilchShaderIRType* int4Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               4,
                               intType,
  ...
  ZilchShaderIRType* boolType = translator->MakeCoreType(
      shaderLibrary, ShaderIRTypeBaseType::Bool, 1, nullptr, core.BooleanType);
  ZilchShaderIRType* bool2Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               2,
                               boolType,
                               core.Boolean2Type);
  ZilchShaderIRType* bool3Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               3,
                               boolType,
                               core.Boolean3Type);
  ZilchShaderIRType* bool4Type =
      translator->MakeCoreType(shaderLibrary,
                               ShaderIRTypeBaseType::Vector,
                               4,
                               boolType,
...
void ZilchShaderIRCore::RegisterPrimitiveFunctions(
    ...
    ZilchShaderIRType* shaderType)
{
  ...
  TypeResolvers& primitiveTypeResolvers =
      shaderLibrary->mTypeResolvers[zilchType];
  primitiveTypeResolvers.mDefaultConstructorResolver =
      &TranslatePrimitiveDefaultConstructor;
  primitiveTypeResolvers.mBackupConstructorResolver =
      &TranslateBackupPrimitiveConstructor;
  primitiveTypeResolvers.RegisterBackupFieldResolver(
      &ScalarBackupFieldResolver);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetStaticProperty(zilchType, "Count")->Get, ResolveVectorTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetInstanceProperty(zilchType, "Count")->Get, ResolveVectorTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName),
      ResolvePrimitiveGet);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(
          zilchType, Zilch::OperatorSet, intTypeName, zilchTypeName),
      ResolvePrimitiveSet);
}

void ZilchShaderIRCore::RegisterVectorFunctions(
    ZilchSpirVFrontEnd* translator,
    ZilchShaderIRLibrary* shaderLibrary,
    TypeGroups& types,
    Array<ZilchShaderIRType*>& vectorTypes)
{
  ...
  for (size_t i = 1; i < vectorTypes.Size(); ++i)
  {
    ...
    primitiveTypeResolvers.RegisterBackupFieldResolver(
        &VectorBackupFieldResolver);
    primitiveTypeResolvers.RegisterConstructorResolver(
        zilchType->GetDefaultConstructor(),
        TranslateCompositeDefaultConstructor);
    primitiveTypeResolvers.RegisterConstructorResolver(
        GetConstructor(zilchType, zilchComponentTypeName),
    ...
    primitiveTypeResolvers.RegisterFunctionResolver(
        GetMemberOverloadedFunction(
            zilchType, Zilch::OperatorSet, intTypeName, zilchComponentTypeName),
        ResolveVectorSet);
    primitiveTypeResolvers.RegisterBackupSetterResolver(
        VectorBackupPropertySetter);
    primitiveTypeResolvers.mBackupConstructorResolver =
        &TranslateBackupCompositeConstructor;
  }
}

void ZilchShaderIRCore::RegisterMatrixFunctions(
    ZilchSpirVFrontEnd* translator,
    ZilchShaderIRLibrary* shaderLibrary,
    TypeGroups& types,
    Array<ZilchShaderIRType*>& matrixTypes)
{
  ...
  for (size_t i = 0; i < matrixTypes.Size(); ++i)
  {
    ...
    TypeResolvers& matrixTypeResolver =
        shaderLibrary->mTypeResolvers[zilchType];
    // Constructors
    matrixTypeResolver.mDefaultConstructorResolver =
        &TranslateMatrixDefaultConstructor;
    matrixTypeResolver.RegisterConstructorResolver(
        zilchType->GetDefaultConstructor(), TranslateMatrixDefaultConstructor);
    matrixTypeResolver.RegisterConstructorResolver(
        GetConstructor(zilchType, zilchScalarType->ToString()),
        TranslateCompositeSplatConstructor);
    // Constructor for each scalar entry in the matrix
    Array<String> constructorParams;
    for (size_t i = 0; i < shaderType->mComponents * componentType->mComponents;
         ++i)
      constructorParams.PushBack(zilchScalarType->ToString());
    matrixTypeResolver.RegisterConstructorResolver(
        GetConstructor(zilchType, constructorParams),
...
void ZilchShaderIRCore::RegisterQuaternionFunctions(
    ZilchSpirVFrontEnd* translator,
    ZilchShaderIRLibrary* shaderLibrary,
    TypeGroups& types,
    ZilchShaderIRType* quaternionType)
{
  ...
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetStaticProperty(zilchType, "Count")->Get, ResolveQuaternionTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetInstanceProperty(zilchType, "Count")->Get, ResolveQuaternionTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName),
      ResolveQuaternionGet);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(
          zilchType, Zilch::OperatorSet, intTypeName, zilchComponentTypeName),
      ResolveQuaternionSet);
  primitiveTypeResolvers.RegisterBackupSetterResolver(
      QuaternionBackupPropertySetter);
  primitiveTypeResolvers.mBackupConstructorResolver =
      &TranslateBackupQuaternionConstructor;
}
