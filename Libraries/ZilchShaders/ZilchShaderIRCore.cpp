  ...
  for (size_t y = 2; y <= 4; ++y)
  {
    for (size_t x = 2; x <= 4; ++x)
    {
      ...
      ZilchShaderIRType* shaderMatrixType =
          translator->MakeCoreType(shaderLibrary, ShaderIRTypeBaseType::Matrix, y, basisType, zilchMatrixType);
...
void ZilchShaderIRCore::RegisterPrimitiveFunctions(ZilchSpirVFrontEnd* translator,
                                                   ZilchShaderIRLibrary* shaderLibrary,
                                                   TypeGroups& types,
                                                   ZilchShaderIRType* shaderType)
{
  ...
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName), ResolvePrimitiveGet);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorSet, intTypeName, zilchTypeName), ResolvePrimitiveSet);
}

void ZilchShaderIRCore::RegisterVectorFunctions(ZilchSpirVFrontEnd* translator,
                                                ZilchShaderIRLibrary* shaderLibrary,
                                                TypeGroups& types,
                                                Array<ZilchShaderIRType*>& vectorTypes)
{
  ...
  for (size_t i = 1; i < vectorTypes.Size(); ++i)
  {
    ...
    primitiveTypeResolvers.RegisterConstructorResolver(zilchType->GetDefaultConstructor(),
    ...
    primitiveTypeResolvers.RegisterFunctionResolver(
        GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName), ResolveVectorGet);
    primitiveTypeResolvers.RegisterFunctionResolver(
        GetMemberOverloadedFunction(zilchType, Zilch::OperatorSet, intTypeName, zilchComponentTypeName),
        ResolveVectorSet);
...
void ZilchShaderIRCore::RegisterMatrixFunctions(ZilchSpirVFrontEnd* translator,
                                                ZilchShaderIRLibrary* shaderLibrary,
                                                TypeGroups& types,
                                                Array<ZilchShaderIRType*>& matrixTypes)
{
  ...
  for (size_t i = 0; i < matrixTypes.Size(); ++i)
  {
    ...
    matrixTypeResolver.RegisterConstructorResolver(zilchType->GetDefaultConstructor(),
                                                   TranslateMatrixDefaultConstructor);
    matrixTypeResolver.RegisterConstructorResolver(GetConstructor(zilchType, zilchScalarType->ToString()),
                                                   TranslateCompositeSplatConstructor);
    ...
    matrixTypeResolver.RegisterConstructorResolver(GetConstructor(zilchType, constructorParams),
                                                   TranslateMatrixFullConstructor);

    // Fields (M00, etc...)
    matrixTypeResolver.RegisterBackupFieldResolver(&MatrixBackupFieldResolver);

    matrixTypeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName),
                                                ResolveMatrixGet);
...
void ZilchShaderIRCore::RegisterQuaternionFunctions(ZilchSpirVFrontEnd* translator,
                                                    ZilchShaderIRLibrary* shaderLibrary,
                                                    TypeGroups& types,
                                                    ZilchShaderIRType* quaternionType)
{
  ...
  primitiveTypeResolvers.RegisterFunctionResolver(GetStaticProperty(zilchType, "Count")->Get,
                                                  ResolveQuaternionTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(GetInstanceProperty(zilchType, "Count")->Get,
                                                  ResolveQuaternionTypeCount);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorGet, intTypeName), ResolveQuaternionGet);
  primitiveTypeResolvers.RegisterFunctionResolver(
      GetMemberOverloadedFunction(zilchType, Zilch::OperatorSet, intTypeName, zilchComponentTypeName),
