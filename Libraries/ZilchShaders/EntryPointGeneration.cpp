...
InterfaceInfoGroup::FieldInfo*
InterfaceInfoGroup::FindFieldInfo(const ShaderFieldKey& fieldKey)
{
...
InterfaceInfoGroup::FieldInfo*
InterfaceInfoGroup::FindOrCreateFieldInfo(const ShaderFieldKey& fieldKey)
{
...
ShaderInterfaceStruct::GetField(const ShaderFieldKey& fieldKey)
{
  for (size_t i = 0; i < mFields.Size(); ++i)
  {
...
ZilchShaderIROp* ShaderInterfaceStruct::GetFieldPointerByIndex(
    size_t index,
    EntryPointGeneration* entryPointGeneration,
    BasicBlock* block,
    spv::StorageClass storageClass)
{
  return GetFieldPointerByIndex(
      mInstance, index, entryPointGeneration, block, storageClass);
}

ZilchShaderIROp* ShaderInterfaceStruct::GetFieldPointerByIndex(
    ZilchShaderIROp* instance,
    size_t index,
    EntryPointGeneration* entryPointGeneration,
    BasicBlock* block,
    spv::StorageClass storageClass)
{
  return entryPointGeneration->GetMemberInstanceFrom(
      block, instance, index, storageClass);
}

void ShaderInterfaceStruct::DeclareInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    ...
    EntryPointInfo* entryPointInfo)
{
  ...
  ZilchShaderIRType* blockType =
      translator->MakeStructType(translator->mLibrary,
                                 blockTypeName,
                                 nullptr,
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    entryPointGeneration->CreateShaderInterfaceField(
        interfaceField, interfaceGroup, i);

    // Create reflection data for this field
    ShaderResourceReflectionData& memberReflection =
        fieldList[i].mReflectionData;
    memberReflection.mInstanceName = interfaceField.mFieldName;
    memberReflection.mTypeName =
        interfaceField.mFieldType->mZilchType->ToString();

    // Find the field type we're reading/writing to (storage type function)
    ZilchShaderIRType* fieldShaderType =
        interfaceField.mFieldType->mDereferenceType;
...
void ShaderInterfaceStruct::DecorateInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo)
{
  ...
  entryPointGeneration->WriteTypeDecorations(
      interfaceGroup.mTypeDecorations, decorationBlock, mType);
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    // here).
    entryPointGeneration->WriteTypeDecorations(
        fieldInfo.mTypeDecorations,
        decorationBlock,
        memberInterfaceType->mDereferenceType);
  }
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    ZilchShaderIRConstantLiteral* memberIndexLiteral =
        translator->GetOrCreateConstantIntegerLiteral(i);

    entryPointGeneration->WriteMemberDecorations(
        fieldInfo.mDecorations, decorationBlock, mType, memberIndexLiteral);
  }
}

void ShaderInterfaceStruct::DefineInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo)
{
  ...
  ZilchShaderIROp* interfaceInstance =
      translator->BuildOpVariable(&entryPointInfo->mVariables,
                                  interfaceType->mPointerType,
                                  interfaceGroup.mStorageClass,
                                  context);
  ...
  entryPointGeneration->WriteTypeDecorations(
      interfaceGroup.mInstanceDecorations, decorationBlock, interfaceInstance);
...
void ShaderInterfaceStruct::CopyInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyHelperData)
{
  ...
  for (size_t i = 0; i < mFields.Size(); ++i)
  {
    ...
    for (size_t j = 0; j < fieldInfo.mLinkedFields.Size(); ++j)
    {
      ...
      int memberIndex =
          currentType->mMemberNamesToIndex.FindValue(linkedFieldName, -1);
      ...
      ZilchShaderIROp* blockIndexConstant =
          translator->GetIntegerConstant(blockMemberIndex, context);
      ZilchShaderIROp* blockAccessChainOp =
          translator->BuildIROp(targetFnBlock,
                                OpType::OpAccessChain,
                                blockFieldMemberType,
                                blockVarOp,
                                blockIndexConstant,
                                context);

      // Get a pointer to the field on 'this'
      ZilchShaderIROp* selfIndexConstant =
          translator->GetIntegerConstant(memberIndex, context);
      ZilchShaderIROp* selfAccessChainOp =
          translator->BuildIROp(targetFnBlock,
                                OpType::OpAccessChain,
                                memberPtrType,
                                copyHelperData.mSelfParam,
                                selfIndexConstant,
                                context);
...
ShaderInterfaceField*
ShaderInterfaceGlobals::GetField(const ShaderFieldKey& fieldKey)
{
  for (size_t i = 0; i < mFields.Size(); ++i)
  {
    ShaderInterfaceField& fieldInfo = mFields[i];
...
ZilchShaderIROp* ShaderInterfaceGlobals::GetFieldPointerByIndex(
    size_t index,
    EntryPointGeneration* entryPointGeneration,
    BasicBlock* block,
    spv::StorageClass storageClass)
...
void ShaderInterfaceGlobals::DeclareInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo)
{
  ...
  for (size_t i = 0; i < interfaceGroup.mFields.Size(); ++i)
  {
    ...
    // Create reflection data for this field
    ShaderResourceReflectionData& memberReflection =
        interfaceGroup.mFields[i].mReflectionData;
    memberReflection.mInstanceName = interfaceField.mFieldName;
    memberReflection.mTypeName =
        interfaceField.mFieldType->mZilchType->ToString();
  }
...
void ShaderInterfaceGlobals::DecorateInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo)
{
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    // here).
    entryPointGeneration->WriteTypeDecorations(
        fieldList[i].mTypeDecorations,
        decorationBlock,
        interfaceFieldMemberType->mDereferenceType);
  }
}

void ShaderInterfaceGlobals::DefineInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo)
{
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    ZilchShaderIROp* varOp =
        translator->BuildOpVariable(&entryPointInfo->mVariables,
                                    interfaceFieldMemberType,
                                    interfaceGroup.mStorageClass,
                                    context);
    ...
    entryPointGeneration->WriteTypeDecorations(
        fieldInfo.mDecorations, decorationBlock, varOp);
...
void ShaderInterfaceGlobals::CopyInterfaceType(
    EntryPointGeneration* entryPointGeneration,
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyHelperData)
...
ZilchShaderIROp* ShaderInterfaceStructArray::GetPointerByIndex(
    IZilchShaderIR* index,
    ...
    spv::StorageClass storageClass)
{
  ...
  ZilchShaderIROp* itemInstance = entryPointGeneration->mTranslator->BuildIROp(
      block,
      OpType::OpAccessChain,
      shaderSubType->mPointerType,
      mInstance,
      index,
      entryPointGeneration->mContext);

  // Find the field
  ShaderInterfaceField* fieldInfo = mStructType->GetField(fieldKey);
  if (fieldInfo != nullptr)
    return entryPointGeneration->GetMemberInstanceFrom(
        block, itemInstance, fieldInfo->mFieldIndex, storageClass);

  Error("Failed to find field");
  return nullptr;
}

GeometryStageInfo::GeometryStageInfo(ZilchSpirVFrontEnd* translator,
                                     ...
                                     Zilch::GenericFunctionNode* node)
{
  ...
  mOutputVertexType =
      mOutputStreamType->mParameters[0]->As<ZilchShaderIRType>();
  mInputStreamUserData =
      mInputStreamType->mZilchType->Has<Zilch::GeometryStreamUserData>();
  mOutputStreamUserData =
      mOutputStreamType->mZilchType->Has<Zilch::GeometryStreamUserData>();
}
...
void EntryPointGeneration::DeclareVertexInterface(
    ...
    ZilchSpirVFrontEndContext* context)
{
  ...
  EntryPointHelperFunctionData copyInputsData =
      GenerateCopyHelper(function, "CopyInputs");
  EntryPointHelperFunctionData copyOutputsData =
      GenerateCopyHelper(function, "CopyOutputs");
  BuildBasicEntryPoint(
      node, function, copyInputsData.mFunction, copyOutputsData.mFunction);
  ...
  // to this output
  interfaceInfo.mOutputs.mTypeDecorations.PushBack(
      InterfaceInfoGroup::DecorationParam(spv::DecorationBlock));
  interfaceInfo.mOutputs.mInstanceDecorations.PushBack(
      InterfaceInfoGroup::DecorationParam(spv::DecorationLocation, 0));
  ...
  // groups
  DeclareStageBlocks(
      interfaceInfo, entryPointInfo, copyInputsData, copyOutputsData);

  // Make sure to add the terminator op for both functions
  copyInputsData.mBlock->mTerminatorOp = translator->BuildIROp(
      copyInputsData.mBlock, OpType::OpReturn, nullptr, context);
  copyOutputsData.mBlock->mTerminatorOp = translator->BuildIROp(
      copyOutputsData.mBlock, OpType::OpReturn, nullptr, context);
...
void EntryPointGeneration::DeclareGeometryInterface(
    ZilchSpirVFrontEnd* translator,
    Zilch::GenericFunctionNode* node,
    ZilchShaderIRFunction* function,
    ZilchSpirVFrontEndContext* context)
{
  ...
  EntryPointHelperFunctionData copyInputsData = GenerateGeometryCopyHelper(
      function, "CopyInputs", stageInfo.mInputStreamType, copyInputsStreamVar);
  ...
  WriteGeometryStageInterface(
      function, stageInfo, entryPointInfo, copyInputsData, copyInputsStreamVar);
  ...
  // Write out the true entry point
  {
    ...
    ZilchShaderIROp* inputVar = translator->BuildOpVariable(
        stageInfo.mInputStreamType->mPointerType, context);
    inputVar->mDebugResultName = "inputStream";
    ZilchShaderIROp* outputVar = translator->BuildOpVariable(
        stageInfo.mOutputStreamType->mPointerType, context);
    ...
    translator->BuildIROp(currentBlock,
                          OpType::OpFunctionCall,
                          voidType,
                          copyInputsData.mFunction,
                          selfVar,
                          inputVar,
                          context);
    ZilchShaderIROp* fnCallOp = translator->BuildIROpNoBlockAdd(
        OpType::OpFunctionCall, voidType, context);
    fnCallOp->mArguments.PushBack(function);
    translator->WriteFunctionCallArguments(entryPointArguments,
                                           voidType,
                                           function->mFunctionType,
                                           fnCallOp,
                                           context);
  ...
  ShaderIRAttributeParameter* maxVerticesParam =
      geometryAttribute->FindFirstParameter(nameSettings.mMaxVerticesParam);
  int maxVertices = maxVerticesParam->GetIntValue();
  ZilchShaderIRConstantLiteral* modeLiteral =
      translator->GetOrCreateConstantIntegerLiteral(
          spv::ExecutionModeOutputVertices);
  ZilchShaderIRConstantLiteral* vertexCountLiteral =
      translator->GetOrCreateConstantIntegerLiteral(maxVertices);
  translator->BuildIROp(executionModes,
                        OpType::OpExecutionMode,
                        nullptr,
                        entryPointFn,
                        modeLiteral,
                        vertexCountLiteral,
  ...
  modeLiteral = translator->GetOrCreateConstantIntegerLiteral(
      stageInfo.mInputStreamUserData->mExecutionMode);
  translator->BuildIROp(executionModes,
                        OpType::OpExecutionMode,
                        nullptr,
                        entryPointFn,
  ...
  modeLiteral = translator->GetOrCreateConstantIntegerLiteral(
      stageInfo.mOutputStreamUserData->mExecutionMode);
  translator->BuildIROp(executionModes,
                        OpType::OpExecutionMode,
                        nullptr,
                        entryPointFn,
...
void EntryPointGeneration::DeclarePixelInterface(
    ZilchSpirVFrontEnd* translator,
    Zilch::GenericFunctionNode* node,
    ZilchShaderIRFunction* function,
    ZilchSpirVFrontEndContext* context)
{
  ...
  interfaceInfo.mInputs.mTypeDecorations.PushBack(
      InterfaceInfoGroup::DecorationParam(spv::DecorationBlock));
  interfaceInfo.mInputs.mInstanceDecorations.PushBack(
      InterfaceInfoGroup::DecorationParam(spv::DecorationLocation, 0));
...
void EntryPointGeneration::DeclareComputeInterface(
    ZilchSpirVFrontEnd* translator,
    Zilch::GenericFunctionNode* node,
    ZilchShaderIRFunction* function,
    ZilchSpirVFrontEndContext* context)
{
  ...
  Zilch::ComputeFragmentUserData* computeUserData =
      currentType->mZilchType->Has<Zilch::ComputeFragmentUserData>();
  // Write out the local size execution mode
  BasicBlock* block = &entryPointInfo->mExecutionModes;
  ZilchShaderIROp* executionModeOp =
      mTranslator->BuildIROp(block,
                             OpType::OpExecutionMode,
                             nullptr,
                             entryPointInfo->mEntryPointFn,
                             context);
  executionModeOp->mArguments.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(
          spv::ExecutionModeLocalSize));
  executionModeOp->mArguments.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(
          computeUserData->mLocalSizeX));
  executionModeOp->mArguments.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(
          computeUserData->mLocalSizeY));
  executionModeOp->mArguments.PushBack(
      translator->GetOrCreateConstantIntegerLiteral(
          computeUserData->mLocalSizeZ));
...
void EntryPointGeneration::CreateEntryPointFunction(
    Zilch::GenericFunctionNode* node,
    ZilchShaderIRFunction* function,
    ...
    BasicBlock*& entryPointBlock)
{
  ...
  ZilchShaderIRFunction* entryPointFn =
      currentType->CreateFunction(translator->mLibrary);
  ...
  // current type.
  entryPointFn->mDebugResultName = BuildString(
      "EntryPoint_", function->mName, "_", context->mCurrentType->mName);
  entryPointFn->mName = entryPointFn->mDebugResultName;
  Array<Zilch::Type*> signature;
  translator->GenerateFunctionType(
      node, entryPointFn, ZilchTypeId(void), signature, context);
  ...
  ZilchShaderIRFunction* globalsInitializerFn =
      CreateGlobalsInitializerFunction(node);
  translator->BuildIROp(entryPointBlock,
                        OpType::OpFunctionCall,
                        voidType,
                        globalsInitializerFn,
                        context);
  ...
  // check both.
  ZilchShaderIRFunction* defaultConstructorFn =
      currentType->mAutoDefaultConstructor;
  if (defaultConstructorFn == nullptr)
    defaultConstructorFn = translator->mLibrary->FindFunction(
        currentType->mZilchType->GetDefaultConstructor());
  ...
  ZilchShaderIRType* defaultConstructorReturnType =
      defaultConstructorFn->GetReturnType();
  ZilchShaderIROp* constructorOp =
      translator->BuildIROp(entryPointBlock,
                            OpType::OpFunctionCall,
                            defaultConstructorReturnType,
                            defaultConstructorFn,
                            selfVarOp,
                            context);
}

void EntryPointGeneration::BuildBasicEntryPoint(
    Zilch::GenericFunctionNode* node,
    ZilchShaderIRFunction* function,
...
ZilchShaderIRFunction* EntryPointGeneration::CreateGlobalsInitializerFunction(
    Zilch::GenericFunctionNode* node)
{
  ZilchSpirVFrontEnd* translator = mTranslator;
  ZilchSpirVFrontEndContext* context = mContext;
  ...
  ZilchShaderIRFunction* initializeGlobalsFunction =
      currentType->CreateFunction(translator->mLibrary);
  initializeGlobalsFunction->mDebugResultName =
      initializeGlobalsFunction->mName = "InitializeGlobals";
  // Make the function type
  Array<Zilch::Type*> parameters;
  translator->GenerateFunctionType(
      node, initializeGlobalsFunction, ZilchTypeId(void), parameters, context);
  ...
  currentType->mEntryPoint->mGlobalsInitializerFunction =
      initializeGlobalsFunction;

  return initializeGlobalsFunction;
}

EntryPointHelperFunctionData EntryPointGeneration::GenerateCopyHelper(
    ZilchShaderIRFunction* userMainFn, StringParam name)
{
  ZilchSpirVFrontEnd* translator = mTranslator;
  ZilchSpirVFrontEndContext* context = mContext;
  ...
  ZilchShaderIRFunction* helperFn =
      currentType->CreateFunction(translator->mLibrary);
  ...
  ZilchShaderIROp* selfParam =
      translator->BuildIROp(&helperFn->mParameterBlock,
...
EntryPointHelperFunctionData EntryPointGeneration::GenerateGeometryCopyHelper(
    ZilchShaderIRFunction* userMainFn,
    StringParam name,
    ...
    ZilchShaderIROp*& inputStreamVar)
{
  ...
  translator->GenerateFunctionType(
      nullptr, helperFn, ZilchTypeId(void), signature, context);
  // Add a debug name
  helperFn->mDebugResultName = BuildString(name, "_", userMainFn->mName);
  ...
  inputStreamVar = translator->BuildIROp(parameterBlock,
                                         OpType::OpFunctionParameter,
                                         inputStreamType->mPointerType,
                                         context);
    ...
    ZilchShaderIROp* copyInputsStreamVar)
{
  ...
  CollectInputInterfaceVariables(function,
                                 vertexInputInterfaceInfo,
                                 stageInfo.mInputVertexType,
                                 ShaderStage::Geometry);
  CollectOutputInterfaceVariables(function,
                                  vertexOutputInterfaceInfo,
                                  stageInfo.mOutputVertexType,
                                  ShaderStage::Geometry);
  ...
  CollectInputInterfaceVariables(function,
                                 compositeInterfaceInfo,
                                 stageInfo.mShaderType,
                                 ShaderStage::Geometry);
  CollectUniformInterfaceVariables(function,
                                   compositeInterfaceInfo,
                                   stageInfo.mShaderType,
                                   ShaderStage::Geometry);
  // Also decorate uniforms
  ...
  DeclareStageBlocks(
      compositeInterfaceInfo, entryPointInfo, copyInputsData, copyInputsData);
  ...
  DeclareGeometryVertexInputs(stageInfo,
                              ...
                              inputVertexInterfaceTypes);
  DeclareGeometryVertexOutputs(stageInfo,
                               entryPointInfo,
                               vertexOutputInterfaceInfo,
                               inputStreamInterfaceTypes);

  // Write the copy inputs terminator
  copyInputsBlock->mTerminatorOp = translator->BuildIROp(
      copyInputsBlock, OpType::OpReturn, nullptr, context);
...
void EntryPointGeneration::CollectGeometryStreamTypes(
    ZilchShaderIRFunction* function, GeometryStageInfo& stageInfo)
{
  ...
  for (size_t i = 0; i < function->mBlocks.Size(); ++i)
  {
    ...
    for (size_t j = 0; j < block->mLocalVariables.Size(); ++j)
    {
      ...
      Zilch::GeometryStreamUserData* streamUserData =
          streamPointerType->mZilchType->Has<Zilch::GeometryStreamUserData>();
...
void EntryPointGeneration::DeclareGeometryVertexInputs(
    GeometryStageInfo& stageInfo,
    EntryPointInfo* entryPointInfo,
    ...
    Array<ShaderInterfaceType*>& inputVertexInterfaceTypes)
{
  ...
  // Declare the input vertex stream type and sub-type (no built-ins)
  {
    ...
    // Add decorations
    interfaceGroup.mTypeDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationBlock));
    interfaceGroup.mInstanceDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationLocation, 0));
    ...
    geometryInfo.mArrayTypeName =
        BuildString("VertexInStreamType", baseShaderName);
    geometryInfo.mInstanceName = "In";

    ShaderInterfaceStructArray* arrayInterfaceType =
        DeclareGeometryVertexInput(interfaceGroup,
                                   entryPointInfo,
                                   geometryInfo,
                                   inputStreamInterfaceTypes,
                                   inputVertexInterfaceTypes);
  ...
  for (; !range.Empty(); range.PopFront())
  {
    ...
    GeometryInOutTypeInfo geometryInfo;
    geometryInfo.mArraySize = stageInfo.mInputStreamType->mParameters[1];
    geometryInfo.mItemTypeName =
        BuildString("BuiltInVertexInType", baseShaderName);
    geometryInfo.mArrayTypeName =
        BuildString("BuiltInVertexInStreamType", baseShaderName);
...
ShaderInterfaceStructArray* EntryPointGeneration::DeclareGeometryVertexInput(
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    ...
    Array<ShaderInterfaceType*>& inputVertexInterfaceTypes)
{
  ...
  inputVertexInterface->DeclareInterfaceType(
      this, interfaceGroup, entryPointInfo);
  ...
  inputVertexInterface->DecorateInterfaceType(
      this, interfaceGroup, entryPointInfo);

  // Create the input stream (array) type.
  ZilchShaderIRType* arrayType =
      translator->MakeTypeAndPointer(translator->mLibrary,
  ...
  ShaderInterfaceStructArray* arrayInterfaceType =
      new ShaderInterfaceStructArray();
...
void EntryPointGeneration::DeclareGeometryVertexOutputs(
    GeometryStageInfo& stageInfo,
    EntryPointInfo* entryPointInfo,
    ...
    Array<ShaderInterfaceType*>& inputStreamInterfaceTypes)
{
  ...
  // Declare per-vertex outputs
  {
    ...
    WriteGeometryInterfaceOutput(
        interfaceGroup, entryPointInfo, outputVertexInterfaceTypes);
  }
  ...
  // stage output
  WriteGeometryAppendFunctions(stageInfo,
                               entryPointInfo,
                               outputVertexInterfaceTypes,
                               inputStreamInterfaceTypes);

  DeleteObjectsIn(outputVertexInterfaceTypes);
}

void EntryPointGeneration::WriteGeometryInterfaceOutput(
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
...
void EntryPointGeneration::WriteGeometryAppendFunctions(
    GeometryStageInfo& stageInfo,
    EntryPointInfo* entryPointInfo,
    Array<ShaderInterfaceType*>& outputVertexInterfaceTypes,
    Array<ShaderInterfaceType*>& inputStreamInterfaceTypes)
{
  ...
  for (size_t i = 0; i < stageInfo.mOutputStreamTypes.Size(); ++i)
  {
    ...
    GenerateProvokingVertexAppend(stageInfo,
                                  entryPointInfo,
                                  appendFunctions[i],
                                  stageInfo.mOutputStreamTypes[i]);
  }
  ...
  for (size_t i = 0; i < appendFunctions.Size(); ++i)
  {
    ...
    for (size_t j = 0; j < outputVertexInterfaceTypes.Size(); ++j)
      CopyGeometryOutputInterface(stageInfo,
                                  entryPointInfo,
                                  appendFnData,
                                  *outputVertexInterfaceTypes[j],
                                  inputStreamInterfaceTypes);

    // Invoke the append callback if it exists (allows custom api transform
    // logic)
    CallbackSettings& callbackSettings =
        mTranslator->mSettings->mCallbackSettings;
    if (callbackSettings.mAppendCallback != nullptr)
    {
      ...
      callbackSettings.mAppendCallback(
          callbackData, callbackSettings.mAppendCallbackUserData);
    }

    // Add the instruction to actually emit the vertex
    translator->BuildIROp(
        appendFnData.mBlock, OpType::OpEmitVertex, nullptr, context);
  }
...
void EntryPointGeneration::GenerateProvokingVertexAppend(
    GeometryStageInfo& stageInfo,
    EntryPointInfo* entryPointInfo,
    ...
    ZilchShaderIRType* outputStreamType)
{
  ...
  ZilchShaderIRType* outputVertexType =
      outputStreamType->mParameters[0]->As<ZilchShaderIRType>();
  ...
  Zilch::Function* zilchAppendFn = GetMemberOverloadedFunction(
      zilchStreamType, "Append", outputVertexType->mName, "Integer");
  ZilchShaderIRFunction* originalAppendFn =
      translator->mLibrary->FindFunction(zilchAppendFn);
  ...
  entryPointInfo->mLateBoundFunctions.InsertOrError(originalAppendFn,
                                                    lateBoundAppendFn);
  ...
  // index 1 here)
  ZilchShaderIROp* outDataValueInstance =
      lateBoundAppendFn->mParameterBlock.mLines[1]->As<ZilchShaderIROp>();

  ZilchShaderIROp* defaultVertexIndex =
      lateBoundAppendFn->mParameterBlock.mLines[2]->As<ZilchShaderIROp>();
  ...
  ZilchShaderIROp* outVertexInstance = translator->BuildOpVariable(
      firstBlock, outVertexPtrType, spv::StorageClassFunction, context);
  outVertexInstance->mDebugResultName = "outDataLocal";
  translator->BuildIROp(firstBlock,
                        OpType::OpStore,
                        nullptr,
                        outVertexInstance,
                        outDataValueInstance,
                        context);
...
EntryPointGeneration::CloneAppendFn(ZilchShaderIRFunction* originalAppendFn)
{
  ZilchSpirVFrontEnd* translator = mTranslator;
  ZilchSpirVFrontEndContext* context = mContext;
  ...
  {
    ZilchShaderIROp* originalParameterOp =
        originalParameterBlock->mLines[i]->As<ZilchShaderIROp>();
    ZilchShaderIROp* newParameterOp =
        translator->BuildIROp(newParameterBlock,
                              OpType::OpFunctionParameter,
                              originalParameterOp->mResultType,
                              context);
    ...
    Array<ShaderInterfaceType*>& inputStreamInterfaceTypes)
{
  ...
  ShaderIRTypeMeta* typeMeta =
      appendFnData.mOutputDataInstance->mResultType->mDereferenceType->mMeta;
  for (size_t i = 0; i < typeMeta->mFields.Size(); ++i)
  {
    ...
    for (size_t j = 0; j < fieldMeta->mAttributes.Size(); ++j)
    {
      ...
      if (attribute->mAttributeName == nameSettings.mStageOutputAttribute ||
          attribute->mAttributeName ==
              nameSettings.mHardwareBuiltInOutputAttribute)
      {
  ...
  for (size_t i = 0; i < count; ++i)
  {
    ...
    for (size_t j = 0; j < fieldMeta->mAttributes.Size(); ++j)
    {
      ...
      if (!fieldName.Empty())
      {
        ZilchShaderIROp* dest = interfaceType.GetFieldPointerByIndex(
            i, this, block, spv::StorageClassOutput);
        ZilchShaderIROp* source =
            GetNamedMemberInstanceFrom(block,
                                       appendFnData.mOutputDataInstance,
                                       fieldName,
                                       spv::StorageClassFunction);
      ...
      else
      {
        ...
        for (size_t j = 0; j < inputStreamInterfaceTypes.Size(); ++j)
        {
          ...
          ShaderInterfaceStructArray* inputStreamType =
              inputStreamInterfaceTypes[j]->As<ShaderInterfaceStructArray>();
          if (inputStreamType->ContainsField(fieldKey))
          {
            ...
            ZilchShaderIROp* inputInstance = inputStreamType->GetPointerByIndex(
                appendFnData.mDefaultVertexId,
                fieldKey,
                this,
                block,
                spv::StorageClassInput);
            CopyField(block,
                      inputInstance->mResultType->mDereferenceType,
                      inputInstance,
                      dest);
          }
        }
...
void EntryPointGeneration::CollectInterfaceVariables(
    ZilchShaderIRFunction* function,
    ShaderInterfaceInfo& interfaceInfo,
    ShaderStage::Enum shaderStage)
{
  ...
  CollectInputInterfaceVariables(
      function, interfaceInfo, currentType, shaderStage);
  CollectOutputInterfaceVariables(
      function, interfaceInfo, currentType, shaderStage);
  CollectUniformInterfaceVariables(
      function, interfaceInfo, currentType, shaderStage);
}

void EntryPointGeneration::CollectInputInterfaceVariables(
    ZilchShaderIRFunction* function,
    ShaderInterfaceInfo& interfaceInfo,
    ...
    ShaderStage::Enum shaderStage)
{
  ...
  for (size_t i = 0; i < typeMeta->mFields.Size(); ++i)
  {
    ...
    ShaderIRAttribute* stageInputAttribute =
        fieldMeta->mAttributes.FindFirstAttribute(
            nameSettings.mStageInputAttribute);
    if (stageInputAttribute != nullptr)
    {
      ...
      InterfaceInfoGroup::FieldInfo* fieldInfo =
          interfaceInfo.mInputs.FindOrCreateFieldInfo(fieldKey);
      ...
      fieldInfo->mFieldMeta->mZilchName =
          fieldMeta->GetFieldAttributeName(stageInputAttribute);
    ...
    ShaderIRAttribute* hardwareBuiltInAttribute =
        fieldMeta->mAttributes.FindFirstAttribute(
            nameSettings.mHardwareBuiltInInputAttribute);
    if (hardwareBuiltInAttribute != nullptr)
    {
      ...
      InterfaceInfoGroup* builtInGroup =
          ProcessBuiltIn(function,
                         interfaceInfo,
                         shaderStage,
                         fieldMeta,
      ...
      if (builtInGroup != nullptr)
      {
        ...
        ShaderFieldKey fieldKey =
            fieldMeta->MakeFieldKey(hardwareBuiltInAttribute);
        InterfaceInfoGroup::FieldInfo* fieldInfo =
            builtInGroup->FindFieldInfo(fieldKey);
        fieldInfo->mLinkedFields.PushBack(fieldMeta);
      }
    }
  }
...
void EntryPointGeneration::CollectOutputInterfaceVariables(
    ZilchShaderIRFunction* function,
    ShaderInterfaceInfo& interfaceInfo,
    ZilchShaderIRType* type,
    ShaderStage::Enum shaderStage)
{
  ...
  for (size_t i = 0; i < typeMeta->mFields.Size(); ++i)
  {
    ...
    ShaderIRAttribute* stageOutputAttribute =
        fieldMeta->mAttributes.FindFirstAttribute(
            nameSettings.mStageOutputAttribute);
    if (stageOutputAttribute != nullptr)
    {
      // Create the field info
      ShaderFieldKey fieldKey = fieldMeta->MakeFieldKey(stageOutputAttribute);
      InterfaceInfoGroup::FieldInfo* fieldInfo =
          interfaceInfo.mOutputs.FindOrCreateFieldInfo(fieldKey);
      // Create a new field meta for this field based upon the field we're
      // matching to. In particular, make sure this field's name is set based
      // upon the attribute name.
      fieldInfo->mFieldMeta = fieldMeta->Clone(mTranslator->mLibrary);
      fieldInfo->mFieldMeta->mZilchName =
          fieldMeta->GetFieldAttributeName(stageOutputAttribute);
      fieldInfo->mFieldMeta->mOwner = nullptr;
...
void EntryPointGeneration::CollectUniformInterfaceVariables(
    ZilchShaderIRFunction* function,
    ShaderInterfaceInfo& interfaceInfo,
    ZilchShaderIRType* type,
    ShaderStage::Enum shaderStage)
  ...
  for (size_t i = 0; i < typeMeta->mFields.Size(); ++i)
  {
    ...
    for (size_t j = 0; j < fieldMeta->mAttributes.Size(); ++j)
    {
          ...
          attribute->mAttributeName == nameSettings.mPropertyInputAttribute)
      {
        ...
        if (fieldMeta->mZilchType->HasAttribute(
                nameSettings.mNonCopyableAttributeName))
          continue;

        InterfaceInfoGroup* interfaceGroup = ProcessUniformBlock(
            function, interfaceInfo, fieldMeta, attribute, mapping);

        ShaderFieldKey fieldKey = fieldMeta->MakeFieldKey(attribute);
        // Store the name of the resolved field on the resultant field info
        InterfaceInfoGroup::FieldInfo* fieldInfo =
            interfaceGroup->FindOrCreateFieldInfo(fieldKey);
        fieldInfo->mLinkedFields.PushBack(fieldMeta);
        break;
      }
    }
...
void EntryPointGeneration::ProcessUniformBlockSettings(
    ...
    HashMap<ShaderFieldKey, UniformBufferDescription*>& mapping)
{
  ...
  for (size_t i = 0; i < settings->mUniformBufferDescriptions.Size(); ++i)
  {
    UniformBufferDescription& description =
        settings->mUniformBufferDescriptions[i];
    ...
    for (size_t fieldIndex = 0; fieldIndex < description.mFields.Size();
         ++fieldIndex)
    {
...
InterfaceInfoGroup*
EntryPointGeneration::ProcessBuiltIn(ZilchShaderIRFunction* function,
                                     ShaderInterfaceInfo& interfaceInfo,
                                     ShaderStage::Enum shaderStage,
                                     ...
                                     StringParam name)
{
  ...
  BuiltInStageDescription& builtInDescriptions =
      settings->mBuiltIns[fragmentType];
  if (storageClass == spv::StorageClassOutput)
  ...
  {
    String fullMsg =
        String::Format("BuiltIn '%s' is not valid in shader stage %s",
                       fieldKey.mKey.c_str(),
                       FragmentType::Names[fragmentType]);
    if (fieldMeta->mZilchProperty != nullptr)
      translator->SendTranslationError(
          fieldMeta->mZilchProperty->Location, "Invalid BuiltIn", fullMsg);
    return nullptr;
  }
  ...
  if (block->mInterfaceBlock)
  {
    ...
    {
      group.mName = name;
      group.mTypeDecorations.PushBack(
          InterfaceInfoGroup::DecorationParam(spv::DecorationBlock));
    ...
    {
      BuiltInBlockDescription::BuiltInFieldMeta& builtInMeta =
          block->mFields[i];
      InterfaceInfoGroup::FieldInfo& fieldInfo = group.mFields.PushBack();
      fieldInfo.mFieldMeta = builtInMeta.mMeta;
      fieldInfo.mDecorations.PushBack(InterfaceInfoGroup::DecorationParam(
          spv::DecorationBuiltIn, builtInMeta.mId));
    }
  }
  ...
  else
  {
    ...
    BuiltInBlockDescription::BuiltInFieldMeta* foundBuiltInMeta =
        block->FindField(fieldKey);
    if (foundBuiltInMeta != nullptr)
    {
      ...
      if (fieldInfo == nullptr)
      {
        ...
        fieldInfo->mDecorations.PushBack(InterfaceInfoGroup::DecorationParam(
            spv::DecorationBuiltIn, foundBuiltInMeta->mId));
      }
    }
  }
  return &group;
    ...
    HashMap<ShaderFieldKey, UniformBufferDescription*>& mapping)
{
  ...
  if (!exists)
  {
    ...
    uniformGroup.mInstanceDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationDescriptorSet,
                                            uniformBlock->mDescriptorSetId));
    uniformGroup.mInstanceDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationBinding, bindingId));
  ...
  else
  {
    ...
    fieldInfo.mFieldMeta = fieldMeta->Clone(mTranslator->mLibrary);
    fieldInfo.mFieldMeta->mZilchName =
        fieldMeta->GetFieldAttributeName(attribute);
    uniformGroup.mFields.PushBack(fieldInfo);
  }
  return &uniformGroup;
}

void EntryPointGeneration::DeclareStageBlocks(
    ShaderInterfaceInfo& interfaceInfo,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyInputsData,
    EntryPointHelperFunctionData& copyOutputsData)
{
  ...
  DeclareBlock(
      interfaceInfo.mOutputs, entryPointInfo, copyOutputsData, mOutputs);

  // Write out BuiltIns. These can be inputs and outputs as well as interface
  // blocks or globals
  DeclareGroupBlocks(interfaceInfo.mBuiltInGroups,
                     entryPointInfo,
                     copyInputsData,
                     copyOutputsData,
                     mBuiltIns);
  ...
  DeclareGroupBlocks(interfaceInfo.mUniformGroups,
                     entryPointInfo,
                     copyInputsData,
                     copyOutputsData,
                     mUniforms);
...
void EntryPointGeneration::DeclareGroupBlocks(
    ShaderInterfaceInfo::InterfaceGroupMap& interfaceGroups,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyInputsData,
    EntryPointHelperFunctionData& copyOutputsData,
...
void EntryPointGeneration::DeclareBlock(
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyHelperData,
    Array<ShaderInterfaceType*>& outArray)
{
  ...
  if (interfaceGroup.mIsStruct)
    DeclareBlockStruct(
        interfaceGroup, entryPointInfo, copyHelperData, outArray);
  else
    DeclareBlockNoStruct(
        interfaceGroup, entryPointInfo, copyHelperData, outArray);
}

void EntryPointGeneration::DeclareBlockStruct(
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyHelperData,
    Array<ShaderInterfaceType*>& outArray)
{
  ...
  interfaceStruct->CopyInterfaceType(
      this, interfaceGroup, entryPointInfo, copyHelperData);
  outArray.PushBack(interfaceStruct);
}

void EntryPointGeneration::DeclareBlockNoStruct(
    InterfaceInfoGroup& interfaceGroup,
    EntryPointInfo* entryPointInfo,
    EntryPointHelperFunctionData& copyHelperData,
    Array<ShaderInterfaceType*>& outArray)
{
  ...
  interfaceGlobals->CopyInterfaceType(
      this, interfaceGroup, entryPointInfo, copyHelperData);
                                     ...
                                     ZilchShaderIROp* dest)
{
  ...
  {
    ZilchShaderIRType* sourceElementType =
        sourceValueType->mParameters[0]->As<ZilchShaderIRType>();
    ZilchShaderIRType* destElementType =
        destValueType->mParameters[0]->As<ZilchShaderIRType>();
    ...
    // class on the type.
    ZilchShaderIROp* sourceValue = translator->BuildIROp(
        targetFnBlock, OpType::OpLoad, sourceValueType, source, context);
    for (size_t i = 0; i < sourceValueType->mComponents; ++i)
    {
      // Extract from the source (by value)
      ZilchShaderIRConstantLiteral* indexLiteral =
          translator->GetOrCreateConstantIntegerLiteral(i);
      ZilchShaderIROp* sourceElement =
          translator->BuildIROp(targetFnBlock,
                                OpType::OpCompositeExtract,
                                sourceElementType,
                                sourceValue,
                                indexLiteral,
                                context);
      // Get the destination element (by pointer)
      ZilchShaderIROp* indexConstant =
          translator->GetIntegerConstant(i, context);
      ZilchShaderIROp* destElement =
          translator->BuildIROp(targetFnBlock,
                                OpType::OpAccessChain,
                                destElementType->mPointerType,
                                dest,
                                indexConstant,
                                context);
      // Store to the destination
      translator->BuildStoreOp(
          targetFnBlock, destElement, sourceElement, context);
    }
  }
  ...
  {
    ShaderIRTypeBaseType::Enum sourceBaseType =
        sourceValueType->GetBasePrimitiveType();
    ShaderIRTypeBaseType::Enum destBaseType =
        destValueType->GetBasePrimitiveType();

    // Convert the source integer into the destination boolean
    if (sourceBaseType == ShaderIRTypeBaseType::Int &&
        destBaseType == ShaderIRTypeBaseType::Bool)
    {
      ZilchShaderIROp* sourceValue = translator->GetOrGenerateValueTypeFromIR(
          targetFnBlock, source, context);
      ZilchShaderIROp* result = translator->GenerateIntegerToBoolCast(
          targetFnBlock, sourceValue, destValueType, context);
      translator->BuildStoreOp(targetFnBlock, dest, result, context);
      return;
    }
    // Convert the source boolean into the destination integer
    if (sourceBaseType == ShaderIRTypeBaseType::Bool &&
        destBaseType == ShaderIRTypeBaseType::Int)
    {
      // Cast the bool to an integer (selection between 0 and 1)
      ZilchShaderIROp* sourceValue = translator->GetOrGenerateValueTypeFromIR(
          targetFnBlock, source, context);
      ZilchShaderIROp* result = translator->GenerateBoolToIntegerCast(
          targetFnBlock, sourceValue, destValueType, context);
      translator->BuildStoreOp(targetFnBlock, dest, result, context);
      return;
    }
    ...
    spv::StorageClass sourceStorageClass)
{
  ...
  {
    ShaderInterfaceStructArray* arrayInterface =
        sourceInterface->As<ShaderInterfaceStructArray>();
    // Get the size of the array
    ZilchShaderIROp* sizeConstant =
        sourceValueType->mParameters[1]->As<ZilchShaderIROp>();
    ZilchShaderIRConstantLiteral* sizeLiteral =
        sizeConstant->mArguments[0]->As<ZilchShaderIRConstantLiteral>();
    int sourceSize = sizeLiteral->mValue.Get<int>();

    for (int index = 0; index < sourceSize; ++index)
    {
      ZilchShaderIROp* indexConstant =
          translator->GetIntegerConstant(index, context);
      ZilchShaderIRType* sourceArrayItemType =
          sourceValueType->mParameters[0]->As<ZilchShaderIRType>();
      ZilchShaderIRType* destArrayItemType =
          destValueType->mParameters[0]->As<ZilchShaderIRType>();
      ZilchShaderIROp* sourceItem =
          translator->BuildIROp(block,
                                OpType::OpAccessChain,
                                sourceArrayItemType->mPointerType,
                                source,
                                indexConstant,
                                context);
      ZilchShaderIROp* destItem =
          translator->BuildIROp(block,
                                OpType::OpAccessChain,
                                destArrayItemType->mPointerType,
                                dest,
                                indexConstant,
                                context);
      CopyFromInterfaceType(block,
                            destItem,
                            sourceItem,
                            arrayInterface->mStructType,
                            destStorageClass,
                            sourceStorageClass);
    }
  }
  else if (sourceBaseType == ShaderIRTypeBaseType::Struct)
  {
    ...
    // (Hacky)
    ShaderInterfaceStruct* structInterface =
        sourceInterface->As<ShaderInterfaceStruct>();
    ...
    for (size_t i = 0; i < fieldCount; ++i)
    {
      ...
      // one).
      ShaderInterfaceField* interfaceField =
          sourceInterface->GetFieldAtIndex(i);
      for (size_t j = 0; j < interfaceField->mLinkedFields.Size(); ++j)
      {
        ...
        ZilchShaderIROp* destMemberPtr = GetNamedMemberInstanceFrom(
            block, dest, linkedFieldMeta->mZilchName, destStorageClass);
        ...
        {
          Error("Destination field name '%s' is invalid",
                linkedFieldMeta->mZilchName.c_str());
          continue;
        }

        // Get the source field instance to copy from given the source instance
        ZilchShaderIROp* sourceMemberPtr =
            structInterface->GetFieldPointerByIndex(
                source, i, this, block, sourceStorageClass);
        CopyField(block,
                  sourceMemberPtr->mResultType->mDereferenceType,
                  sourceMemberPtr,
                  destMemberPtr);
      }
    }
...
ZilchShaderIROp* EntryPointGeneration::GetMemberInstanceFrom(
    BasicBlock* block,
    ZilchShaderIROp* source,
    int sourceOffset,
    spv::StorageClass sourceStorageClass)
{
  ...
  // type.
  ZilchShaderIRType* sourceMemberValueType =
      sourceValueType->GetSubType(sourceOffset);
  ZilchShaderIRType* sourceMemberPtrType = sourceMemberValueType->mPointerType;
  if (sourceStorageClass != spv::StorageClassFunction)
    sourceMemberPtrType = translator->FindOrCreatePointerInterfaceType(
        translator->mLibrary, sourceMemberValueType, sourceStorageClass);

  // Get the member pointer to the source struct's member
  ZilchShaderIROp* sourceOffsetConstant =
      translator->GetIntegerConstant(sourceOffset, context);
  ZilchShaderIROp* sourceMemberPtr =
      translator->BuildIROp(block,
                            OpType::OpAccessChain,
                            sourceMemberPtrType,
                            source,
                            sourceOffsetConstant,
                            context);
  return sourceMemberPtr;
}

ZilchShaderIROp* EntryPointGeneration::GetNamedMemberInstanceFrom(
    BasicBlock* block,
    ZilchShaderIROp* source,
    StringParam memberName,
    spv::StorageClass sourceStorageClass)
{
  ErrorIf(!source->mResultType->IsPointerType(),
          "Source must be a pointer type");

  // Find the member index by name
  ZilchShaderIRType* sourceValueType = source->mResultType->mDereferenceType;
  int sourceOffset =
      sourceValueType->mMemberNamesToIndex.FindValue(memberName, -1);
    ...
    spv::StorageClass sourceStorageClass)
{
  ...
  ZilchShaderIRType* sourceValueType = source->mResultType->mDereferenceType;
  int sourceOffset =
      sourceValueType->mMemberKeysToIndex.FindValue(fieldKey, -1);
  // If this member didn't exist then we can't get the member so return null
  if (sourceOffset == -1)
    return nullptr;
...
void EntryPointGeneration::DecorateUniformGroups(
    ShaderInterfaceInfo& interfaceInfo)
{
...
void EntryPointGeneration::DecorateUniformGroup(InterfaceInfoGroup& infoGroup)
{
  ...
  infoGroup.mTypeDecorations.PushBack(
      InterfaceInfoGroup::DecorationParam(spv::DecorationBlock));
...
void EntryPointGeneration::AddOffsetDecorations(InterfaceInfoGroup& infoGroup)
{
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    ZilchShaderIRType* memberType =
        translator->FindType(fieldMeta->mZilchType, nullptr);
    ...
    // alignment
    currentByteOffset =
        GetSizeAfterAlignment(currentByteOffset, requiredAlignment);

    // Add the offset decoration
    fieldInfo.mDecorations.PushBack(InterfaceInfoGroup::DecorationParam(
        spv::DecorationOffset, currentByteOffset));

    // Store the offset and size of this field
    ShaderResourceReflectionData& fieldReflectionData =
        fieldInfo.mReflectionData;
...
void EntryPointGeneration::AddMemberTypeDecorations(
    ZilchShaderIRType* memberType,
    InterfaceInfoGroup::FieldInfo& fieldInfo,
    ShaderResourceReflectionData& memberReflection)
{
  ...
  if (memberType->mBaseType == ShaderIRTypeBaseType::Matrix)
  {
    ...
    int matrixStride = 16;
    fieldInfo.mDecorations.PushBack(InterfaceInfoGroup::DecorationParam(
        spv::DecorationMatrixStride, matrixStride));
    fieldInfo.mDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationColMajor));
  ...
  else if (memberType->mBaseType == ShaderIRTypeBaseType::FixedArray)
  {
    ZilchShaderIRType* elementType =
        memberType->mParameters[0]->As<ZilchShaderIRType>();
    ...
    fieldInfo.mTypeDecorations.PushBack(InterfaceInfoGroup::DecorationParam(
        spv::DecorationArrayStride, stride));
...
void EntryPointGeneration::AddVertexLocationDecorations(
    InterfaceInfoGroup& infoGroup)
{
  VertexDefinitionDescription& vertexDefinitions =
      mTranslator->mSettings->mVertexDefinitions;
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    location = vertexDefinitionLocations.FindValue(
        fieldInfo.mFieldMeta->MakeFieldKey(), -1);
    if (location == -1)
      location = lastVertexDefinitionIndex++;

    fieldInfo.mDecorations.PushBack(
        InterfaceInfoGroup::DecorationParam(spv::DecorationLocation, location));
...
void EntryPointGeneration::AddFlatDecorations(InterfaceInfoGroup& infoGroup)
{
  ...
  for (size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    ShaderIRTypeBaseType::Enum basePrimitiveType =
        shaderType->GetBasePrimitiveType();
    if (basePrimitiveType == ShaderIRTypeBaseType::Int ||
        basePrimitiveType == ShaderIRTypeBaseType::Bool)
      fieldInfo.mDecorations.PushBack(
          InterfaceInfoGroup::DecorationParam(spv::DecorationFlat));
  }
}

void EntryPointGeneration::WriteTypeDecorations(
    ...
    IZilchShaderIR* toDecorate)
{
  ...
  for (size_t i = 0; i < decorations.Size(); ++i)
  {
    ...
    ZilchShaderIRConstantLiteral* builtInDecoration =
        translator->GetOrCreateConstantIntegerLiteral(
            decoration.mDecorationType);
    ZilchShaderIROp* decorationOp = translator->BuildIROp(decorationBlock,
                                                          OpType::OpDecorate,
                                                          nullptr,
                                                          toDecorate,
                                                          builtInDecoration,
...
void EntryPointGeneration::WriteMemberDecorations(
    Array<InterfaceInfoGroup::DecorationParam>& decorations,
    BasicBlock* decorationBlock,
    IZilchShaderIR* toDecorate,
    ZilchShaderIRConstantLiteral* memberIndexLiteral)
{
  ...
  for (size_t i = 0; i < decorations.Size(); ++i)
  {
    ...
    ZilchShaderIROp* decorationOp =
        translator->BuildIROp(decorationBlock,
                              OpType::OpMemberDecorate,
                              nullptr,
                              toDecorate,
                              memberIndexLiteral,
                              context);
...
void EntryPointGeneration::FindAndDecorateGlobals(
    ZilchShaderIRType* currentType, EntryPointInfo* entryPointInfo)
{
...
void EntryPointGeneration::DecorateImagesAndSamplers(
    TypeDependencyCollector& collector, EntryPointInfo* entryPointInfo)
{
  ZilchSpirVFrontEnd* translator = mTranslator;
  ZilchSpirVFrontEndContext* context = mContext;
  ...
  ShaderStageInterfaceReflection& stageReflectionData =
      entryPointInfo->mStageReflectionData;
  ...
  for (; !range.Empty(); range.PopFront())
  {
    ...
    ShaderStageInterfaceReflection::SampledImageRemappings* remappings =
        nullptr;
    ...
    BasicBlock* decorationBlock = &entryPointInfo->mDecorations;
    translator->BuildDecorationOp(decorationBlock,
                                  globalVarInstance,
    ...
    resourceInfo->mReflectionData.mTypeName =
        opValueType->mZilchType->ToString();
  }
}

void EntryPointGeneration::DecorateRuntimeArrays(
    TypeDependencyCollector& collector, EntryPointInfo* entryPointInfo)
{
  ...
  for (; !range.Empty(); range.PopFront())
  {
    ...
    // runtime array)
    if (zilchType == nullptr ||
        zilchType->TemplateBaseName != SpirVNameSettings::mRuntimeArrayTypeName)
      continue;
    ...
    ZilchShaderIRType* spirvRuntimeArrayType =
        zilchRuntimeArrayType->mParameters[0]->As<ZilchShaderIRType>();
    ZilchShaderIRType* elementType =
        spirvRuntimeArrayType->mParameters[0]->As<ZilchShaderIRType>();
    ...
    translator->BuildDecorationOp(
        decorationBlock, opValueType, spv::DecorationBlock, context);
    translator->BuildMemberDecorationOp(
        decorationBlock, opValueType, 0, spv::DecorationOffset, 0, context);

    // Create a new reflection object for the runtime array.
    ShaderStageInterfaceReflection& stageReflectionData =
        entryPointInfo->mStageReflectionData;
    ShaderStageResource& stageResource =
        stageReflectionData.mStructedStorageBuffers.PushBack();
    ShaderResourceReflectionData& reflectionData =
        stageResource.mReflectionData;
    ...
    AddRuntimeArrayDecorations(decorationBlock,
                               zilchRuntimeArrayType,
                               spirvRuntimeArrayType,
                               elementType,
                               stageResource);
  }
}
    ...
    ShaderStageResource& stageResource)
{
  ...
  {
    translator->SendTranslationError(
        nullptr, "Runtime array cannot directly contain a FixedArray.");
  ...
  case ShaderIRTypeBaseType::Matrix:
  {
    ...
    // @JoshD: Maybe make a packing option for this later?
    int matrixStride = 16;
    translator->BuildMemberDecorationOp(decorationBlock,
                                        zilchRuntimeArrayType,
                                        0,
...
void EntryPointGeneration::RecursivelyDecorateStructType(
    BasicBlock* decorationBlock,
    ...
    ShaderStageResource& stageResource)
{
  ...
  for (size_t i = 0; i < structType->mParameters.Size(); ++i)
  {
    // Find the type of the member
    ZilchShaderIRType* memberType =
        structType->mParameters[i]->As<ZilchShaderIRType>();
    ...
    ShaderResourceReflectionData& fieldReflectionData =
        stageResource.mMembers.PushBack();
    ...
    if (memberType->mBaseType == ShaderIRTypeBaseType::Matrix)
    {
      ...
      translator->BuildMemberDecorationOp(decorationBlock,
                                          ...
                                          context);
      translator->BuildMemberDecorationOp(
          decorationBlock, structType, i, spv::DecorationColMajor, context);
...
int EntryPointGeneration::FindBindingId(HashSet<int>& usedIds1,
                                        HashSet<int>& usedIds2)
{
...
void EntryPointGeneration::CopyReflectionDataToEntryPoint(
    EntryPointInfo* entryPointInfo, ShaderInterfaceInfo& interfaceInfo)
{
  ShaderStageInterfaceReflection& reflection =
      entryPointInfo->mStageReflectionData;
  // Save the type name
  ...
  CopyReflectionData(
      reflection.mStageInputs, interfaceInfo, interfaceInfo.mInputs);
  CopyReflectionData(
      reflection.mStageOutputs, interfaceInfo, interfaceInfo.mOutputs);
}

void EntryPointGeneration::CopyReflectionData(
    Array<ShaderStageResource>& resourceList,
    ShaderInterfaceInfo& interfaceInfo,
    InterfaceInfoGroup& group)
{
  if (group.mIsStruct)
    ...
    CopyReflectionDataGlobals(
        resourceList, interfaceInfo, interfaceInfo.mInputs);
}

void EntryPointGeneration::CopyReflectionDataStruct(
    Array<ShaderStageResource>& resourceList,
    ShaderInterfaceInfo& interfaceInfo,
    InterfaceInfoGroup& group)
{
  ...
  for (size_t i = 0; i < group.mFields.Size(); ++i)
  {
    ...
    ShaderResourceReflectionData& fieldReflection =
        stageResource.mMembers.PushBack();
...
void EntryPointGeneration::CopyReflectionDataGlobals(
    Array<ShaderStageResource>& resourceList,
    ShaderInterfaceInfo& interfaceInfo,
    InterfaceInfoGroup& group)
{
...
void EntryPointGeneration::CreateShaderInterfaceField(
    ShaderInterfaceField& interfaceField,
    InterfaceInfoGroup& interfaceGroup,
    int index)
{
  ...
  if (actualZilchType != fieldMeta->mZilchType)
    interfaceField.mFieldName = BuildString(
        interfaceField.mFieldName, "_", fieldMeta->mZilchType->ToString());
  ...
  interfaceField.mFieldType = translator->FindOrCreateInterfaceType(
      library, actualZilchType, ShaderIRTypeBaseType::Pointer, storageClass);
  interfaceField.mFieldType->mDereferenceType =
      translator->FindType(actualZilchType, nullptr, true);
  ...
  interfaceField.mOriginalFieldType = translator->FindOrCreateInterfaceType(
      library, originalZilchType, ShaderIRTypeBaseType::Pointer, storageClass);
  interfaceField.mOriginalFieldType->mDereferenceType =
      translator->FindType(originalZilchType, nullptr, true);
}

Zilch::BoundType*
EntryPointGeneration::ConvertInterfaceType(Zilch::BoundType* inputType)
{
...
void EntryPointGeneration::WriteExecutionModeOriginUpperLeft(
    EntryPointInfo* entryPointInfo)
{
  BasicBlock* block = &entryPointInfo->mExecutionModes;
  ZilchShaderIRConstantLiteral* literalOp =
      mTranslator->GetOrCreateConstantIntegerLiteral(
...
ZilchShaderIROp*
EntryPointGeneration::FindField(ShaderFieldKey& fieldKey,
...
ShaderIRFieldMeta*
EntryPointGeneration::FindFieldViaAttribute(ShaderIRTypeMeta* typeMeta,
                                            ...
                                            ShaderFieldKey& fieldKey)
{
  ...
  for (size_t i = 0; i < typeMeta->mFields.Size(); ++i)
  {
    ...
    {
      ShaderIRAttribute* attribute = fieldMeta->mAttributes.GetAtIndex(j);
      if (attribute->mAttributeName == attributeName &&
          fieldMeta->MakeFieldKey(attribute) == fieldKey)
        return fieldMeta;
    }
  }
  return nullptr;
...
void EntryPointGeneration::PerspectiveTransformAppendVertexCallback(
    AppendCallbackData& callbackData, void* userData)
{
  ...
  ZilchShaderIRType* appendVertexDataType =
      outDataInstance->mResultType->mDereferenceType;
  ShaderIRFieldMeta* perspectivePositionFieldMeta =
      self->FindFieldViaAttribute(appendVertexDataType->mMeta,
                                  nameSettings.mStageOutputAttribute,
                                  perspectivePositionKey);
  if (perspectivePositionFieldMeta == nullptr)
    return;

  // Find the perspective position field pointer on the output vertex type
  int* index = appendVertexDataType->mMemberKeysToIndex.FindPointer(
      perspectivePositionFieldMeta->MakeFieldKey());
  ...
  ShaderFieldKey apiPerspectivePositionKey(
      nameSettings.mApiPerspectivePositionName, real4TypeName);
  ZilchShaderIROp* apiPerspectivePosition =
      self->FindField(apiPerspectivePositionKey,
                      *callbackData.mOutputVertexInterfaceTypes,
                      block,
                      spv::StorageClassOutput);

  // Find the api perspective transform matrix from all of the uniform buffers
  // available
  ShaderFieldKey perspectiveTransformKey(
      nameSettings.mPerspectiveToApiPerspectiveName, real4x4TypeName);
  ZilchShaderIROp* transformMatrixOp =
      self->FindField(perspectiveTransformKey,
                      self->mUniforms,
                      block,
                      spv::StorageClassUniform);

  // If we have all three variables then we can write a transform
  if (apiPerspectivePosition != nullptr && transformMatrixOp != nullptr &&
      index != nullptr)
  {
    // Get the instance pointer to perspective position
    ZilchShaderIROp* source = self->GetMemberInstanceFrom(
        block, outDataInstance, *index, spv::StorageClassFunction);

    // Matrix multiplication needs value types not pointers, so make sure to
    // dereference if necessary
    ZilchShaderIROp* valueMatrix =
        self->mTranslator->GetOrGenerateValueTypeFromIR(
            block, transformMatrixOp, self->mContext);
    ZilchShaderIROp* valueVector =
        self->mTranslator->GetOrGenerateValueTypeFromIR(
            block, source, self->mContext);
    // Get the result type of the transform (should always be Real4)
    ZilchShaderIRType* resultType =
        apiPerspectivePosition->mResultType->mDereferenceType;
    // Write Multiply(PerspectiveToApiPerspective, PerspectivePosition)
    ZilchShaderIROp* result =
        self->mTranslator->BuildIROp(block,
                                     OpType::OpMatrixTimesVector,
                                     resultType,
                                     valueMatrix,
                                     valueVector,
                                     self->mContext);
    // Store the result into the final ApiPerspectivePosition output
    self->mTranslator->BuildStoreOp(
        block, apiPerspectivePosition, result, self->mContext);
  }
}
