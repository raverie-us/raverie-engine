...
static_assert(
    sizeof(s8) == sizeof(std::atomic<s8>),
    "For STL atomics to work as implemented these sizes must be the same");
static_assert(
    sizeof(s16) == sizeof(std::atomic<s16>),
    "For STL atomics to work as implemented these sizes must be the same");
static_assert(
    sizeof(s32) == sizeof(std::atomic<s32>),
    "For STL atomics to work as implemented these sizes must be the same");
static_assert(
    sizeof(s64) == sizeof(std::atomic<s64>),
    "For STL atomics to work as implemented these sizes must be the same");

// AtomicStore
void AtomicStore(volatile s8* target, s8 value)
{
  volatile std::atomic<s8>* t =
      reinterpret_cast<volatile std::atomic<s8>*>(target);
  std::atomic_store(t, value);
}
void AtomicStore(volatile s16* target, s16 value)
{
  volatile std::atomic<s16>* t =
      reinterpret_cast<volatile std::atomic<s16>*>(target);
  std::atomic_store(t, value);
}
void AtomicStore(volatile s32* target, s32 value)
{
  volatile std::atomic<s32>* t =
      reinterpret_cast<volatile std::atomic<s32>*>(target);
  std::atomic_store(t, value);
}
void AtomicStore(volatile s64* target, s64 value)
{
  volatile std::atomic<s64>* t =
      reinterpret_cast<volatile std::atomic<s64>*>(target);
  std::atomic_store(t, value);
}
void AtomicStore(void* volatile* target, void* value)
{
  volatile std::atomic<void*>* t =
      reinterpret_cast<volatile std::atomic<void*>*>(target);
  std::atomic_store(t, value);
}
