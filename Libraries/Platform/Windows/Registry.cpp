...
bool GetRegistryValue(void* key,
                      StringParam subKey,
                      StringParam value,
                      String& result)
{
  ...
  HKEY hKey;
  LONG queryResult =
      RegOpenKeyEx(hRoot, Widen(subKey).c_str(), 0, KEY_QUERY_VALUE, &hKey);
  if (queryResult != ERROR_SUCCESS)
    return false;

  queryResult = RegQueryValueEx(
      hKey, Widen(value).c_str(), 0, 0, (LPBYTE)nameBuffer, &length);
  if (queryResult != ERROR_SUCCESS)
    return false;
                      ...
                      String& result)
{
  ...
  ReturnIf(winKey == nullptr,
           false,
           "Unable to find KEY by name (or unhandled case)");
                                            ...
                                            String& result)
{
  String path1 = String::Format(
      "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{%s}_is1",
      programGuid.c_str());
