...
void OpenglRenderer::DoRenderTaskPostProcess(RenderTaskPostProcess* task)
{
  ...
  ShaderKey shaderKey(compositeName,
                      StringPair(String("PostVertex"), String()));
...
void OpenglRenderer::DrawStatic(ViewNode& viewNode, FrameNode& frameNode)
{
  ...
  ShaderKey shaderKey(
      ...
      StringPair(mCoreVertexTypeNames[frameNode.mCoreVertexType],
                 mRenderPassName));
  GlShader* shader = GetShader(shaderKey);
  if (shader == nullptr)
    return;
