...
extern "C" EMSCRIPTEN_KEEPALIVE void EmscriptenWebRequestOnHeadersReceived(
    char* responseHeaders, int responseCode, WebRequest* request)
{
  ...
  if (request->mOnHeadersReceived)
  {
    ...
    forRange(StringRange headerLine, headerLines) headers.PushBack(headerLine);

    request->mOnHeadersReceived(
        headers, (WebResponseCode::Enum)responseCode, request);
  }

  free(responseHeaders);
}

extern "C" EMSCRIPTEN_KEEPALIVE void EmscriptenWebRequestOnDataReceived(
    byte* data, size_t length, size_t totalDownloaded, WebRequest* request)
{
...
EmscriptenWebRequestOnComplete(char* errorMessage, WebRequest* request)
{
...
EM_JS(void,
       ...
       WebRequest* request),
      {
        xhrRun(isPost,
               ...
               UTF8ToString(postData),
               request);
      });

EM_JS(void, EmscriptenWebRequestCancel, (WebRequest * request), {
  xhrCancel(request);
});
...
void WebRequest::Run()
{
  ...
  EmscriptenWebRequestRun(!mPostData.Empty(),
                          mUrl.c_str(),
                          headers.c_str(),
                          postData.c_str(),
                          this);
