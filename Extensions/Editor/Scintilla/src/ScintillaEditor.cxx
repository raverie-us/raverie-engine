...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	for (int margin = 0; margin < vs.margins; margin++) {
        ...
        /* ZERO: Remove patterned fill on folding margin 
...
void Editor::Paint(Surface *surfaceWindow, PRectangle rcArea) {
    ...
    // ZERO: draw our margins last so text within the document doesn't draw over them when you scroll horizontally
...
void Editor::AddCharUTF(char *s, unsigned int len, bool treatAsDBCS) {
	FilterSelections();
	{
		...
		ScVector<SelectionRange *> selPtrs;
		for (size_t r = 0; r < sel.Count(); r++) {
			selPtrs.push_back(&sel.Range(r));
		}
		std::sort(selPtrs.begin(), selPtrs.end(), cmpSelPtrs);

		for (ScVector<SelectionRange *>::reverse_iterator rit = selPtrs.rbegin();
...
void Editor::ChangeCaseOfSelection(int caseMapping) {
	...
	for (size_t r=0; r<sel.Count(); r++) {
		...
		if (rangeBytes > 0) {
			ScString sText(text, rangeBytes);

			ScString sMapped = CaseMapString(sText, caseMapping);
...
ScString Editor::CaseMapString(const ScString &s, int caseMapping) {
	ScString ret(s);
...
void Editor::CheckForChangeOutsidePaint(Scintilla::Range r) {
	if (paintState == painting && !paintingAllText) {
