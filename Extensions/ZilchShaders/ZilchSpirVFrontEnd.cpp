...
ZilchShaderIRType* ZilchSpirVFrontEnd::MakeTypeInternal(ZilchShaderIRLibrary* shaderLibrary, ShaderIRTypeBaseType::Enum baseType, StringParam typeName, Zilch::BoundType* zilchType, spv::StorageClass storageClass)
{
  ...
  // Don't set the debug name here, that's up to the callee if they want a debug name
  return shaderType;
}

ZilchShaderIRType* ZilchSpirVFrontEnd::MakeTypeAndPointer(ZilchShaderIRLibrary* shaderLibrary, ShaderIRTypeBaseType::Enum baseType, StringParam typeName, Zilch::BoundType* zilchType, spv::StorageClass pointerStorageClass)
{
  ...
  ZilchShaderIRType* shaderType = MakeTypeInternal(shaderLibrary, baseType, typeName, zilchType, spv::StorageClassGeneric);
  ZilchShaderIRType* pointerType = MakeTypeInternal(shaderLibrary, ShaderIRTypeBaseType::Pointer, BuildString(typeName, "_ptr"), zilchType, pointerStorageClass);
...
ZilchShaderIRType* ZilchSpirVFrontEnd::MakeStructType(ZilchShaderIRLibrary* shaderLibrary, StringParam typeName, Zilch::BoundType* zilchType, spv::StorageClass pointerStorageClass)
{
  return MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::Struct, typeName, zilchType, pointerStorageClass);
}

ZilchShaderIRType* ZilchSpirVFrontEnd::FindOrCreateInterfaceType(ZilchShaderIRLibrary* shaderLibrary, StringParam baseTypeName, Zilch::BoundType* zilchType, ShaderIRTypeBaseType::Enum baseType, spv::StorageClass storageClass)
{
...
ZilchShaderIRType* ZilchSpirVFrontEnd::FindOrCreateInterfaceType(ZilchShaderIRLibrary* shaderLibrary, Zilch::BoundType* zilchType, ShaderIRTypeBaseType::Enum baseType, spv::StorageClass storageClass)
{
  return FindOrCreateInterfaceType(shaderLibrary, zilchType->Name, zilchType, baseType, storageClass);
}

ZilchShaderIRType* ZilchSpirVFrontEnd::FindOrCreatePointerInterfaceType(ZilchShaderIRLibrary* shaderLibrary, ZilchShaderIRType* valueType, spv::StorageClass storageClass)
{
  ErrorIf(valueType->IsPointerType(), "Type must be a value type");
  ZilchShaderIRType* pointerType = FindOrCreateInterfaceType(shaderLibrary, valueType->mName, valueType->mZilchType, ShaderIRTypeBaseType::Pointer, storageClass);

  pointerType->mDereferenceType = valueType;
  return pointerType;
}

ShaderIRTypeMeta* ZilchSpirVFrontEnd::MakeShaderTypeMeta(ZilchShaderIRType* shaderType, Zilch::NodeList<Zilch::AttributeNode>* nodeAttributeList)
{
  ...
  // Parse attributes if possible (interface types don't have backing zilch types)
...
bool ZilchSpirVFrontEnd::Translate(Zilch::SyntaxTree& syntaxTree, ZilchShaderIRProject* project, ZilchShaderIRLibrary* library)
{
  ...
  // Now that we know about all types we can generate all template types (templates have dependencies on other types)
  PreWalkTemplateTypes(&context);

  // Now we can walk classes again to get variable and function signatures since we have all types
...
void ZilchSpirVFrontEnd::ParseAttributes(Zilch::Array<Zilch::Attribute>& zilchAttributes, Zilch::NodeList<Zilch::AttributeNode>* attributeNodes, ShaderIRTypeMeta* typeMeta)
{
  ...
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
    ...
    {
      fragmentTypeAttributeIndices.PushBack(i);
      ValidateSingleParamAttribute(shaderAttribute, nameSettings.mMaxVerticesParam, Zilch::ConstantType::Integer, false);
    }
...
void ZilchSpirVFrontEnd::ParseAttributes(Zilch::Array<Zilch::Attribute>& zilchAttributes, Zilch::NodeList<Zilch::AttributeNode>* attributeNodes, ShaderIRFunctionMeta* functionMeta)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void ZilchSpirVFrontEnd::ParseAttributes(Zilch::Array<Zilch::Attribute>& zilchAttributes, Zilch::NodeList<Zilch::AttributeNode>* attributeNodes, ShaderIRFieldMeta* fieldMeta)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
    ...
    else if(attributeName == nameSettings.mAppBuiltInInputAttribute)
    {
      ...
      // If this is an explicit attribute then check to see if it matches something in the uniform buffers
      if(!shaderAttribute->mImplicitAttribute)
      {
        ...
        // Make sure to get the actual field name being searched for (in case of overrides)
        ...
        if(!isValid)
        {
          String msg = String::Format("Field '%s : %s' does not match any provided %s with fragment type '%s'.",
            appBuiltInName.c_str(), fieldType.c_str(), shaderAttribute->mAttributeName.c_str(), FragmentType::Names[currentFragmentType]);
...
void ZilchSpirVFrontEnd::ParseZilchAttributes(Zilch::Array<Zilch::Attribute>& zilchAttributes, Zilch::NodeList<Zilch::AttributeNode>* attributeNodes, ShaderIRAttributeList& shaderAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void ZilchSpirVFrontEnd::ParseZilchAttribute(Zilch::Attribute& zilchAttribute, Zilch::AttributeNode* attributeNode, ShaderIRAttributeList& shaderAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void ZilchSpirVFrontEnd::ValidateAllowedAttributes(ShaderIRAttributeList& shaderAttributes, HashMap<String, AttributeInfo>& allowedAttributes, StringParam errorTypeName)
{
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
...
void ZilchSpirVFrontEnd::ValidateNameOverrideAttribute(ShaderIRAttribute* shaderAttribute)
{
  ...
  if(paramCount > 1)
  { 
    String msg = String::Format("Too many parameters to attribute '%s'. Signature must be '%s : String'",
      shaderAttribute->mAttributeName.c_str(), SpirVNameSettings::mNameOverrideParam.c_str());
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
  ...
  if(paramCount == 1)
  {
    ...
    {
      String msg = String::Format("Invalid parameter type to attribute '%s'. Signature must be '%s : String'",
        shaderAttribute->mAttributeName.c_str(), SpirVNameSettings::mNameOverrideParam.c_str());
      SendTranslationError(shaderAttribute->GetLocation(), msg);
      return;
    }
    ...
    {
      String msg = String::Format("Invalid parameter name to attribute '%s'. Signature must be '%s : String'",
        shaderAttribute->mAttributeName.c_str(), SpirVNameSettings::mNameOverrideParam.c_str());
      SendTranslationError(shaderAttribute->GetLocation(), msg);
      return;
    }

    // Validate that the name parameter is a valid identifier (needed for the compositor and a few other places)
    String paramValue = param.GetStringValue();
    if(Zilch::LibraryBuilder::CheckUpperIdentifier(paramValue) == false)
    {
      String msg = String::Format("Parameter '%s' must be a valid zilch uppercase identifier.", SpirVNameSettings::mNameOverrideParam.c_str());
      SendTranslationError(shaderAttribute->GetLocation(), msg);
      return;
    }
  }
...
void ZilchSpirVFrontEnd::ValidateSingleParamAttribute(ShaderIRAttribute* shaderAttribute, StringParam expectedParamName, Zilch::ConstantType::Enum expectedParamType, bool allowEmptyName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  if(paramCount == 0)
  {
    String msg = String::Format("Not enough parameters to attribute '%s'. Signature must be '%s : %s'",
      shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Zilch::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
  ...
  {
    String msg = String::Format("Invalid parameter type '%s' to attribute '%s'. Signature must be '%s : %s'",
      Zilch::ConstantType::Names[actualParamType], shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Zilch::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
  ...
  {
    String msg = String::Format("Invalid parameter name '%s' to attribute '%s'. Signature must be '%s : %s'",
      actualParamName.c_str(), shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Zilch::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
}
...
void ZilchSpirVFrontEnd::ValidateAttributeDependencies(ShaderIRAttribute* shaderAttribute, ShaderIRAttributeList& shaderAttributeList, Array<String>& dependencies)
{
...
void ZilchSpirVFrontEnd::ValidateAttributeExclusions(ShaderIRAttribute* shaderAttribute, ShaderIRAttributeList& shaderAttributeList, Array<String>& exclusions)
{
...
void ZilchSpirVFrontEnd::ValidateHardwareBuiltIn(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* shaderAttribute, bool isInput)
{
  // If this is an explicit attribute then check to see if it matches something in the uniform buffers
  if(!shaderAttribute->mImplicitAttribute)
  {
...
String ZilchSpirVFrontEnd::BuildFunctionTypeString(Zilch::Function* zilchFunction, ZilchSpirVFrontEndContext* context)
{
  ...
  if(zilchFunction != nullptr)
  {
    ...
    {
      // Get the actual 'this' type from the function (deals with extension methods)
...
String ZilchSpirVFrontEnd::BuildFunctionTypeString(Zilch::BoundType* zilchReturnType, Array<Zilch::Type*>& signature, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::GenerateFunctionType(Zilch::SyntaxNode* locationNode, ZilchShaderIRFunction* function, Zilch::Function* zilchFunction, ZilchSpirVFrontEndContext* context)
{
  ...
  if(zilchFunctionType != nullptr)
  {
    ...
    for(size_t i = 0; i < zilchFunctionType->Parameters.Size(); ++i)
    {
      ...
      // Make sure to pass through the correct shader type (pointer/value) based upon the zilch type.
      ...
      {
        // If this is a value type parameter that can't be copied then display an error.
        if(ContainsAttribute(shaderParameterType, SpirVNameSettings::mNonCopyableAttributeName))
        {
          String msg = String::Format("Type '%s' cannot be copied. This parameter must be passed through by reference (ref keyword).", shaderParameterType->mName.c_str());
          SendTranslationError(locationNode->Location, msg);
        }
...
void ZilchSpirVFrontEnd::GenerateFunctionType(Zilch::SyntaxNode* locationNode, ZilchShaderIRFunction* function, Zilch::BoundType* zilchReturnType, Array<Zilch::Type*>& signature, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIRFunction* ZilchSpirVFrontEnd::GenerateIRFunction(Zilch::SyntaxNode* node, Zilch::NodeList<Zilch::AttributeNode>* nodeAttributeList, ZilchShaderIRType* owningType, Zilch::Function* zilchFunction, StringParam functionName, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::AddImplements(Zilch::SyntaxNode* node, Zilch::Function* zilchFunction, ZilchShaderIRFunction* shaderFunction, StringParam functionName, ZilchSpirVFrontEndContext* context)
{
  if(zilchFunction == nullptr)
    return;
  ...
  // We found a replacement function. Add a duplicate mapping for the zilch function we're replacing to the target shader function
  ...
  // Otherwise we failed to find a function to implement (override). Now report an error to the user.
...
void ZilchSpirVFrontEnd::PreWalkClassVariables(Zilch::MemberVariableNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  {
    // @JoshD: Right now I'm assuming this isn't a function storage class. Change later?
...
{
  // If this is a geometry fragment, try and find the function Main(inputStream, outputStream)
  ZilchShaderIRType* currentType = context->mCurrentType;
  ...
  if(fragmentType == FragmentType::Geometry)
  {
    if(node->Parameters.Size() == 2)
    {
      ...
      // Write out user data to the type so the compositor can know what the main function looks like.
...
void ZilchSpirVFrontEnd::PreWalkErrorCheck(ZilchSpirVFrontEndContext* context)
{
  ...
  if(mSettings->mErrorSettings.mFrontEndErrorOnNoMainFunction)
  {
    if(typeMeta->mFragmentType == FragmentType::Geometry)
    {
      ...
      {
        String msg = "Geometry shader must have a 'Main' function of signature (InputStream, OutputStream).";
...
void ZilchSpirVFrontEnd::GeneratePreConstructor(Zilch::ClassNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIRFunction* function = GenerateIRFunction(node, nullptr, context->mCurrentType, zilchFunction, zilchFunction->Name, context);

  BasicBlock* currentBlock = BuildBlock(String(), context);
  context->mCurrentBlock = currentBlock;

  // Declare the self param
  ZilchShaderIROp* selfOp = BuildIROp(&function->mParameterBlock, OpType::OpFunctionParameter, currentType->mPointerType, context);
  ...
  for(size_t i = 0; i < node->Variables.Size(); ++i)
  {
    ...
    // If for some reason this variable isn't a member (e.g. samplers) then skip initialization.
    // This variable was probably promoted to a global.
...
void ZilchSpirVFrontEnd::GenerateDefaultConstructor(Zilch::ClassNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  // unfortunately don't have a backing zilch function so manually generate the signature.
...
void ZilchSpirVFrontEnd::GenerateStaticVariableInitializer(Zilch::MemberVariableNode*& node, ZilchSpirVFrontEndContext* context)
{
  // Ignore specialization constants. They're global but they can't have an initializer function.
...
void ZilchSpirVFrontEnd::DefaultConstructType(Zilch::SyntaxNode* locationNode, ZilchShaderIRType* type, ZilchShaderIROp* selfVar, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  // If we can find a default constructor resolver for this type then call it (e.g. Real)
  ...
  // Check if this type is forced to be a global type. If so then ignore the default
  // constructor because this class doesn't actually "own" the variable.
...
void ZilchSpirVFrontEnd::GenerateFunctionParameters(Zilch::GenericFunctionNode* node, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIRType* currentType = context->mCurrentType;
  // Debug sanity
  context->mCurrentBlock = nullptr;
  ...
  // Generate a function's parameter block. This contains the declarations and ids for all input parameters

  // First check if this is a member function. If so then we need to declare that we take in the this pointer.
  if(!zilchFunction->IsStatic)
  {
    // Get the actual 'this' type from the function (deals with extension methods)
    Zilch::BoundType* zilchThisType = zilchFunction->Owner;
  ...
  for(size_t i = 0; i < node->Parameters.Size(); ++i)
  {
    ...
    // types as the beginning statements so that the signature is actually correct.

    // We take all parameters by pointer type. This makes it significantly easier
    // to generate code in case the user ever assigns the the input
    ZilchShaderIROp* op = BuildIROp(&function->mParameterBlock, OpType::OpFunctionParameter, shaderParameterType, context);
...
void ZilchSpirVFrontEnd::GenerateEntryPoint(Zilch::GenericFunctionNode* node, ZilchShaderIRFunction* function, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::WalkFunctionCallNode(Zilch::FunctionCallNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  // Check if this is a member access function call (could be a function, member, etc...)
...
void ZilchSpirVFrontEnd::WalkConstructorCallNode(Zilch::FunctionCallNode*& node, Zilch::StaticTypeNode* constructorNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  // Deal with auto-generated default constructors (they don't have a generated zilch function)
...
void ZilchSpirVFrontEnd::WalkMemberAccessCallNode(Zilch::FunctionCallNode*& node, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();

  Zilch::Function* zilchFunction = memberAccessNode->AccessedFunction;
  // Build the function call op but don't add it as a line in the current block yet. We have to generate a bit more code to reference the parameters
  ...
  // Check for a member function resolver. If we find one on the 'this' type then leave translation up to it.
  Zilch::Type* selfType = memberAccessNode->LeftOperand->ResultType;
  // Make sure to handle indirection types otherwise resolvers won't be found (e.g. turn 'Array ref' into 'Array')
...
void ZilchSpirVFrontEnd::WalkMemberAccessFunctionCallNode(Zilch::FunctionCallNode*& node, Zilch::MemberAccessNode* memberAccessNode, ZilchShaderIRFunction* shaderFunction, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  // If this is an instance function call then add the self type as the first argument
  Zilch::Function* zilchFunction = memberAccessNode->AccessedFunction;
  if(!zilchFunction->IsStatic)
  {
...
void ZilchSpirVFrontEnd::WalkMemberAccessFunctionCall(Array<IZilchShaderIR*>& arguments, Zilch::MemberAccessNode* memberAccessNode, ZilchShaderIRFunction* shaderFunction, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GenerateFunctionCall(ZilchShaderIRFunction* shaderFunction, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GenerateFunctionCall(BasicBlock* block, ZilchShaderIRFunction* shaderFunction, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIROp* functionCallOp = GenerateFunctionCall(shaderFunction, context);
...
{
  // Should this variable declaration be forwarded to another variable? (Used in expression initializers).
  // If so, we just need to walk the variable this is referencing and return and mark that.
  ...
  // Since we're declaring a member variable we have to mark a dependency on the variable type
  ...
  // If there's no initial value then we're done (we've already declared the variable)
  if(node->InitialValue == nullptr)
    return;

  // Check if this type is non-copyable. If so we can't assign a default value (generate an error)
  ...
  // Validate that we can assign to the type (the result could be void or something else)
...
void ZilchSpirVFrontEnd::WalkStaticTypeOrCreationCallNode(Zilch::StaticTypeNode*& node, ZilchSpirVFrontEndContext* context)
{
  SendTranslationError(node->Location, "StaticTypeOrCreationCallNode not translatable.");
  context->PushIRStack(GenerateDummyIR(node, context));
}

void ZilchSpirVFrontEnd::WalkExpressionInitializerNode(Zilch::ExpressionInitializerNode*& node, ZilchSpirVFrontEndContext* context)
{
...
{
  // If this is a dereference operator then generically generate an OpLoad to turn a pointer type into a
  // value type (should only be used in extension methods that need to pass 'this' into a function)
  if(node->Operator->TokenId == Zilch::Grammar::Dereference)
  {
    ...
    ZilchShaderIROp* dereferenceOp = BuildCurrentBlockIROp(OpType::OpLoad, operand->mResultType->mDereferenceType, operand, context);
    context->PushIRStack(dereferenceOp);
    return;
  }

  // If this is an address of operator then this is almost always &variable which just returns the variable pointer
  // itself. The only time this isn't a pointer type is on the result of an expression such as &(a + b)
  // which I will currently always report as an error.
...
void ZilchSpirVFrontEnd::WalkBinaryOperationNode(Zilch::BinaryOperatorNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  {
    // Deal with setters. This requires 'promoting' the setter above the assignment op (e.g. A = B -> A.Set(B))
    ...
    // Validate that the left hand side is a pointer type otherwise we can't store to it
...
{
  // @JoshD: This probably has to change later with templates (the type might not exist)
  ...
  if(node->AccessedMember)
  {
    ...
    if(node->AccessedGetterSetter != nullptr)
    {
      ...
      if(node->IoUsage == Zilch::IoMode::ReadRValue)
      {
        ...
        // If this is an existing zilch function then we have to translate this member access into a function call
        if(shaderFunction != nullptr)
        {
          Array<IZilchShaderIR*> arguments;
      ...
      else
      {
        ...
        // level (binary ops right now). If we get here then something went wrong.
    ...
    {
      // Check if this is a global variable. If so just put the variable on the stack and
      // return (no member access chain). This happens with forced global types like samplers.
      ...
      // This is basically the same as a global except it requires a separate lookup.
    ...
    // If we failed to translate something more specialized up above then try to find a
    // field resolver. We have to do this last because some types (like vectors) can have backup field resolvers.
    // If we did this first then getters would fail to get called.
    MemberAccessResolverIRFn fieldResolver = mLibrary->FindFieldResolver(leftOperandType->mZilchType, node->AccessedField);
    ...
    // Get the member variable pointer. If this is a static variable then we access the globals map,
    // otherwise we walk the left operand and get the variable pointer to this id
    ...
    else
    {
      ...
      // Build the member access operation
      ZilchShaderIROp* memberAccessOp = BuildCurrentBlockIROp(OpType::OpCompositeExtract, memberType, operandResultOp, memberIndexLiteral, context);
      context->PushIRStack(memberAccessOp);
    }
  }
  else
...
void ZilchSpirVFrontEnd::WalkIfRootNode(Zilch::IfRootNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  // Pre-allocate the blocks for the if-statements (makes it easier to point at later blocks)
...
void ZilchSpirVFrontEnd::WalkReturnNode(Zilch::ReturnNode*& node, ZilchSpirVFrontEndContext* context)
{
  ...
  // We have to generate different op code depending on if this has a return value or not
  if(node->ReturnValue)
  {
    // @JoshD: Fix
    // For now, assume that all return types must be value types (deal with pointers later)
...
void ZilchSpirVFrontEnd::WalkGenericLoop(Zilch::SyntaxNode* initializerNode, Zilch::SyntaxNode* iterator, Zilch::ConditionalLoopNode* conditionalNode, Zilch::LoopScopeNode* loopScopeNode, ZilchSpirVFrontEndContext* context)
{
  // Always walk the initializer node first if it exists. The contents of this go before any loop block.
...
void ZilchSpirVFrontEnd::GenerateLoopHeaderBlock(BasicBlock* headerBlock, BasicBlock* branchTarget, BasicBlock* mergeBlock, BasicBlock* continueBlock, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::GenerateLoopConditionBlock(Zilch:: ConditionalLoopNode* conditionalNode, BasicBlock* conditionBlock, BasicBlock* branchTrueBlock, BasicBlock* branchFalseBlock, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::GenerateLoopStatements(Zilch::LoopScopeNode* loopScopeNode, BasicBlock* loopBlock, BasicBlock* mergeBlock, BasicBlock* continueBlock, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::GenerateLoopContinueBlock(Zilch::SyntaxNode* iterator, BasicBlock* continueBlock, BasicBlock* headerBlock, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::FixBlockTerminators(BasicBlock* block, ZilchSpirVFrontEndContext* context)
{
  ...
  for(size_t i = 0; i < opCount; ++i)
  {
    ...
    if(op->mOpType == OpType::OpReturn || op->mOpType == OpType::OpReturnValue ||
      op->mOpType == OpType::OpBranch || op->mOpType == OpType::OpBranchConditional ||
  ...
  // First terminator is the last op in the block. This block is good and there's nothing more to do.
  ...
  if(firstTerminatorIndex >= opCount)
  {
    ...
    // If the return type isn't void then this was likely a block that was generated after a conditional
    // that can't be reached (e.g. if + else and then after the else).
    // These statements should be unreachable otherwise we'd have a zilch error.
  ...
  // There's dead instructions after the terminator. Clean up everything afterwards
...
bool ZilchSpirVFrontEnd::ResolveSetter(Zilch::BinaryOperatorNode* node, ZilchShaderIROp* resultValue, Zilch::SyntaxNode* resultNode, ZilchSpirVFrontEndContext* context)
{
  Zilch::MemberAccessNode* memberAccessNode = Zilch::Type::DynamicCast<Zilch::MemberAccessNode*>(node->LeftOperand);
...
IZilchShaderIR* ZilchSpirVFrontEnd::PerformBinaryOp(Zilch::BinaryOperatorNode*& node, OpType opType, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
IZilchShaderIR* ZilchSpirVFrontEnd::PerformBinaryOp(Zilch::BinaryOperatorNode*& node, OpType opType, IZilchShaderIR* lhs, IZilchShaderIR* rhs, ZilchSpirVFrontEndContext* context)
{
...
void ZilchSpirVFrontEnd::PerformBinaryAssignmentOp(Zilch::BinaryOperatorNode*& node, OpType opType, ZilchSpirVFrontEndContext* context)
{
  // Walk both sides of the op
  IZilchShaderIR* leftInstruction = WalkAndGetResult(node->LeftOperand, context);
  IZilchShaderIR* rightInstruction = WalkAndGetResult(node->RightOperand, context);
...
void ZilchSpirVFrontEnd::PerformBinaryAssignmentOp(Zilch::BinaryOperatorNode*& node, OpType opType, IZilchShaderIR* lhs, IZilchShaderIR* rhs, ZilchSpirVFrontEndContext* context)
{
  // All binary operators require value types so load from pointers if we had them
  ZilchShaderIROp* leftValueOp = GetOrGenerateValueTypeFromIR(lhs, context);
  ZilchShaderIROp* rightValueOp = GetOrGenerateValueTypeFromIR(rhs, context);
  ...
  // Deal with setters. This requires 'promoting' the setter above the assignment op (e.g. A += B -> A.Set(A + B))
...
IZilchShaderIR* ZilchSpirVFrontEnd::PerformUnaryOp(Zilch::UnaryOperatorNode*& node, OpType opType, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIRType* resultType = FindType(node->ResultType, node);
...
IZilchShaderIR* ZilchSpirVFrontEnd::PerformUnaryIncDecOp(Zilch::UnaryOperatorNode*& node, IZilchShaderIR* constantOne, OpType opType, ZilchSpirVFrontEndContext* context)
{
  ...
  // If the operand was a pointer type instead of a temporary then we need to store the value back into the pointer
...
IZilchShaderIR* ZilchSpirVFrontEnd::PerformTypeCast(Zilch::TypeCastNode*& node, OpType opType, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIRType* resultType = FindType(node->ResultType, node);
  
  // Get the thing we're casting. Casts require a value type so generate a value type if necessary.
  ZilchShaderIROp* operandValueResult = WalkAndGetValueTypeResult(node->Operand, context);
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GetConstant(ZilchShaderIRType* type, Zilch::Any value, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::ConstructCompositeFromScalar(BasicBlock* block, ZilchShaderIRType* compositeType, IZilchShaderIR* scalar, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::AddSpecializationConstant(Zilch::MemberVariableNode* node, ZilchShaderIRType* varType, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIRConstantLiteral* defaultLiteral = nullptr;
  // If the initial value of the node is a value node then we can initialize this constant to the actual value.
  // Other types are more complicated as the default value requires actually constructing the type
  // to get value (something like var Pi : Real = Math.Pi). @JoshD: Deal with finding these constants later.
...
ZilchShaderIROp* ZilchSpirVFrontEnd::AddSpecializationConstantRecursively(void* key, ZilchShaderIRType* varType, StringParam varName, ZilchShaderIRConstantLiteral* literalValue, Zilch::CodeLocation& codeLocation, ZilchSpirVFrontEndContext* context)
{
  ...
  if(varType->mBaseType == ShaderIRTypeBaseType::Vector || varType->mBaseType == ShaderIRTypeBaseType::Matrix)
  {
    ...
    // Create a sub-constant for each constituent. For vectors these are scalar, for matrices these are vectors.
    ...
    for(size_t i = 0; i < varType->mComponents; ++i)
    {
      ...
      ZilchShaderIROp* constituent = AddSpecializationConstantRecursively(nullptr, componentType, fullSubVarName, nullptr, codeLocation, context);
      specConstantCompositeOp->mArguments.PushBack(constituent);
    }
    return specConstantCompositeOp;
  }

  // Structs are handled almost the same as vectors/matrices, the only difference is how
  // members are iterated over and how to create their debug names.
  if(varType->mBaseType == ShaderIRTypeBaseType::Struct)
  {
    ...
    for(size_t i = 0; i < varType->mParameters.Size(); ++i)
    {
      ...
      ZilchShaderIROp* constituent = AddSpecializationConstantRecursively(nullptr, subType, fullSubVarName, nullptr, codeLocation, context);
      specConstantCompositeOp->mArguments.PushBack(constituent);
    }
    return specConstantCompositeOp;
  }
...
ZilchShaderIROp* ZilchSpirVFrontEnd::CreateSpecializationConstant(void* key, OpType opType, ZilchShaderIRType* varType, ZilchSpirVFrontEndContext* context)
{
  ...
  // Additionally, if there's a valid key then store this in the library's lookup map.
  // This is used to find the constant at a later point in time via the key (such as the zilch field).
  // If the key is null then we don't have/need to lookup this constant later (e.g. a sub-member of a vector).
  if(key != nullptr)
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, ZilchShaderIRType* resultType, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, IZilchShaderIR* arg1, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIROp* result = BuildIROp(block, opType, resultType, context);
  result->mArguments.PushBack(arg0);
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, IZilchShaderIR* arg1, IZilchShaderIR* arg2, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIROp* result = BuildIROp(block, opType, resultType, context);
  result->mArguments.PushBack(arg0);
  result->mArguments.PushBack(arg1);
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, ZilchShaderIRType* resultType, ZilchSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, ZilchSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, arg0, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, IZilchShaderIR* arg1, ZilchSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, arg0, arg1, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, ZilchShaderIRType* resultType, IZilchShaderIR* arg0, IZilchShaderIR* arg1, IZilchShaderIR* arg2, ZilchSpirVFrontEndContext* context)
{
...
{
  // All variable declarations must be at the beginning of the first block of a function.
  // To do this we always grab the first block and add to the local variables section of this block (which is emitted first)
...
ZilchShaderIROp* ZilchSpirVFrontEnd::BuildOpVariable(BasicBlock* block, ZilchShaderIRType* resultType, int storageConstant, ZilchSpirVFrontEndContext* context)
{
  ...
  // Make the variable declaration (with function storage) and add it to the local variables section of the block
...
ZilchShaderIROp* ZilchSpirVFrontEnd::WalkAndGetValueTypeResult(BasicBlock* block, Zilch::SyntaxNode* node, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::WalkAndGetValueTypeResult(Zilch::SyntaxNode* node, ZilchSpirVFrontEndContext* context)
{
  IZilchShaderIR* nodeResult = WalkAndGetResult(node, context);
  // The block must be fetched here as walking the node can change the current block
  ZilchShaderIROp* valueResult = GetOrGenerateValueTypeFromIR(context->GetCurrentBlock(), nodeResult, context);
  return valueResult;
}

ZilchShaderIROp* ZilchSpirVFrontEnd::GetOrGenerateValueTypeFromIR(BasicBlock* block, IZilchShaderIR* instruction, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GetOrGenerateValueTypeFromIR(IZilchShaderIR* instruction, ZilchSpirVFrontEndContext* context)
{
  return GetOrGenerateValueTypeFromIR(context->GetCurrentBlock(), instruction, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::GetOrGeneratePointerTypeFromIR(BasicBlock* block, IZilchShaderIR* instruction, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIROp* op = instruction->As<ZilchShaderIROp>();
  if(op->IsResultPointerType())
    return op;
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GetOrGeneratePointerTypeFromIR(IZilchShaderIR* instruction, ZilchSpirVFrontEndContext* context)
{
  return GetOrGeneratePointerTypeFromIR(context->GetCurrentBlock(), instruction, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::BuildStoreOp(IZilchShaderIR* target, IZilchShaderIR* source, ZilchSpirVFrontEndContext* context, bool forceLoadStore)
{
  return BuildStoreOp(context->GetCurrentBlock(), target, source, context);
}

ZilchShaderIROp* ZilchSpirVFrontEnd::BuildStoreOp(BasicBlock* block, IZilchShaderIR* target, IZilchShaderIR* source, ZilchSpirVFrontEndContext* context, bool forceLoadStore)
{
  ZilchShaderIROp* sourceOp = source->As<ZilchShaderIROp>();
  // Change what op we use to store depending on if the source is a value or pointer type.
  // If the source was a pointer type we'd have to load it first before setting,
  // but instead we can just use CopyMemory.
  if(sourceOp->IsResultPointerType())
  {
    // Currently there's various issues with the spirv tool chains that don't handle OpCopyMemory well.
    // For this reason support forcing load/store instead.
    if(forceLoadStore)
    {
      ZilchShaderIROp* loadOp = BuildIROp(block, OpType::OpLoad, sourceOp->mResultType->mDereferenceType, sourceOp, context);
...
void ZilchSpirVFrontEnd::WriteFunctionCallArguments(Zilch::FunctionCallNode*& node, ZilchShaderIROp* functionCallOp, ZilchSpirVFrontEndContext* context)
{
  WriteFunctionCallArguments(node, node->ResultType, functionCallOp, context);
}

void ZilchSpirVFrontEnd::WriteFunctionCallArguments(Zilch::FunctionCallNode*& node, Zilch::Type* returnType, ZilchShaderIROp* functionCallOp, ZilchSpirVFrontEndContext* context)
{
  // @JoshD: This function is somewhat legacy and would be nice to remove eventually
  // and unify with the below one. The issue right now is with calling extended instructions.
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void ZilchSpirVFrontEnd::WriteFunctionCallArguments(Array<IZilchShaderIR*> arguments, ZilchShaderIRType* returnType, ZilchShaderIRType* functionType, ZilchShaderIROp* functionCallOp, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void ZilchSpirVFrontEnd::WriteFunctionCallArgument(IZilchShaderIR* argument, ZilchShaderIROp* functionCallOp, ZilchShaderIRType* paramType, ZilchSpirVFrontEndContext* context)
{
  ...
  // the given parameter op is a pointer the convert the pointer to a value type.
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GenerateBoolToIntegerCast(BasicBlock* block, ZilchShaderIROp* source, ZilchShaderIRType* destType, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GenerateIntegerToBoolCast(BasicBlock* block, ZilchShaderIROp* source, ZilchShaderIRType* destType, ZilchSpirVFrontEndContext* context)
{
...
ZilchShaderIROp* ZilchSpirVFrontEnd::GenerateToBoolCast(BasicBlock* block, OpType op, ZilchShaderIROp* source, ZilchShaderIRType* destType, IZilchShaderIR* zero, ZilchSpirVFrontEndContext* context)
{
  ...
  {
    // Construct the comparison source vector type from the individual zero scalar
...
bool ZilchSpirVFrontEnd::CheckForNonCopyableType(ZilchShaderIRType* shaderType, Zilch::ExpressionNode* node, ZilchSpirVFrontEndContext* context, bool generateDummyResult)
{
...
bool ZilchSpirVFrontEnd::ValidateResultType(IZilchShaderIR* instruction, ShaderIRTypeBaseType::Enum expectedType, Zilch::CodeLocation& codeLocation, bool throwException)
{
  ZilchShaderIROp* op = instruction->As<ZilchShaderIROp>();
...
void ZilchSpirVFrontEnd::SendTranslationError(Zilch::CodeLocation& codeLocation, StringParam shortMsg, StringParam fullMsg)
{
