...
void ResolveBoolToRealCast(ZilchSpirVFrontEnd* translator, Zilch::TypeCastNode* node, ZilchSpirVFrontEndContext* context)
{
...
void ResolveToBoolCast(ZilchSpirVFrontEnd* translator, Zilch::TypeCastNode* node, OpType op, IZilchShaderIR* zero, ZilchSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ZilchShaderIROp* condition = translator->WalkAndGetValueTypeResult(node->Operand, context);
  ZilchShaderIRType* destType = translator->FindType(node->ResultType, node);
...
void ResolveRealToBoolCast(ZilchSpirVFrontEnd* translator, Zilch::TypeCastNode* node, ZilchSpirVFrontEndContext* context)
{
  ZilchShaderIRType* realType = translator->FindType(ZilchTypeId(float), node, context);
  IZilchShaderIR* zero = translator->GetConstant(realType, 0.0f, context);
