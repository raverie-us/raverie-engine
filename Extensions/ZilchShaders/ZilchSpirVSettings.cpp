...
//-------------------------------------------------------------------UniformBufferDescription
ShaderStage::Enum UniformBufferDescription::mAllStagesMask = (ShaderStage::Vertex | ShaderStage::PreTesselation | ShaderStage::PostTesselation | ShaderStage::Geometry | ShaderStage::Pixel | ShaderStage::Compute);
...
void UniformBufferDescription::Set(int bindingId, int descriptorSetId, ShaderStage::Enum allowedStages, StringParam debugName)
{
...
void BuiltInBlockDescription::BuiltInFieldMeta::CopyFrom(const BuiltInFieldMeta& source)
{
  ...
  mMeta->mZilchType = source.mMeta->mZilchType;
  mMeta->mAttributes = source.mMeta->mAttributes;
  mId = source.mId;
}

void BuiltInBlockDescription::AddField(Zilch::BoundType* type, StringParam fieldName, spv::BuiltIn builtInId, StringParam attribute)
{
  BuiltInFieldMeta builtInMeta;
  builtInMeta.mMeta = new ShaderIRFieldMeta();
  builtInMeta.mMeta->mZilchName = fieldName;
  builtInMeta.mMeta->mZilchType = type;
...
void BuiltInBlockDescription::SetBuiltInName(spv::BuiltIn builtInId, StringParam name)
{
  ...
  for(size_t i = 0; i < mFields.Size(); ++i)
  {
    ...
    fieldMeta.mMeta->mZilchName = name;
  }
}
...
ZilchShaderErrorSettings::ZilchShaderErrorSettings()
{
...
bool ZilchShaderSpirVSettings::IsValidUniform(FragmentType::Enum fragmentType, StringParam fieldType, StringParam fieldName)
{
...
bool ZilchShaderSpirVSettings::IsValidHardwareBuiltIn(FragmentType::Enum fragmentType, StringParam fieldType, StringParam fieldName, bool isInput)
{
  ShaderFieldKey fieldKey(fieldName, fieldType);
  ...
  // if no fragment type is set then check all available stage descriptions so at least typos / wrong types could be caught.
...
void ZilchShaderSpirVSettings::SetMaxSimultaneousRenderTargets(size_t maxNumber)
{
...
String ZilchShaderSpirVSettings::GetLanguageVersionSpecializationName()
{
  return mLanguageVersionSpecConstantName;
}

void* ZilchShaderSpirVSettings::GetLanguageSpecializationKey()
{
  return &mLanguageSpecConstantName;
}

void* ZilchShaderSpirVSettings::GetLanguageVersionSpecializationKey()
{
  return &mLanguageVersionSpecConstantName;
}

void ZilchShaderSpirVSettings::Finalize()
{
  if(mFinalized)
  {
...
bool ZilchShaderSpirVSettings::IsFinalized() const
{
  return mFinalized;
}

void ZilchShaderSpirVSettings::Validate()
{
...
void ZilchShaderSpirVSettings::ValidateUniformsDescriptions()
{
  ...
  for(size_t i = 0; i < mUniformBufferDescriptions.Size(); ++i)
  {
    ...
    if(descriptorMap.ContainsKey(descriptorPair))
    {
      ...
      Error("Uniform block '%s' shares the descriptor set '%d' and binding '%d' with block '%s'.",
        description.mDebugName.c_str(), descriptorPair.first, descriptorPair.second, oldDescription->mDebugName.c_str());
    }
    ...
    for(size_t fieldId = 0; fieldId < description.mFields.Size(); ++fieldId)
    {
      ...
      { 
        Error("Field '%s' on block '%s' is registered for the same stage multiple times.",
          fieldKey.mKey.c_str(), description.mDebugName.c_str());
      }
...
void ZilchShaderSpirVSettings::ValidateBuiltInNames(BuiltInBlockDescription& blockDescription, HashMap<String, spv::BuiltIn>& keyMappings)
{
...
void ZilchShaderSpirVSettings::ValidateAppBuiltInsAgainstHardwareBuiltIns()
{
  for(size_t i = 0; i < mUniformBufferDescriptions.Size(); ++i)
  {
    UniformBufferDescription& description = mUniformBufferDescriptions[i];
    ...
    for(size_t j = 0; j < description.mFields.Size(); ++j)
    {
      ...
      ErrorIf(isHardwardBuiltIn, "AppBuiltIn '%s : %s' matches a HardwareBuiltIn. "
        "This is currently not supported as additional name mangling would be necessary to prevent name conflicts which would make reflection significantly more complicated.",
        fieldMeta->mZilchName.c_str(), fieldMeta->mZilchType->ToString().c_str());
    }
  }
}

void ZilchShaderSpirVSettings::InitializeBuiltIns()
{
