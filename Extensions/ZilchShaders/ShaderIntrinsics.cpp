...
ZilchShaderIROp* GetOrCreateLanguageSpecConstant(ZilchSpirVFrontEnd* translator, void* specKey, int defaultValue, StringParam specName, ZilchSpirVFrontEndContext* context)
{
...
void ResolveIsLanguage(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* comparisonLanguageOp = translator->WalkAndGetValueTypeResult(functionCallNode->Arguments[0], context);
  ZilchShaderIROp* result = translator->BuildCurrentBlockIROp(OpType::OpIEqual, boolType, languageSpecConst, comparisonLanguageOp, context);
...
void ResolveIsLanguageMinMaxVersion(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* isLanguageOp = translator->BuildCurrentBlockIROp(OpType::OpIEqual, boolType, languageSpecConst, comparisonLanguageOp, context);
  // Check the minVersion <= languageVersion <= maxVersion
  ZilchShaderIROp* isGreaterThanMinOp = translator->BuildCurrentBlockIROp(OpType::OpSLessThanEqual, boolType, minVersionOp, languageVersionSpecConst, context);
  ZilchShaderIROp* isLessThanMaxOp = translator->BuildCurrentBlockIROp(OpType::OpSLessThanEqual, boolType, languageVersionSpecConst, maxVersionOp, context);
  // Combine the comparisons together into one bool
  ZilchShaderIROp* isInVersionRange = translator->BuildCurrentBlockIROp(OpType::OpLogicalAnd, boolType, isGreaterThanMinOp, isLessThanMaxOp, context);
  ZilchShaderIROp* result = translator->BuildCurrentBlockIROp(OpType::OpLogicalAnd, boolType, isLanguageOp, isInVersionRange, context);
...
void RegisterShaderIntrinsics(ZilchSpirVFrontEnd* translator, ZilchShaderIRLibrary* shaderLibrary)
{
  ...
  Zilch::BoundType* shaderIntrinsicsType = shaderLibrary->mZilchLibrary->BoundTypes.FindValue("ShaderIntrinsics", nullptr);
