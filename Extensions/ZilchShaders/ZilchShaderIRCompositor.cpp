...
bool ZilchShaderIRCompositor::Composite(ShaderDefinition& shaderDef, const ShaderCapabilities& capabilities, ZilchShaderSpirVSettingsRef& settings)
{
  ...
  // Figure out what each stage could output. This is necessary to actually link any inputs.
  ...
  // The gpu stage is special as it's the only stage where we don't have fragments to walk for inputs.
  // Instead we have to walk the possible render targets and check for stage outputs from previous stages.
  ResolveGpuStage(compositeInfo);

  // Fix the order of stage in/out variables so that the declaration order will match. In a simple scenario
  // the fields will already be in the right order because they're declared when linked. When pass-through
  // happen though the order can get mixed up. For example, a pixel's first input can trigger a pass-through
  // which adds this to the end of the vertex stage, the geometry stage has to make sure it matches these in/outs in the right order.
  ...
  {
    StageLinkingInfo* currentStage = compositeInfo.mActiveStages[i];
    GenerateZilchComposite(currentStage, shaderDef.mResults[currentStage->mFragmentType], shaderDef.mExtraAttributes);
  }
...
void ZilchShaderIRCompositor::CollectFragmentsPerStage(Array<ZilchShaderIRType*>& inputFragments, CompositedShaderInfo& compositeInfo)
{
  ...
  for(size_t i = 0; i < inputFragments.Size(); ++i)
  {
    ...
    {
      // Add this fragment to the corresponding compositor shader stage (not to be confused with 'ShaderStage')
  ...
  // The same is true for the pixel stage (they operator on interpolated vertex data)
  ...
  // Now record what stages are actually active. A stage is active if it has any fragments.
  // The cpu, vertex, pixel, and gpu stage are always active though. The hardware stages
  // are always present and vertex/pixel shaders have to exist.
  // They also must be auto-generated for pass-through to work.
  for(size_t i = 0; i < CompositorShaderStage::Size; ++i)
  {
    StageLinkingInfo* stageInfo = &compositeInfo.mStages[i];
    if(i == CompositorShaderStage::Cpu || i == CompositorShaderStage::Gpu || 
      i == CompositorShaderStage::Vertex || i == CompositorShaderStage::Pixel ||
      !stageInfo->mFragmentTypes.Empty())
      compositeInfo.mActiveStages.PushBack(stageInfo);
  }

  // For each active stage, link the attachment sub-stages to their previous and next
  // respective attachment sub-stages. This is necessary when resolving stage in/outs to facilitate pass-through.
...
bool ZilchShaderIRCompositor::ValidateStages(CompositedShaderInfo& compositeInfo)
{
  // Currently only check for an active stage that our capabilities don't support.
  for(size_t i = 0; i < compositeInfo.mActiveStages.Size(); ++i)
  {
    ...
    for(size_t i = 0; i < stageInfo->mFragmentTypes.Size(); ++i)
    {
      ...
      if(!fragmentType->mHasMainFunction)
      {
        ...
        ReturnIf(zilchType == nullptr, false, "Cannot composite a fragment with no backing zilch type");

        String msg;
        if(stageInfo->mFragmentType == FragmentType::Geometry)
          msg = String::Format("Geometry fragment '%s' must have a 'Main' function of signature(InputStream, OutputStream).", zilchType->Name.c_str());
        else
    ...
    if(stageInfo->mFragmentType == FragmentType::Geometry)
    {
      ...
      {
        Zilch::BoundType* zilchType = fragmentType->mMeta->mZilchType;
        SendTranslationError(zilchType->Location, "Geometry shader stage only supports one fragment at a time.");
        return false;
      }
    }
  }
...
void ZilchShaderIRCompositor::CollectExpectedOutputs(CompositedShaderInfo& compositeInfo)
{
  ...
  for(size_t i = 1; i < compositeInfo.mActiveStages.Size() - 1; ++i)
  {
    StageLinkingInfo* currentStage = compositeInfo.mActiveStages[i];
    StageLinkingInfo* previousStage = compositeInfo.mActiveStages[i - 1];
    // Any previous stage's expected output could be the current stage's output due to pass-through
...
void ZilchShaderIRCompositor::CreateCpuStage(CompositedShaderInfo& compositeInfo)
{
...
void ZilchShaderIRCompositor::ResolveInputs(StageLinkingInfo* previousStage, StageLinkingInfo* currentStage)
{
  // Link together the two stages. This requires linking the vertex to vertex and
  // primitive to primitive. Primitive stages are mostly for geometry/tessellation.
...
void ZilchShaderIRCompositor::Link(StageAttachmentLinkingInfo& prevStageInfo, StageLinkingInfo* currentStage, Array<ZilchShaderIRType*>& fragmentTypes, StageAttachmentLinkingInfo& currStageInfo)
{
  ...
  for(size_t fragIndex = 0; fragIndex < fragmentTypes.Size(); ++fragIndex)
  {
    ...
    for(size_t fieldIndex = 0; fieldIndex < fragmentTypeMeta->mFields.Size(); ++fieldIndex)
    {
      ...
      // Non-copyable fields (like samplers) cannot be composited as they can't be copied.
      // These fields are instead output by the owning fragment. We do however need to
      ...
      // Create the information about how this field was linked within this shader stage
      ...
      for(size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
      {
        ...
        {
          // If the fragment input matches a previous fragment's output then this attribute is satisfied
          ShaderFieldKey fieldKey = MakeFieldKey(fieldMeta, attribute);
        ...
        {
          // If the stage input matches a previous stage's potential output then this attribute is satisfied
          ShaderFieldKey fieldKey = MakeFieldKey(fieldMeta, attribute);
          ...
          if(previousStageExpectedOutput != nullptr)
          {
            ...
            // A stage output only becomes an actual output if an input matches it.
            // Mark this property as an input/output between the given stages (including pass-through).
        ...
        if(attribute->mAttributeName == nameSettings.mPropertyInputAttribute)
        {
          ...
          fieldLinkInfo.mFieldPropertyName = MakePropertyName(fieldLinkInfo.mOutputFieldDependencyName, fieldMeta->mOwner->mZilchName);
          break;
        }
        ...
        {
          // Specialization constant inputs are treated almost the exact same as property inputs.
          ...
          // The main difference is the attributes we add, in particular the actual
          // specialization constant must be declared as a static variable.
      ...
      // Check for stage output attributes. We have to loop to check for multiple fulfillment (due to name overrides)
      for(size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
      {
        ShaderIRAttribute* attribute = fieldMeta->mAttributes.GetAtIndex(attributeIndex);

        // Check if this is a fragment output, if so then park it as the last fragment to output this field
        ...
        // Check for hardware built-in outputs (gpu built-ins). This has to match a pre-existing built-in to be valid.
        if(attribute->mAttributeName == nameSettings.mHardwareBuiltInOutputAttribute)
        {
          ...
          if(builtInFieldMeta != nullptr)
          {
            ...
            ResolvedFieldOutputInfo resolvedFieldOutput(builtInFieldMeta->mZilchName, fieldMeta);
...
void ZilchShaderIRCompositor::AddStageInput(ExpectedOutputData* previousStageOutputData, StageAttachmentLinkingInfo* currStageAttachment, StringParam fieldVarName, StringParam fieldAttributeName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  // output was last declared in. For each stage, add in/outs between the current 
  // stage and the previous so that pass-through happens.
  ...
  // Mark a resolved output field for the stage that actually emitted the output.
  // This is only needed in this stage as this is the only stage that has to write
  // out the last fragment to the stage variable.
...
ShaderIRFieldMeta* ZilchShaderIRCompositor::FindUniform(ShaderFieldKey& fieldKey, FragmentType::Enum fragmentType)
{
...
ShaderIRFieldMeta* ZilchShaderIRCompositor::FindHardwareBuiltInInput(ShaderFieldKey& fieldKey, FragmentType::Enum fragmentType)
{
  ZilchShaderSpirVSettings* settings = mSettings;
...
ShaderIRFieldMeta* ZilchShaderIRCompositor::FindHardwareBuiltInOutput(ShaderFieldKey& fieldKey, FragmentType::Enum fragmentType)
{
  ZilchShaderSpirVSettings* settings = mSettings;
  BuiltInStageDescription& builtInStageDescription = settings->mBuiltIns[fragmentType];
...
ShaderFieldKey ZilchShaderIRCompositor::MakeFieldKey(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
  String fieldName = GetFieldInOutName(fieldMeta, attribute);
  return ShaderFieldKey(fieldName, fieldMeta->mZilchType->ToString());
}

String ZilchShaderIRCompositor::MakePropertyName(StringParam fieldName, StringParam ownerType)
{
  return GenerateSpirVPropertyName(fieldName, ownerType);
}

String ZilchShaderIRCompositor::GetFieldInOutName(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
  return fieldMeta->GetFieldAttributeName(attribute);
}


void ZilchShaderIRCompositor::GetStageFieldName(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute, String& fieldVarNameOut, String& fieldAttributeNameOut)
{
  ...
  // The actual field name needs to be mangled to avoid conflicts. To do this pre-pend with 'stage' 
  // and post-pend with the type. Type is needed to prevent conflicts between different stage variables
  // with the same name but different types. 'Stage' is needed to prevent conflicts with BuiltIns.
  fieldVarNameOut = BuildString("Stage_", fieldAttributeNameOut, "_", fieldMeta->mZilchType->ToString());
}
...
void ZilchShaderIRCompositor::ResolveGpuStage(CompositedShaderInfo& compositeInfo)
{
  ...
  for(size_t i = 0; i < settings->mRenderTargetNames.Size(); ++i)
  {
    ...
    if(previousStageExpectedOutput != nullptr)
    {
      ...
      // Get the render target field's name and attribute name (the attribute name is always renderTargetName)
      String fieldVarName, fieldAttributeName;
...
void ZilchShaderIRCompositor::ResolveStageLinkOrder(CompositedShaderInfo& compositeInfo)
{
  // For all programmable shader stage pairings (e.g. no cpu/gpu), make sure the in/out orders match
  for(size_t i = 1; i < compositeInfo.mActiveStages.Size() - 2; ++i)
  {
    ZilchShaderIRCompositor::StageLinkingInfo* currStage = compositeInfo.mActiveStages[i];
    ZilchShaderIRCompositor::StageLinkingInfo* nextStage = compositeInfo.mActiveStages[i + 1];
    ...
    // The inputs and outputs should match at this point in contents, just not order. Simply override the output order with the inputs
    outputs.Clear();
...
void ZilchShaderIRCompositor::GenerateZilchComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  // Invoke the custom compositor callback if it exists. This allows forcing certain in/outs.
...
void ZilchShaderIRCompositor::GenerateBasicZilchComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  // Write out all fields we resolve. Vertex fragments need to preserve their output order
  // so interfaces match with all subsequent stages (vertex definition order doesn't matter).
  // Every other stage can just match inputs first (currently only pixel).
...
void ZilchShaderIRCompositor::GenerateGeometryZilchComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ShaderCodeBuilder builder;
  ...
  // Declare any primitive fields. These don't have a specific order as no stage inputs exist here.
  ...
  String compositeInputStreamType = BuildString(zilchInputStreamType->TemplateBaseName, "[" , inputTypeName, "]");
  String compositeOutputStreamType = BuildString(zilchOutputStreamType->TemplateBaseName, "[", outputTypeName, "]");
  // Generate the entry points function signature: [EntryPoint] function Main(input : 'InputType', output : 'OutputType')
  builder << builder.EmitIndent();
  ...
  builder.WriteLocalVariableDefaultConstruction("fragmentInput", zilchInputStreamType->Name);
  ...
  for(size_t i = 0; i < inputVertexLinkInfo.mFieldList.Size(); ++i)
  {
    ...
    // If this was linked as a stage input (should only be stage or none) then copy from the composite input data
    if(linkInfo.mLinkedType == LinkedFieldType::Stage)
    {
      builder << builder.EmitIndent() << "fragmentInput[i]." << linkInfo.mFieldMeta->mZilchName;
  ...
  builder.WriteLocalVariableDefaultConstruction("fragmentOutput", zilchOutputStreamType->Name);
...
void ZilchShaderIRCompositor::CreateFragmentAndCopyInputs(StageLinkingInfo* currentStage, ShaderCodeBuilder& builder, StringParam currentClassName, ZilchShaderIRType* fragmentType, StringParam fragmentVarName)
{
  ...
  for(size_t i = 0; i < linkInfo.mFieldList.Size(); ++i)
  {
    ...
    // If this is a property input, then we have to copy from the mangled property input name
    if(fieldLinkInfo.mLinkedType == LinkedFieldType::Property)
    {
      builder << builder.EmitIndent() << fragmentVarName << "." << fieldMeta->mZilchName << " = ";
      builder << "this." << fieldLinkInfo.mFieldPropertyName;
      builder << ";" << builder.EmitLineReturn();
    }
    // If this is a fragment input, then copy from the resolved dependency fragment
    else if(fieldLinkInfo.mLinkedType == LinkedFieldType::Fragment)
    {
      ...
      builder << builder.EmitIndent() << fragmentVarName << "." << fieldMeta->mZilchName << " = ";
      builder << fragmentDependencyVarName << "." << outputFieldDependency->mZilchName;
      builder << ";" << builder.EmitLineReturn();
    }
    // If this is a stage input, copy from the composite given the dependency variable's name
    ...
    // If this is a application built-in, copy from the composite given the dependency variable's name
...
void ZilchShaderIRCompositor::DeclareFieldsInOrder(ShaderCodeBuilder& builder, StageAttachmentLinkingInfo& linkingInfo, OrderedHashSet<ShaderFieldKey>& orderMap)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  // Then declare everything else in the resolved fields. If we've already declared a field skip it.
...
void ZilchShaderIRCompositor::DeclareFieldsWithAttribute(ShaderCodeBuilder& builder, StageAttachmentLinkingInfo& linkingInfo, OrderedHashSet<ShaderFieldKey>& fieldSet, StringParam attributeName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void ZilchShaderIRCompositor::GenerateStageDescriptions(CompositedShaderInfo& compositeInfo, ShaderDefinition& shaderDef)
{
  ...
  for(size_t i = 1; i < compositeInfo.mActiveStages.Size() - 1; ++i)
  {
    ...
    for(size_t fragIndex = 0; fragIndex < currentStage->mFragmentTypes.Size(); ++fragIndex)
    {
      ...
      // Create the output description for this fragment. Copy this data from the internal linkage info
...
//-------------------------------------------------------------------FragmentLinkingInfo
void ZilchShaderIRCompositor::FragmentLinkingInfo::CopyTo(ShaderFragmentDescription& fragDesc)
{
...
void ZilchShaderIRCompositor::FragmentLinkingInfo::CopyFieldTo(FieldLinkingInfo& fieldInfo, ShaderFragmentDescription& fragDesc)
{
...
ZilchShaderIRCompositor::ResolvedFieldOutputInfo::ResolvedFieldOutputInfo(StringParam fieldName, ShaderIRFieldMeta* outputDependency)
{
...
ZilchShaderIRCompositor::StageAttachmentLinkingInfo::StageAttachmentLinkingInfo()
{
...
void ZilchShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(ShaderIRFieldMeta* fieldMeta, StringParam attributeName)
{
  AddResolvedField(fieldMeta->mZilchName, fieldMeta->mZilchType->ToString(), attributeName);
}

void ZilchShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedFieldProperty(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
  String baseFieldName = ZilchShaderIRCompositor::GetFieldInOutName(fieldMeta, attribute);
  String fieldName = ZilchShaderIRCompositor::MakePropertyName(baseFieldName, fieldMeta->mOwner->mZilchName);
  AddResolvedField(fieldName, fieldMeta->mZilchType->ToString(), attribute->mAttributeName);
}

void ZilchShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedStageField(SpirVNameSettings& nameSettings, ShaderIRFieldMeta* fieldMeta, StringParam fieldName, StringParam attributeName, StringParam attributeParameter)
{
  // Add the field with the given attribute
  ShaderIRAttribute* fieldAttribute = AddResolvedField(fieldName, fieldMeta->mZilchType->ToString(), attributeName);
  ...
  // Record this as either an input or output. This records what order inputs/outputs must be declared in.
...
ShaderIRAttribute* ZilchShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(StringParam fieldName, StringParam fieldType, StringParam attributeName)
{
...
ZilchShaderIRCompositor::ResolvedFieldInfo* ZilchShaderIRCompositor::StageAttachmentLinkingInfo::CreateResolvedField(StringParam fieldName, StringParam fieldType)
{
...
void ZilchShaderIRCompositor::StageLinkingInfo::SetFragmentType(FragmentType::Enum fragType)
{
...
void ZilchShaderIRCompositor::ApiPerspectivePositionCallback(CompositorCallbackData& callbackData, void* userData)
{
