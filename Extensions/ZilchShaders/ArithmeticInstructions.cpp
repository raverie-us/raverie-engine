...
void ResolveIntIncDecUnaryOperator(ZilchSpirVFrontEnd* translator, Zilch::UnaryOperatorNode* unaryOpNode, ZilchSpirVFrontEndContext* context)
{
...
template <OpType opType>
void ResolveFloatIncDecUnaryOperator(ZilchSpirVFrontEnd* translator, Zilch::UnaryOperatorNode* unaryOpNode, ZilchSpirVFrontEndContext* context)
{
...
void ResolveFMod(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ResolveStaticBinaryFunctionOp(translator, functionCallNode, spv::OpFMod, context);
}

void ResolveDot(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
...
template <OpType opType>
void ResolveVectorOpSplatScalar(ZilchSpirVFrontEnd* translator, Zilch::BinaryOperatorNode* binaryOpNode, ZilchSpirVFrontEndContext* context)
{
  ...
  ZilchShaderIROp* splattedScalarOperand = translator->ConstructCompositeFromScalar(currentBlock, vectorType, scalarOperand, context);
...
template <OpType opType>
void ResolveSimpleStaticBinaryFunctionOp(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
...
void ResolveMatrixTimesVector(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ResolveFlippedStaticBinaryFunctionOp(translator, functionCallNode, OpType::OpVectorTimesMatrix, context);
}

void ResolveMatrixTimesMatrix(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ResolveFlippedStaticBinaryFunctionOp(translator, functionCallNode, OpType::OpMatrixTimesMatrix, context);
}

void ResolveMatrixTranspose(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  // Get the result type
  ZilchShaderIRType* resultType = translator->FindType(functionCallNode->ResultType, functionCallNode);
  // Walk each operand
...
void RegisterArithmeticOps(ZilchSpirVFrontEnd* translator, ZilchShaderIRLibrary* shaderLibrary, TypeGroups& types)
{
  ...
  opResolvers.RegisterUnaryOpResolver(intType, Zilch::Grammar::Increment, ResolveIntIncDecUnaryOperator<OpType::OpIAdd>);
  opResolvers.RegisterUnaryOpResolver(intType, Zilch::Grammar::Decrement, ResolveIntIncDecUnaryOperator<OpType::OpISub>);
  opResolvers.RegisterUnaryOpResolver(realType, Zilch::Grammar::Increment, ResolveFloatIncDecUnaryOperator<OpType::OpFAdd>);
  opResolvers.RegisterUnaryOpResolver(realType, Zilch::Grammar::Decrement, ResolveFloatIncDecUnaryOperator<OpType::OpFSub>);
  ...
  for(size_t i = 0; i < types.mRealVectorTypes.Size(); ++i)
  {
    ...
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentSubtract, ResolveBinaryOperator<OpType::OpFSub>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentMultiply, ResolveBinaryOperator<OpType::OpFMul>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentDivide, ResolveBinaryOperator<OpType::OpFDiv>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentModulo, ResolveBinaryOperator<OpType::OpFMod>);

    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Add, ResolveBinaryOperator<spv::OpFAdd>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Subtract, ResolveBinaryOperator<OpType::OpFSub>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Multiply, ResolveBinaryOperator<OpType::OpFMul>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Divide, ResolveBinaryOperator<OpType::OpFDiv>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Modulo, ResolveBinaryOperator<OpType::OpFMod>);

    opResolvers.RegisterUnaryOpResolver(zilchType, Zilch::Grammar::Subtract, ResolveUnaryOperator<OpType::OpFNegate>);

    mathTypeResolver.RegisterFunctionResolver(GetStaticFunction(mathType, "FMod", zilchTypeName, zilchTypeName), ResolveFMod);
  }
  ...
  for(size_t i = 1; i < types.mRealVectorTypes.Size(); ++i)
  {
    ...
    opResolvers.RegisterBinaryOpResolver(zilchType, realType, Zilch::Grammar::Multiply, ResolveBinaryOperator<spv::OpVectorTimesScalar>);
    opResolvers.RegisterBinaryOpResolver(zilchType, realType, Zilch::Grammar::AssignmentMultiply, ResolveBinaryOperator<OpType::OpVectorTimesScalar>);
    opResolvers.RegisterBinaryOpResolver(zilchType, realType, Zilch::Grammar::Divide, ResolveVectorOpSplatScalar<OpType::OpFDiv>);
    opResolvers.RegisterBinaryOpResolver(zilchType, realType, Zilch::Grammar::AssignmentDivide, ResolveVectorOpSplatScalar<OpType::OpFDiv>);
  }
  ...
  for(size_t i = 0; i < types.mIntegerVectorTypes.Size(); ++i)
  {
    ...
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentSubtract, ResolveBinaryOperator<OpType::OpISub>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentMultiply, ResolveBinaryOperator<OpType::OpIMul>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentDivide, ResolveBinaryOperator<OpType::OpSDiv>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::AssignmentModulo, ResolveBinaryOperator<OpType::OpSMod>);

    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Add, ResolveBinaryOperator<spv::OpIAdd>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Subtract, ResolveBinaryOperator<OpType::OpISub>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Multiply, ResolveBinaryOperator<OpType::OpIMul>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Divide, ResolveBinaryOperator<OpType::OpSDiv>);
    opResolvers.RegisterBinaryOpResolver(zilchType, zilchType, Zilch::Grammar::Modulo, ResolveBinaryOperator<OpType::OpSMod>);
  ...
  for(size_t i = 1; i < types.mIntegerVectorTypes.Size(); ++i)
  {
    ...
    //translator->mBinaryOpInstructions[BinaryOpTypeId(vectorScalarTypePair, Zilch::Grammar::Multiply)] = OpType::OpVectorTimesScalar;
  ...
  for(size_t y = 2; y <= 4; ++y)
  {
    for(size_t x = 2; x <= 4; ++x)
    {
      ...
      // Matrix times vector
      mathTypeResolver.RegisterFunctionResolver(GetStaticFunction(mathType, "Multiply", zilchTypeName, vectorType->ToString()), ResolveMatrixTimesVector);
