...
{
public:
  StackAllocator() : mStack(nullptr)
  {
  }

  StackAllocator(Memory::Stack* manager) : mStack(manager)
  {
  }

  MemPtr Allocate(size_t numberOfBytes)
  {
    return mStack->Allocate(numberOfBytes);
  };
  void Deallocate(MemPtr ptr, size_t numberOfBytes)
  {
    mStack->Deallocate(ptr, numberOfBytes);
  }
  Memory::Stack* mStack;
};
