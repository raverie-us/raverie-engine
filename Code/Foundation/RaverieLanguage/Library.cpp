...
namespace Raverie
{
RaverieDefineType(Library, builder, type)
{
}
                                   ...
                                   Array<BoundType*>& outOrderedTypes)
{
  ...
  RaverieForEach (const LibraryRef& library, libraries)
  {
    // Loop through all types created in this library
    RaverieForEach (Type* type, library->OwnedTypes)
    {
                                                         ...
                                                         const LibraryArray& fromLibraries)
{
  ...
  for (size_t i = 0; i < fromLibraries.Size(); ++i)
  {
    ...
    if (info != nullptr)
    {
      ...
      {
        RaverieForEach (Library* dependentLibrary, fromLibraries)
        {
...
LibraryBuilder::AddSendsEvent(BoundType* forType, StringParam name, BoundType* sentType, StringParam description)
{
  ...
  GuidType guid = RaverieTypeId(EventsClass)->Hash();
...
String LibraryBuilder::FixIdentifier(StringParam ident, TokenCheck::Flags flags, char invalidCharacter)
{
  ...
  {
    Rune first = identifier.Front();
    Rune last = identifier.Back();
    ...
    if (readFirstLetter == false)
    {
      ...
      if (CharacterUtilities::IsAlpha(r))
      {
        ...
        if (isUpper && expectLowerIdentifier)
        {
          ...
          ErrorIf(asserts, "The first letter '%c' must be lowercase", r.value);
          builder.Append(UTF8::ToLower(r));
          readFirstLetter = true;
        }
        else if (isLower && expectUpperIdentifier)
        {
          // Append an uppercase version of the first letter
          ErrorIf(asserts, "The first letter '%c' must be uppercase", r.value);
      ...
      else
      {
        ...
        // character yet...
        ErrorIf(asserts, "The first character '%c' must be a letter", r.value);
      }
    }
    // If any part of the rest of the identifier is NOT alpha-numeric or
    // underscore
    else if (CharacterUtilities::IsAlphaNumeric(r) == false && r.value != '_')
    {
      ErrorIf(asserts,
...
CodeEntry::CodeEntry() : CodeUserData(nullptr)
{
}
...
void Library::GenerateDefinitionStubCode()
{
  ...
  RaverieForEach (BoundType* type, this->BoundTypes.Values())
  {
...
DocumentationModule* Module::BuildDocumentation()
{
  ...
  for (size_t i = 0; i < this->Size(); ++i)
  {
    ...
    ScriptDocumentationLibrary* docLibrary = new ScriptDocumentationLibrary();
...
void BuildBaseChain(StringBuilderExtended& builder, ScriptDocumentationLibrary* library, DocumentationType* type)
{
...
void Module::BuildDocumentationRst(StringParam directory)
{
  ...
  {
    ScriptDocumentationLibrary* library = docs->Libraries[i];
    ...
    for (size_t j = 0; j < library->TypesSorted.Size(); ++j)
    {
      ...
      Raverie::WriteToFile(rstFileName.c_str(), (const byte*)typeRst.c_str(), typeRst.SizeInBytes());
    }
  }
}
