...
namespace Raverie
{
RaverieSetup* RaverieSetup::Instance = nullptr;

RaverieSetup::RaverieSetup(SetupFlags::Type flags)
{
  Instance = this;
  ...
  // to RaverieStartup However, if the user defines 'RaverieDisableDocumentation',
  ...
  bool useRaverieErrorHandler = !(flags & SetupFlags::CustomAssertHandlerOrNoAsserts);
  if (useRaverieErrorHandler)
  {
    ErrorSignaler::SetErrorHandler(Raverie::Os::ErrorProcessHandler);
  }

  // Register the command handle managers we use
  RaverieRegisterUniqueHandleManager(HeapManager);
  RaverieRegisterUniqueHandleManager(StackManager);
  RaverieRegisterSharedHandleManager(PointerManager);
  RaverieRegisterSharedHandleManager(StringManager);
...
void ReconstructSingleton()
{
  ...
  // (should be reset)
  RaverieTodo("Make singletons into pointers so we don't have to do this silly stuff");
  T* singleton = &T::GetInstance();
  singleton->~T();
  new (singleton) T();
}

RaverieSetup::~RaverieSetup()
{
...
int RaverieMain(int argc, char* argv[])
{
  ...
  RaverieSetup setup(SetupFlags::None);

  // Get the arguments in a convenient to query form
  MainArguments arguments;
  RaverieParseMainArguments(argc, argv, arguments);

  // Hook up the standard write and read callbacks to the console (which allows
  // us to read from stdin and write to stdout)
  EventConnect(&ScriptConsole::Events, Events::ConsoleWrite, DefaultWriteText);
  EventConnect(&ScriptConsole::Events, Events::ConsoleRead, DefaultReadText);
  ...
  RaverieForEach (String& fileName, arguments.InputValues)
  {
  ...
  if (compileAndReport || compileOnly || run)
  {
    ...
    if (compileAndReport)
    {
      ...
      builder.Begin(JsonType::Object);
      {
        ...
        builder.Value(errorEvent.GetFormattedMessage(MessageFormat::Raverie));
      }
      builder.End();
    ...
    else
    {
      if (library == nullptr)
      {
        ...
        String errorMessage = errorEvent.GetFormattedMessage(MessageFormat::Raverie);
        printf("* %s\n", errorMessage.c_str());
        result = -1;
      }
      else if (run)
  ...
  else
  {
    ...
    if (autoCompleteCursor != nullptr && autoCompleteOrigin != nullptr)
    {
      ...
      Raverie::ToValue(*autoCompleteCursor, cursorPosition);
