...
namespace Events
{
RaverieDefineEvent(WebSocketAcceptedConnection);
RaverieDefineEvent(WebSocketReceivedData);
RaverieDefineEvent(WebSocketError);
RaverieDefineEvent(WebSocketDisconnected);
} // namespace Events

RaverieDefineStaticLibrary(WebSockets)
{
  RaverieInitializeType(WebSocketEvent);
}

RaverieDefineType(WebSocketEvent, builder, type)
{
}
...
WebSocketPacketType::Enum BlockingWebSocketConnection::ReceiveFullPacket(Status& status, String& dataOut)
{
  ...
  for (;;)
  {
    ...
    if (readHeader && this->ReadData.Size() >= payloadSize)
    {
      ...
      // copy our data into)
      Raverie::StringNode* node = String::AllocateNode(payloadSize);
    ...
    for (size_t i = 0; i < remainingHeaderData.Size(); ++i)
    {
      ...
      if (c == '\n')
      {
        ...
        else
        {
          ...
          StringRange foundDivider = range.FindFirstOf(Rune(':'));
