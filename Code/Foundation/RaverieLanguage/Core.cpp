...
namespace Raverie
{
RaverieDefineType(Enum, builder, type)
{
  ...
  type->ToStringFunction = &::Raverie::VirtualMachine::UnknownEnumerationToString;
}

RaverieDeclareExternalType(Members::Enum);

RaverieDefineRange(ParameterArray::range);
RaverieDefineRange(MemberRange<Member>);
RaverieDefineRange(MemberRange<Property>);
RaverieDefineRange(MemberRange<GetterSetter>);
RaverieDefineRange(MemberRange<Field>);
RaverieDefineRange(MemberRange<Function>);
...
BoundType* Core::InstantiatePropertyDelegate(LibraryBuilder& builder, StringParam baseName, StringParam fullyQualifiedName, const Array<Constant>& templateTypes, const void* userData)
{
  ...
  RaverieFullBindConstructor(builder, type, PropertyDelegateTemplate, nullptr);
...
{
  ScriptRune rune = call.Get<ScriptRune>(0);
...
void JoinTwoStrings(Call& call, ExceptionReport& report)
{
  ...
  String result = Raverie::String::Join(separator->mRange, first->mRange, second->mRange);
  call.Set(Call::Return, &result);
}
...
void JoinThreeStrings(Call& call, ExceptionReport& report)
{
  ...
  String result = Raverie::String::Join(separator->mRange, first->mRange, second->mRange, third->mRange);
  call.Set(Call::Return, &result);
}
...
void JoinFourStrings(Call& call, ExceptionReport& report)
{
  ...
  String result = Raverie::String::Join(separator->mRange, first->mRange, second->mRange, third->mRange, fourth->mRange);
  call.Set(Call::Return, &result);
}
...
public:
  ...
  {
    return this->Characters.Contains(rune.value);
  }
};
...
int FormatCSprintf(char* buffer, cstr format, int widthStar, int precisionStar, T value)
{
  ...
  if (buffer == nullptr)
  {
    ...
    if (widthStar >= 0 && precisionStar >= 0)
      return RaverieSPrintfCount(format, widthStar, precisionStar, value);
    else if (widthStar >= 0)
      return RaverieSPrintfCount(format, widthStar, value);
    else if (precisionStar >= 0)
      return RaverieSPrintfCount(format, widthStar, value);
    else
      return RaverieSPrintfCount(format, value);
  }
  else
  {
    // We have to determine the correct arguments to pass in based on whether
...
void StringFormatC(Call& call, ExceptionReport& report)
{
  ...
  // Create a stack array to hold all the arguments we read off the Raverie stack
  // The 'Any' type can hold any value from Raverie, including primitives and
  // classes
        ...
        else if (currentState == &FormatCStateParsedSpecifier)
        {
          ...
          for (; formatIt < formatEnd; ++formatIt)
          {
            ...
            if (*formatIt == '*')
            {
              ...
              if (Type::IsSame(argumentType, RaverieTypeId(Integer)) == false)
              {
          ...
          if (inputType == FormatCInputType::DoubleInteger)
          {
            ...
            if (Type::IsSame(argumentType, RaverieTypeId(Byte)))
              value = (DoubleInteger) * (Byte*)argumentData;
            else if (Type::IsSame(argumentType, RaverieTypeId(Integer)))
              value = (DoubleInteger) * (Integer*)argumentData;
            else if (Type::IsSame(argumentType, RaverieTypeId(DoubleInteger)))
              value = (DoubleInteger) * (DoubleInteger*)argumentData;
            else if (Type::IsSame(argumentType, RaverieTypeId(Real)))
              value = (DoubleInteger) * (Real*)argumentData;
            else if (Type::IsSame(argumentType, RaverieTypeId(DoubleReal)))
  ...
  // Return the string we built to the Raverie stack
...
Byte RaverieParseByte(StringRangeExtended range)
{
  Byte value;
...
Integer RaverieParseInteger(StringRangeExtended range)
{
  Integer value;
  Raverie::ToValue(range.mRange, value);
  return value;
}

DoubleInteger RaverieParseDoubleInteger(StringRangeExtended range)
{
  DoubleInteger value;
  ToValue(range.mRange, value);
  return value;
...
Real RaverieParseReal(StringRangeExtended range)
{
  Real value;
  ToValue(range.mRange, value);
  return value;
...
DoubleReal RaverieParseDoubleReal(StringRangeExtended range)
{
  DoubleReal value;
  ToValue(range.mRange, value);
  return value;
...
Byte RaverieBytePositiveMax()
{
  return Math::BytePositiveMax();
}

Byte RaverieBytePositiveValueClosestToZero()
{
  return 1;
}

Real RaverieRealPositiveMax()
{
  return Math::PositiveMax();
}

Real RaverieRealPositiveValueClosestToZero()
{
  return Math::PositiveMin();
}

Real RaverieRealNegativeValueClosestToZero()
{
  return -Math::PositiveMin();
}

Real RaverieRealNegativeMin()
{
  return -Math::PositiveMax();
}

DoubleReal RaverieDoubleRealPositiveMax()
{
  return Math::DoublePositiveMax();
}

DoubleReal RaverieDoubleRealPositiveValueClosestToZero()
{
  return Math::DoublePositiveMin();
}

DoubleReal RaverieDoubleRealNegativeValueClosestToZero()
{
  return -Math::DoublePositiveMin();
}

DoubleReal RaverieDoubleRealNegativeMin()
{
  return -Math::DoublePositiveMax();
}

Integer RaverieIntegerPositiveMax()
{
  return Math::IntegerPositiveMax();
}

Integer RaverieIntegerPositiveValueClosestToZero()
{
  return 1;
}

Integer RaverieIntegerNegativeValueClosestToZero()
{
  return -1;
}

Integer RaverieIntegerNegativeMin()
{
  return Math::IntegerNegativeMin();
}

DoubleInteger RaverieDoubleIntegerPositiveMax()
{
  return Math::DoubleIntegerPositiveMax();
}

DoubleInteger RaverieDoubleIntegerPositiveValueClosestToZero()
{
  return 1;
}

DoubleInteger RaverieDoubleIntegerNegativeValueClosestToZero()
{
  return -1;
}

DoubleInteger RaverieDoubleIntegerNegativeMin()
{
...
{
  BoundType* stringRangeType = RaverieTypeId(StringRangeExtended);
  BoundType* stringType = RaverieTypeId(String);
  BoundType* runeType = RaverieTypeId(ScriptRune);
  BoundType* runeIteratorType = RaverieTypeId(RuneIterator);
  BoundType* splitRangeType = RaverieTypeId(StringSplitRangeExtended);

  BoundType* doubleIntegerType = RaverieTypeId(DoubleInteger);
  BoundType* doubleRealType = RaverieTypeId(DoubleReal);
  BoundType* byteType = RaverieTypeId(Byte);
  BoundType* booleanType = RaverieTypeId(Boolean);
  BoundType* boolean2Type = RaverieTypeId(Boolean2);
  BoundType* boolean3Type = RaverieTypeId(Boolean3);
  BoundType* boolean4Type = RaverieTypeId(Boolean4);
  BoundType* integerType = RaverieTypeId(Integer);
  BoundType* integer2Type = RaverieTypeId(Integer2);
  BoundType* integer3Type = RaverieTypeId(Integer3);
  BoundType* integer4Type = RaverieTypeId(Integer4);
  BoundType* realType = RaverieTypeId(Real);
  BoundType* real2Type = RaverieTypeId(Real2);
  BoundType* real3Type = RaverieTypeId(Real3);
  BoundType* real4Type = RaverieTypeId(Real4);
  BoundType* quaternionType = RaverieTypeId(Quaternion);
  BoundType* real2x2Type = RaverieTypeId(Real2x2);
  BoundType* real3x3Type = RaverieTypeId(Real3x3);
  BoundType* real4x4Type = RaverieTypeId(Real4x4);
...
{
  RaverieInitializeExternalType(Boolean);
  RaverieInitializeExternalType(Boolean2);
  RaverieInitializeExternalType(Boolean3);
  RaverieInitializeExternalType(Boolean4);
  RaverieInitializeExternalType(Byte);
  RaverieInitializeExternalType(Integer);
  RaverieInitializeExternalType(Integer2);
  RaverieInitializeExternalType(Integer3);
  RaverieInitializeExternalType(Integer4);
  RaverieInitializeExternalType(Real);
  RaverieInitializeExternalType(Real2);
  RaverieInitializeExternalType(Real3);
  RaverieInitializeExternalType(Real4);
  RaverieInitializeExternalType(Quaternion);
  RaverieInitializeExternalType(Real2x2);
  RaverieInitializeExternalType(Real3x3);
  RaverieInitializeExternalType(Real4x4);
  RaverieInitializeExternalType(String);
  RaverieInitializeExternalType(DoubleReal);
  RaverieInitializeExternalType(DoubleInteger);
  RaverieInitializeType(Enum);
  ...
  RaverieInitializeTypeAs(Raverie::Library, "Library");
  RaverieInitializeType(ReflectionObject);
  RaverieInitializeType(Type);
  RaverieInitializeType(AnyType);
  RaverieInitializeType(IndirectionType);
  RaverieInitializeType(DelegateType);
  RaverieInitializeType(BoundType);
  RaverieInitializeType(DelegateParameter);

  RaverieInitializeExternalTypeAs(Members::Enum, "Members");

  RaverieInitializeTypeAs(ScriptConsole, "Console");
  RaverieInitializeType(Exception);
  RaverieInitializeType(EventHandler);
  RaverieInitializeTypeAs(EventsClass, "Events");
  RaverieInitializeType(EventData);
  RaverieInitializeType(ErrorEvent);
  RaverieInitializeType(ExceptionEvent);
  RaverieInitializeType(ConsoleEvent);
  RaverieInitializeType(DebuggerEvent);
  RaverieInitializeType(DebuggerTextEvent);
  RaverieInitializeType(MemoryLeakEvent);
  RaverieInitializeType(OpcodeEvent);
  RaverieInitializeType(FatalErrorEvent);
  RaverieInitializeType(ParseEvent);
  RaverieInitializeType(ExecutableState);
  RaverieInitializeType(Member);
  RaverieInitializeType(Function);
  RaverieInitializeType(Property);
  RaverieInitializeType(GetterSetter);
  RaverieInitializeType(Field);
  RaverieInitializeType(Variable);
  RaverieInitializeType(Random);
  RaverieInitializeTypeAs(StringBuilderExtended, "StringBuilder");
  RaverieInitializeTypeAs(ScriptRune, "Rune");
  RaverieInitializeType(RuneIterator);
  RaverieInitializeTypeAs(StringRangeExtended, "StringRange");
  RaverieInitializeTypeAs(StringSplitRangeExtended, "StringSplitRange");
  RaverieInitializeType(MultiPrimitive);
  RaverieInitializeType(Wrapper);
  RaverieInitializeTypeAs(ColorClass, "Color");
  RaverieInitializeTypeAs(ColorsClass, "Colors");

  RaverieInitializeType(ArrayClass<Handle>);
  RaverieInitializeType(ArrayClass<Delegate>);
  RaverieInitializeType(ArrayClass<Boolean>);
  RaverieInitializeType(ArrayClass<Boolean2>);
  RaverieInitializeType(ArrayClass<Boolean3>);
  RaverieInitializeType(ArrayClass<Boolean4>);
  RaverieInitializeType(ArrayClass<Byte>);
  RaverieInitializeType(ArrayClass<Integer>);
  RaverieInitializeType(ArrayClass<Integer2>);
  RaverieInitializeType(ArrayClass<Integer3>);
  RaverieInitializeType(ArrayClass<Integer4>);
  RaverieInitializeType(ArrayClass<Real>);
  RaverieInitializeType(ArrayClass<Real2>);
  RaverieInitializeType(ArrayClass<Real3>);
  RaverieInitializeType(ArrayClass<Real4>);
  RaverieInitializeType(ArrayClass<Quaternion>);
  RaverieInitializeType(ArrayClass<DoubleInteger>);
  RaverieInitializeType(ArrayClass<DoubleReal>);
  RaverieInitializeType(ArrayClass<Any>);
  RaverieInitializeType(ArrayClass<HandleOfString>);
  ...
  RaverieInitializeRangeAs(ParameterArray::range, "DelegateParameterRange");
  RaverieInitializeRangeAs(MemberRange<Member>, "MemberRange");
  RaverieInitializeRangeAs(MemberRange<Property>, "PropertyRange");
  RaverieInitializeRangeAs(MemberRange<GetterSetter>, "GetterSetterRange");
  RaverieInitializeRangeAs(MemberRange<Field>, "FieldRange");
  RaverieInitializeRangeAs(MemberRange<Function>, "FunctionRange");
...
RaverieDefineType(Exception, builder, type)
{
  // Store this type on the core library
  Core& core = Core::GetInstance();
  core.ExceptionType = type;

  RaverieFullBindDestructor(builder, type, Exception);
  RaverieFullBindConstructor(builder, type, Exception, nullptr);
  RaverieFullBindConstructor(builder, type, Exception, nullptr, StringParam);

  RaverieFullBindField(builder, type, &Exception::Message, "Message", PropertyBinding::GetSet);
  // RaverieFullBindMethod(GetFormattedMessage);
}
...
String Exception::GetFormattedMessage(MessageFormat::Enum format)
{
  ...
  StringBuilder builder;
  builder.Append("********************** Raverie Exception **********************\n");
