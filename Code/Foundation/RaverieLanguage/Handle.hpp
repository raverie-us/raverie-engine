...
// Enable this to add extra debugging support (for Raverie internal use, such as
// working on the compiler) Warning: This will make Raverie run significantly
// slower
//#define RaverieHandleDebug
...
// The user has to be able to fit their entire handle inside of the user data on
// the handle If the user cannot fit the data inside the handle, then they must
// create another object that acts as a reference to their object
...
enum Enum
{
  ...
  // In this mode, we don't do reference counting (this is a useful optimization
  // for handles!) This flag is also set when we're deleting a handle so that it
  // can be passed into a destructor without increasing or decreasing reference
  // counts (this is safe because the handle is already going to be deleted)
  NoReferenceCounting = 1,

  // This flag is set only if RaverieHandleDebug is enabled
  // This is used to track which handles need be removed from the intrusive
  // linked list of all handles If a handle was purely initialized via memory
  // setting to zero, we ignore it
...
public:
  ...
  // If the Any is keeping a value type, then this will use the PointerManager
  // to point at the type (the Any MUST be kept alive)
  ...
  bool operator==(Raverie::NullPointerType) const;
  ...
  // It is possible for the destructor to throw an exception (though not highly
  // recommended)
  bool Delete();

  // Checks to see if this handle is reference counted, or if it's just a
  // standard handle
  inline bool IsReferenceCounted();

  // If the handle is looking at a reference type, this will return the bound
  // type Otherwise if the handle is looking at a value type, then this must be
  // an indirect type Returns null if the handle stores nothing
  Type* GetBoundOrIndirectType() const;

  // Use this only in the case where we are allocating a handle for an object,
  // but we need to return that object via binding (not a handle) This situation
  // would prematurely decrement the only reference count and delete the object,
  // however we know that binding will implicitly add a reference count
  // immediately after This clears the handle so that its destruction will not
  // delete the object, and it also releases the only reference count so the
  // object does not leak This is only valid when the reference count is 1
...
private:
  ...
  // Decrement the reference count (only if we're counted), and destruct/delete
...
public:
  ...
  // actually Contains the type (or the v-table) rather than it being on the
  // object itself. This allows us to invoke interface functions on a stack
  // object via a handle, rather than by boxing the value type
  BoundType* StoredType;
  // DebugPlaceholder<BoundType*> Type;

  // The manager controls the lifetime of the handle, and how we interface with
  // it
  ...
  union
  {
    ...
    MaxAlignmentType DataAligned[RaverieAlignCount(HandleUserDataSize)];

    // These are only provided for convenience (rather than casting Data into a
    // structure)
    void* HandlePointer;
    s32 HandleS32;
    s64 HandleS64;
