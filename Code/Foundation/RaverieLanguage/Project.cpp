...
namespace Events
{
RaverieDefineEvent(PreParser);
RaverieDefineEvent(TypeParsed);
RaverieDefineEvent(PostSyntaxer);
} // namespace Events

RaverieDefineType(ParseEvent, builder, type)
{
}
...
CompletionEntry::CompletionEntry() : CodeUserData(nullptr), Hidden(false)
{
}
...
{
  RaverieErrorIfNotStarted(Project);
}
...
String Project::ReadTextFile(Status& status, StringParam fileName)
{
  ...
  File file;
  file.Open(fileName, Raverie::FileMode::Read, Raverie::FileAccessPattern::Random, Raverie::FileShare::Read, &status);
...
bool Project::Tokenize(Array<UserToken>& tokensOut, Array<UserToken>& commentsOut)
{
  ...
  ScriptTokenizer tokenizer(*this);
  ...
  for (size_t i = 0; i < comments.Size(); ++i)
  {
    ...
    for (size_t line = comment.Location.StartLine; line <= origin.MaxLine && line <= endLine; ++line)
    {
      ...
      if (node != nullptr && node->Location.PrimaryLine == line)
      {
        ...
        NodeChildren children;
        node->PopulateChildren(children);
        RaverieForEach (SyntaxNode** childPtr, children)
        {
...
CompletionOverload& Project::AddAutoCompleteOverload(AutoCompleteInfo& info, DelegateType* delegateType)
{
  ...
  if (Raverie::Type::IsSame(RaverieTypeId(void), delegateType->Return) == false)
  {
...
void Project::GetDefinitionInfo(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, CodeDefinition& resultOut)
{
  ...
  if (resultOut.ToolTip.Empty())
  {
    ...
    {
      String name = RaverieVirtualTypeId(resultOut.DefinedObject)->ToString();
...
void Project::GetDefinitionInfoInternal(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, CodeDefinition& resultOut)
{
  ...
  // function)
  Raverie::SetAndRecallOnDestruction<bool> changeTolerantMode(&this->TolerantMode, true);
  ...
  for (int i = (int)nodes.Size() - 1; i >= 0; --i)
  {
    ...
    if (node != nullptr)
    {
      ...
      BoundType* nodeType = RaverieVirtualTypeId(node);
      ...
      if (!comments.Empty())
      {
        resultOut.ToolTip = BuildString(comments, "\n", resultOut.ToolTip);
      }
      return;
    }
  }
...
void Project::GetAutoCompleteInfo(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, AutoCompleteInfo& resultOut)
{
  ...
  Raverie::BoundType* boundType = Raverie::Type::DynamicCast<Raverie::BoundType*>(resultOut.NearestType);
  ...
  if (boundType == nullptr)
  {
    ...
    // types...
    Raverie::IndirectionType* indirectType = Raverie::Type::DynamicCast<Raverie::IndirectionType*>(resultOut.NearestType);
...
void Project::GetAutoCompleteInfoInternal(Module& dependencies, size_t cursorPosition, StringParam cursorOrigin, AutoCompleteInfo& resultOut)
{
  ...
  for (int i = (int)nodes.Size() - 1; i >= 0; --i)
  {
    ...
    // is NOT a literal, hence the RaverieGetDerivedType == check).
    resultOut.IsLiteral = (expression->RaverieGetDerivedType() == RaverieTypeId(ValueNode));
