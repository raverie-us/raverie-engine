...
namespace Raverie
{
RaverieDefineExternalBaseType(Members::Enum, TypeCopyMode::ValueType, builder, type)
{
  RaverieFullBindEnum(builder, type, SpecialType::Flags);
  RaverieFullBindEnumValue(builder, type, Members::Instance, "Instance");
  RaverieFullBindEnumValue(builder, type, Members::Static, "Static");
  RaverieFullBindEnumValue(builder, type, Members::Inherited, "Inherited");
  RaverieFullBindEnumValue(builder, type, Members::Extension, "Extension");
  RaverieFullBindEnumValue(builder, type, Members::InstanceStatic, "InstanceStatic");
  RaverieFullBindEnumValue(builder, type, Members::InheritedInstanceStatic, "InheritedInstanceStatic");
  RaverieFullBindEnumValue(builder, type, Members::InheritedInstance, "InheritedInstance");
  RaverieFullBindEnumValue(builder, type, Members::InheritedStatic, "InheritedStatic");
  RaverieFullBindEnumValue(builder, type, Members::All, "All");
}

RaverieDefineType(Type, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
  RaverieFullBindMethod(builder, type, &Type::IsA, RaverieNoOverload, "IsA", "baseType");
  RaverieFullBindMethod(builder, type, &Type::IsCastableTo, RaverieNoOverload, "IsCastableTo", "toType");
  RaverieFullBindMethod(builder, type, &Type::IsRawCastableTo, RaverieNoOverload, "IsRawCastableTo", "toType");
...
RaverieDefineType(AnyType, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
}

RaverieDefineType(IndirectionType, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
  RaverieFullBindField(builder, type, &IndirectionType::ReferencedType, "ReferencedType", PropertyBinding::Get);
}

RaverieDefineType(DelegateType, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
...
RaverieDefineType(DelegateParameter, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
  RaverieFullBindField(builder, type, &DelegateParameter::Name, "Name", PropertyBinding::Get);
  RaverieFullBindField(builder, type, &DelegateParameter::ParameterType, "Type", PropertyBinding::Get);
}

RaverieDefineType(BoundType, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
...
bool Type::IsRawSame(Type* a, Type* b)
{
  ...
  Type* aType = RaverieVirtualTypeId(a);
  Type* bType = RaverieVirtualTypeId(b);
...
bool Type::IndirectionIsA(IndirectionType* type, IndirectionType* base)
{
  ...
  return BoundIsA(type->ReferencedType, base->ReferencedType);
}
...
bool Type::IsA(Type* base)
{
  return Type::GenericIsA(this, base);
}
bool Type::IsHandle()
{
  return Type::IsHandleType(this);
}
bool Type::IsValue()
{
  return Type::IsValueType(this);
}
bool Type::IsDelegate()
{
  return Type::IsDelegateType(this);
}
bool Type::IsEnum()
{
  return Type::IsEnumType(this);
}
bool Type::IsFlags()
{
  return Type::IsFlagsType(this);
}
bool Type::IsEnumOrFlags()
{
  return Type::IsEnumOrFlagsType(this);
}
bool Type::IsAny()
{
  return Type::IsAnyType(this);
}

IndirectionType::IndirectionType() : ReferencedType(nullptr)
{
}
...
Type* IndirectionType::GenericGetSameVirtualTypeExceptAny(const byte* value) const
{
  ...
  Error("The virtual type had a BoundType whose 'IndirectType' field "
...
BoundType::BoundType(BoundTypeAssertFn nativeBindingAssert) :
    ...
    Size(0),
    HandleManager(RaverieManagerId(HeapManager)),
...
{
  // When a BoundType is created via native binding in C++, e.g. RaverieTypeId(X),
...
BoundType::~BoundType()
{
  ...
  RaverieForEach (SendsEvent* sendsEvent, this->SendsEvents)
  {
    delete sendsEvent;
  }
}
...
Function* BoundType::GetConstructor(DelegateType* type, bool inherited)
{
  ...
  RaverieForEach (Function* constructor, *constructors)
  {
