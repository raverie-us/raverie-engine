...
DebugInstruction::DebugInstruction() : IsCopy(false)
{
}
...
void GenerateDebugInstructionInfo(Array<DebugInstruction>& debugOut)
{
  ...
  // ToHandle
  {
    ...
    RaverieOperand(info.ReadOperands, ToHandleOpcode, ToHandle, DebugPrimitive::Memory, false);
    RaverieOperand(info.WriteOperands, ToHandleOpcode, SaveLocal, DebugPrimitive::Memory, true);
  }

  // CreateStaticDelegate
  {
    DebugInstruction& info = debugOut[Instruction::CreateStaticDelegate];
    info.FunctionPointers.PushBack(RaverieOffsetOf(CreateStaticDelegateOpcode, BoundFunction));
    RaverieOperand(info.WriteOperands, CreateStaticDelegateOpcode, SaveLocal, DebugPrimitive::Delegate, true);
  }

  // CreateInstanceDelegate
  {
    DebugInstruction& info = debugOut[Instruction::CreateInstanceDelegate];
    info.FunctionPointers.PushBack(RaverieOffsetOf(CreateInstanceDelegateOpcode, BoundFunction));
    RaverieOperand(info.ReadOperands, CreateInstanceDelegateOpcode, ThisHandle, DebugPrimitive::Handle, false);
    RaverieOperand(info.WriteOperands, CreateInstanceDelegateOpcode, SaveLocal, DebugPrimitive::Delegate, true);
  }
  ...
  // IfFalseRelativeGoTo / IfTrueRelativeGoTo
  {
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      RaverieOperand(info.ReadOperands, IfOpcode, Condition, DebugPrimitive::Boolean, false);
      info.OpcodeOffsets.PushBack(RaverieOffsetOf(IfOpcode, JumpOffset));
    }
  }
  ...
  // RelativeGoTo
  {
    ...
    info.OpcodeOffsets.PushBack(RaverieOffsetOf(RelativeJumpOpcode, JumpOffset));
  }

  // PrepForFunctionCall
  {
    DebugInstruction& info = debugOut[Instruction::PrepForFunctionCall];
    RaverieOperand(info.ReadOperands, PrepForFunctionCallOpcode, Delegate, DebugPrimitive::Delegate, false);
    info.OpcodeOffsets.PushBack(RaverieOffsetOf(PrepForFunctionCallOpcode, JumpOffsetIfStatic));
  }

  // NewObject
  {
    DebugInstruction& info = debugOut[Instruction::NewObject];
    info.TypePointers.PushBack(RaverieOffsetOf(CreateTypeOpcode, CreatedType));
    RaverieOperand(info.WriteOperands, CreateTypeOpcode, SaveHandleLocal, DebugPrimitive::Handle, true);
  }

  // LocalObject
  {
    DebugInstruction& info = debugOut[Instruction::LocalObject];
    info.TypePointers.PushBack(RaverieOffsetOf(CreateLocalTypeOpcode, CreatedType));
    RaverieOperand(info.WriteOperands, CreateLocalTypeOpcode, SaveHandleLocal, DebugPrimitive::Handle, true);
    RaverieOperand(info.WriteOperands, CreateLocalTypeOpcode, StackLocal, DebugPrimitive::Memory, true);
  }

  // DeleteObject
  {
    DebugInstruction& info = debugOut[Instruction::DeleteObject];
    RaverieOperand(info.WriteOperands, DeleteObjectOpcode, Object, DebugPrimitive::Handle, false);
  }

  // [UnaryRValueOpcode]
  {
    Instruction::Enum instructions[] = {Instruction::NegateInteger,
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, UnaryRValueOpcode, SingleOperand, primitive, false);
      RaverieOperand(info.WriteOperands, UnaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [UnaryLValueOpcode]
  {
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.WriteOperands, UnaryLValueOpcode, SingleOperand, primitive, false);
    }
  }

  // [BinaryRValueOpcode] (Result + Operands all the same type)
  {
    Instruction::Enum instructions[] = {Instruction::BitshiftLeftInteger,
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Right, primitive, false);
      RaverieOperand(info.WriteOperands, BinaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [BinaryRValueOpcode] (Operands all the same type, Result is a Boolean)
  {
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Right, primitive, false);
      RaverieOperand(info.WriteOperands, BinaryRValueOpcode, Output, DebugPrimitive::Boolean, true);
    }
  }

  // [BinaryRValueOpcode] (Scalar + Vector operations)
  {
    Instruction::Enum instructions[] = {Instruction::ScalarMultiplyReal2,
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Left, primitive, false);
      RaverieOperand(info.ReadOperands, BinaryRValueOpcode, Right, DebugPrimitive::Real, false);
      RaverieOperand(info.WriteOperands, BinaryRValueOpcode, Output, primitive, true);
    }
  }
  ...
  // [BinaryLValueOpcode] (Operands all the same type)
  {
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, BinaryLValueOpcode, Right, primitive, false);
      RaverieOperand(info.WriteOperands, BinaryLValueOpcode, Output, primitive, false);
    }
  }
  ...
  // [BinaryLValueOpcode] (Scalar + Vector assignment operations)
  {
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugPrimitive::Enum primitive = types[i];
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, BinaryLValueOpcode, Right, DebugPrimitive::Real, false);
      RaverieOperand(info.WriteOperands, BinaryLValueOpcode, Output, primitive, false);
    }
  }
  ...
  // [ConversionOpcode]
  {
    Instruction::Enum instructions[] = {Instruction::ConvertIntegerToReal,
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      RaverieOperand(info.ReadOperands, ConversionOpcode, ToConvert, fromPrimitive, false);
      RaverieOperand(info.WriteOperands, ConversionOpcode, Output, toPrimitive, true);
    }
  }
  ...
  // [CopyOpcode]
  {
    Instruction::Enum instructions[] = {Instruction::CopyInteger,
    ...
    for (size_t i = 0; i < RaverieCArrayCount(instructions); ++i)
    {
      ...
      DebugInstruction& info = debugOut[instruction];
      info.IsCopy = true;
      RaverieOperand(info.ReadOperands, CopyOpcode, Source, primitive, false);
      RaverieOperand(info.WriteOperands, CopyOpcode, Destination, primitive, false);
      info.Sizes.PushBack(RaverieOffsetOf(CopyOpcode, Size));
      info.Options.PushBack(RaverieOffsetOf(CopyOpcode, Mode));
    }
  }
}
} // namespace Raverie
