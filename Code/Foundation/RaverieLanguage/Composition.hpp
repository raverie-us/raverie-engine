...
// Allows the user to attach any type to a Raverie primitive
// The primitive must be default constructable
class Composition : public IRaverieObject
{
public:
  Composition();
  ~Composition();
  ...
  {
    Type* type = RaverieVirtualTypeId(component);
  ...
  T* Has()
  {
    ...
    Handle* handle = this->Components.FindPointer(RaverieTypeId(T));
