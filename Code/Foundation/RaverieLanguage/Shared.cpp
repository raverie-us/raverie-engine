...
#define RaverieIntegralOperators(WithType)                                                                               \
  ...
  RaverieScalarOperators(Byte);
  RaverieIntegralOperators(Integer);
  RaverieScalarOperators(Integer);
  RaverieVectorOperators(Integer2, Integer, Boolean2);
  RaverieVectorOperators(Integer3, Integer, Boolean3);
  RaverieVectorOperators(Integer4, Integer, Boolean4);
  RaverieIntegralOperators(Integer2);
  RaverieIntegralOperators(Integer3);
  RaverieIntegralOperators(Integer4);
  RaverieScalarOperators(Real);
  RaverieVectorOperators(Real2, Real, Boolean2);
  RaverieVectorOperators(Real3, Real, Boolean3);
  RaverieVectorOperators(Real4, Real, Boolean4);
  RaverieScalarOperators(DoubleReal);
  RaverieIntegralOperators(DoubleInteger);
  RaverieScalarOperators(DoubleInteger);

  RaverieEqualityOperators(Boolean, Boolean);
  // Handle, Delegate, and Value equality operators are handled specially above

  RaverieCopyOperators(Boolean);
...
BinaryOperator Shared::GetBinaryOperator(Type* lhs, Type* rhs, Grammar::Enum oper, bool allowRecursiveLookup)
{
  ...
  Type* integerType = RaverieTypeId(Integer);
...
CastOperator Shared::GetCastOperator(Type* from, Type* to)
{
  ...
  // handle...
  if (from == RaverieTypeId(Handle) && Type::IsHandleType(to))
    return this->FromAnyHandleCast;

  // If we're casting from an Integer to an enum...
  if (Type::IsSame(from, RaverieTypeId(Integer)) && Type::IsEnumOrFlagsType(to))
    return this->IntegerEnumCast;

  // If we're casting from an enum to an Integer... (can be implicit)
  if (Type::IsEnumOrFlagsType(from) && Type::IsSame(to, RaverieTypeId(Integer)))
  ...
  if (Type::IsSame(RaverieTypeId(NullPointerType), from) && Type::IsDelegateType(to))
    return this->NullToDelegate;

  // If we're converting from null to any other handle type...
  if (Type::IsSame(RaverieTypeId(NullPointerType), from) && Type::IsHandleType(to))
    return this->RawImplicitCast;
