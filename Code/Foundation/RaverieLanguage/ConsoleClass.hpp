...
RaverieDeclareEvent(ConsoleWrite, ConsoleEvent);

// Sent when anyone attempts to read from the console
RaverieDeclareEvent(ConsoleRead, ConsoleEvent);
} // namespace Events
...
{
public:
  RaverieDeclareType(ConsoleEvent, TypeCopyMode::ReferenceType);
...
{
public:
  RaverieDeclareType(ScriptConsole, TypeCopyMode::ReferenceType);

  // Write to the console (not bound to Raverie)
