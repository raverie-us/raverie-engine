...
{
  RaverieErrorIfNotStarted(Syntaxer);
...
void Syntaxer::ApplyToTree(SyntaxTree& syntaxTree, LibraryBuilder& builder, Project& project, const Module& dependencies)
{
  ...
  RaverieForEach (BoundType* type, this->Builder->BuiltLibrary->TypesInDependencyOrder)
  {
        ...
        if (this->Builder->FindBoundType(fullyQualifiedTemplateName) == nullptr && this->ExternalBoundTypes.FindValue(fullyQualifiedTemplateName, nullptr) == nullptr)
        {
          ...
          for (size_t i = 0; i < dataType->TemplateArguments.Size(); ++i)
          {
            ...
            if (templateTypeArgument != nullptr)
            {
              ...
              if (resolvedType == RaverieTypeId(void))
                return;
          ...
          if (ClassNode* classNode = this->InternalBoundTemplates.FindValue(baseName, nullptr))
          {
            ...
            RaverieTodo("Make sure we traverse the added class");
          ...
          else
          {
            ...
            {
              RaverieTodo("This error message can be made a lot better with context");
...
void Syntaxer::PreventNameHiddenBaseMembers(Member* member)
{
  ...
  bool isFunction = (RaverieVirtualTypeId(member) == RaverieTypeId(Function));
...
void Syntaxer::CollectEnum(EnumNode*& node, ClassContext* context)
{
  ...
  type->BaseType = RaverieTypeId(Enum);
...
void Syntaxer::CollectFunction(FunctionNode*& node, TypingContext* context)
{
  ...
  {
    RaverieTodo("Handle extension function overload collisions");
...
void Syntaxer::DecorateStringInterpolant(StringInterpolantNode*& node, TypingContext* context)
{
  ...
  node->ResultType = RaverieTypeId(String);
...
void Syntaxer::DecorateTypeId(TypeIdNode*& node, TypingContext* context)
{
  ...
  if (Type::IsAnyType(node->CompileTimeType) && node->Value != nullptr)
    node->ResultType = RaverieTypeId(Type);
  else
    node->ResultType = RaverieVirtualTypeId(node->CompileTimeType);
}
...
void Syntaxer::DecorateMemberId(MemberIdNode*& node, TypingContext* context)
{
  ...
  if (node->Member != nullptr)
  {
    ...
    if (member != nullptr)
    {
      node->ResultType = RaverieVirtualTypeId(member);
    }
    else
    {
...
void Syntaxer::DecorateCheckFunctionCall(FunctionCallNode*& node, TypingContext* context)
{
  ...
  else if (InitializerNode* initializerNode = Type::DynamicCast<InitializerNode*>(node->LeftOperand))
  {
    ...
    else
    {
      ...
      RaverieTodo("Handle constructor calling");
...
void Syntaxer::ResolveMember(MemberAccessNode*& node, TypingContext* context)
{
  ...
  if (anyType != nullptr)
  {
    ...
    // for chaining)
    node->ResultType = RaverieTypeId(Any);
    this->ErrorAt(node,
                  ErrorCode::GenericError,
...
void Syntaxer::PrecomputeStringInterpolantNode(StringInterpolantNode*& node, TypingContext* context)
{
  ...
  // string
  node->PrecomputedResultType = RaverieTypeId(String);
}
