...
{
public:
  RaverieDeclareType(Type, TypeCopyMode::ReferenceType);
  ...
  static Derived DirectDynamicCast(Base baseClassPointer)
  {
    ...
    // If the cast is safe...
    if (baseClassPointer->RaverieGetDerivedType() == RaverieTypeId(Derived))
    {
...
{
public:
  RaverieDeclareType(IndirectionType, TypeCopyMode::ReferenceType);
...
{
public:
  RaverieDeclareType(AnyType, TypeCopyMode::ReferenceType);
...
public:
      ...
      IteratingType(type), DerivedType(type), Name(name), DeclaredType(declaredType), CurrentMember(nullptr)
  {
    ...
    // leave it at null
    if (memberKind == nullptr && RaverieTypeId(MemberType) != RaverieTypeId(Member))
      memberKind = RaverieTypeId(MemberType);
...
{
public:
  RaverieDeclareType(BoundType, TypeCopyMode::ReferenceType);
  ...
  // Binding for reflection (internally bound in Raverie)
...
{
public:
  RaverieDeclareType(DelegateParameter, TypeCopyMode::ReferenceType);
...
{
public:
  RaverieDeclareType(DelegateType, TypeCopyMode::ReferenceType);
...
BoundType*& TypeBinding::StaticTypeId<T>::GetType()
{
  ...
  // This ensures that RaverieTypeId will always result in a BoundType, even if it
  // is not initialized Note: This does however result BoundTypes being
  // unintentionally created just by using RaverieTypeId on a type that is not
  ...
  if (instance->IsInitialized() == false)
  {
    ErrorIf(NativeBindingList::IsBuildingLibrary() == false,
            "You can only get a type with RaverieTypeId if the type is initialized, "
...
Derived Type::DynamicCast(Base baseClassPointer)
{
  ...
  BoundType* fromType = baseClassPointer->RaverieGetDerivedType();
  fromType->IsInitializedAssert();

  BoundType* toType = RaverieTypeId(Derived);
...
void MemberRange<MemberType>::PopFront()
{
  ...
  RaverieLoop
  {
    ...
    while (this->MembersRange.Empty() == false)
    {
      ...
      BoundType* derivedType = RaverieVirtualTypeId(member);
