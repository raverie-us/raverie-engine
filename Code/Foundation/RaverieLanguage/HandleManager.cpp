...
HandleManagers::HandleManagers() : UniqueCounter(0), Locked(false)
{
}
...
HandleManager* HandleManagers::GetManager(HandleManagerId id, ExecutableState* state)
{
  ...
  if (state == nullptr)
  {
    ...
    return this->Shared.FindValue(RaverieManagerId(PointerManager), nullptr);
  }
...
HandleManager::HandleManager(ExecutableState* state) : State(state)
{
}
...
HeapManager::HeapManager(ExecutableState* state) : HandleManager(state)
{
...
void HeapManager::Allocate(BoundType* type, Handle& handleToInitialize, size_t customFlags)
{
  ...
  byte* memory = (byte*)Raverie::zAllocate(fullSize);

  // If the memory failed to allocate, early out
  if (memory == nullptr)
  {
    Error("Failed memory allocation within a Raverie heap object");
...
void HeapManager::Delete(const Handle& handle)
{
  ...
  Raverie::zDeallocate(data.Header);
}
...
StackManager::StackManager(ExecutableState* state) : HandleManager(state)
{
}
...
{
  handleToInitialize.Flags |= HandleFlags::NoReferenceCounting;
  handleToInitialize.HandlePointer = Raverie::zAllocate(type->Size);
}

void PointerManager::Delete(const Handle& handle)
{
  Raverie::zDeallocate(handle.HandlePointer);
}
...
StringManager::StringManager(ExecutableState* state) : HandleManager(state)
{
}
...
void StringManager::AddReference(const Handle& handle)
{
  ...
  Raverie::StringNode* node = str.GetNode();
  node->addRef();
}
