  ...
  for (size_t typeIndex = 0; typeIndex < Core::MaxMatrixElementTypes; ++typeIndex)
  {
    for (size_t sizeY = Core::MinMatrixComponents; sizeY <= Core::MaxMatrixComponents; ++sizeY)
    {
      for (size_t sizeX = Core::MinMatrixComponents; sizeX <= Core::MaxMatrixComponents; ++sizeX)
      {
        ...
        // BoundType created elsewhere for Real2x2/Real3x3/Real4x4 for ZeroRaverie
        // binding
        BoundType* matrixType = nullptr;
        if (sizeX == 2 && sizeY == 2 && elementType == RaverieTypeId(Real))
          matrixType = RaverieTypeId(Real2x2);
        else if (sizeX == 3 && sizeY == 3 && elementType == RaverieTypeId(Real))
          matrixType = RaverieTypeId(Real3x3);
        else if (sizeX == 4 && sizeY == 4 && elementType == RaverieTypeId(Real))
          matrixType = RaverieTypeId(Real4x4);
        else
