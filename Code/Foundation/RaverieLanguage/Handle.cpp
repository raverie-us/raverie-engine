...
Handle::Handle()
{
  RaverieErrorIfNotStarted(Handle);
...
void Handle::ValidateAllHandles()
{
  // Loop forwards through the list and validate each handle
  {
    ...
    while (value != nullptr)
    {
      ErrorIf(visited.Contains(value), "Attempted to visit a link twice");
...
bool Handle::operator==(Raverie::NullPointerType) const
{
  return this->IsNull();
}
...
void Handle::ReleaseReference()
{
  ...
  if (this->Manager && this->IsReferenceCounted())
  {
    ...
    // deleted For example, new an object in Raverie, then create a temporary
