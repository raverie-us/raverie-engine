...
namespace Raverie
{
RaverieDefineType(ExecutableState, builder, type)
{
...
RaverieDefineType(OpcodeEvent, builder, type)
{
}

RaverieDefineType(FatalErrorEvent, builder, type)
{
}

namespace Events
{
RaverieDefineEvent(FatalError);
RaverieDefineEvent(PreUnhandledException);
RaverieDefineEvent(ExceptionUnhandled);
RaverieDefineEvent(HandledException);
RaverieDefineEvent(OpcodePreStep);
RaverieDefineEvent(OpcodePostStep);
RaverieDefineEvent(EnterFunction);
RaverieDefineEvent(ExitFunction);
RaverieDefineEvent(MemoryLeak);
} // namespace Events
...
RaverieDefineType(ExceptionEvent, builder, type)
{
}
...
void DefaultExceptionCallback(ExceptionEvent* e)
{
  ...
  printf("%s\n", e->ThrownException->GetFormattedMessage(MessageFormat::Raverie).c_str());
}
...
{
  RaverieErrorIfNotStarted(ExecutableState);
...
ExecutableState::~ExecutableState()
{
  ...
  RaverieForEach (StaticFieldPair& pair, this->StaticFieldToMemory)
  {
  ...
  RaverieForEach (HandleManager* manager, this->UniqueManagers.Values())
  {
...
Any ExecutableState::ExecuteStatement(StringParam code)
{
  ...
  Function* main = program->FindFunction(ExpressionMain, Array<Type*>(), RaverieTypeId(Any), FindMemberOptions::Static);
  if (main == nullptr)
...
bool ExecutableState::PushTimeout(PerFrameData* frame, size_t seconds)
{
  ...
  timeout.LengthTicks = seconds * ScriptTimer::TicksPerSecond;
    ...
    while (allInstanceFunctions.Empty() == false)
    {
      ...
      for (size_t i = 0; i < functionArray.Size(); ++i)
      {
        ...
        if (function->NativeVirtual.Index != NativeVirtualInfo::NonVirtual)
        {
          ...
          BoundType* parentRaverieType = derivedType;
          Function* mostDerived = nullptr;

          do
          {
            const FunctionArray* functionArray = parentRaverieType->GetOverloadedInstanceFunctions(function->Name);
            ...
            parentRaverieType = parentRaverieType->BaseType;

            if (parentRaverieType == nullptr)
            {
              Error("We should never reach a null base class before hitting "
                    "the Cpp type");
              break;
            }
          } while (parentRaverieType != cppBaseType);
...
void ExecutableState::ClearStaticFieldsFromLibrary(Library* library)
{
  ...
  // deleted statics) To be entirely safe, we wait to delete the static memory
  // (so we don't magically attempt to allocate it again on access)
  RaverieForEach (Field* field, library->StaticFields)
  {
...
Handle ExecutableState::AllocateCopyConstructedHeapObject(BoundType* type, ExceptionReport& report, HeapFlags::Enum flags, const Handle& fromObject)
{
  ...
  Error("The copy constructor could not be found for type '%s'. "
        "This is often required for when an object is returned to Raverie "
        "via binding if the object has no safe handle manager itself. "
        "If you are using automatic binding call RaverieBindConstructor(const "
        "%s&);",
        type->ToString().c_str(),
        type->ToString().c_str());
  return Handle();
  ...
  if (timeout.AccumulatedTicks > timeout.LengthTicks && !this->EnableDebugEvents)
  {
    ...
    this->ThrowException(report,
                         String::Format("Exceeded the allowed execution time of %d second(s). "
                                        ...
                                        timeout.LengthTicks / ScriptTimer::TicksPerSecond));
...
void ExecutableState::ThrowException(ExceptionReport& report, Handle& handle)
{
  ...
  EventSend(this, Events::ExceptionUnhandled, &toSend);
}
...
void Call::PerformStandardChecks(size_t size, Type* userType, Type* actualType, CheckPrimitive::Enum primitive, ScriptDirection::Enum io)
{
  ...
  if (primitive == CheckPrimitive::Value)
  {
    ...
    if (userType != nullptr)
    {
      ...
      {
        if (!((Type::IsEnumOrFlagsType(actualType)) && Type::IsSame(userType, RaverieTypeId(Integer))))
        {
...
byte* Call::GetChecked(size_t index, size_t size, Type* userType, CheckPrimitive::Enum primitive, ScriptDirection::Enum io)
{
  // If we're setting this... we need to mark that we did for debugging!
  if (io == ScriptDirection::Set)
  {
...
byte* Call::GetThisChecked(size_t size, Type* userType, CheckPrimitive::Enum primitive, ScriptDirection::Enum io)
{
...
byte* Call::GetReturnChecked(size_t size, Type* userType, CheckPrimitive::Enum primitive, ScriptDirection::Enum io)
{
  // As long as the user didn't disable checks...
...
void Call::SetValue(size_t index, const byte* input, size_t size)
{
  ...
  byte* stack = this->GetChecked(index, size, nullptr, CheckPrimitive::Value, ScriptDirection::Set);
...
{
  // Get the stack location and perform checks
  byte* stack = this->GetChecked(index, sizeof(Handle), value.StoredType, CheckPrimitive::Handle, ScriptDirection::Set);
...
{
  // Get the stack location and perform checks
  byte* stack = this->GetChecked(index, size, nullptr, CheckPrimitive::Value, ScriptDirection::Get);
...
{
  // Get the stack location and perform checks
  byte* stack = this->GetChecked(index, sizeof(Handle), nullptr, CheckPrimitive::Handle, ScriptDirection::Get);
...
{
  // Get the stack location and perform checks
  return this->GetChecked(index, sizeof(Handle), nullptr, CheckPrimitive::Handle, ScriptDirection::Get);
}

Delegate& Call::GetDelegate(size_t index)
{
  // Get the stack location and perform checks
  byte* stack = this->GetChecked(index, sizeof(Delegate), nullptr, CheckPrimitive::Delegate, ScriptDirection::Get);
...
{
  // Get the stack location and perform checks
  return this->GetChecked(index, sizeof(Delegate), nullptr, CheckPrimitive::Delegate, ScriptDirection::Get);
}
...
{
  // Get the stack location and perform checks
  byte* stack = call.GetChecked(index, sizeof(Any), RaverieTypeId(Any), CheckPrimitive::Any, ScriptDirection::Set);
...
{
  // Get the stack location and perform checks
  byte* stack = call.GetChecked(index, sizeof(Any), RaverieTypeId(Any), CheckPrimitive::Any, ScriptDirection::Get);
...
{
  // Get the stack location and perform checks
  return call.GetChecked(index, sizeof(Any), RaverieTypeId(Any), CheckPrimitive::Any, ScriptDirection::Get);
}
