...
namespace Raverie
{
const long long ScriptTimer::TicksPerSecond = CLOCKS_PER_SEC;

ScriptTimer::ScriptTimer() : TotalTicks(0), LastClock(Raverie::Time::Clock())
{
}

long long ScriptTimer::GetAndUpdateTicks()
{
  // Get the current clock tick count (it may have wrapped around / overflowed!)
  long long currentClock = Raverie::Time::Clock();
  ...
  if (currentClock >= this->LastClock)
  {
    ...
    ErrorIf(difference > ScriptTimer::TicksPerSecond * SecondsPerMinute * MinutesPerHour * HoursPerDay,
