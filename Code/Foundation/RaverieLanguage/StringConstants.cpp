...
bool IsRaverieQuoteCharacter(Rune r)
{
  return r == '"' || r == '`' || r == '\'';
}
...
StringRange StripStringQuotes(StringRange input)
{
  ...
  ErrorIf(input.SizeInBytes() < 2,
          ...
          "than 2");
  ErrorIf(IsRaverieQuoteCharacter(input.Front()) == false, "The starting character was not a Raverie quotation");
  ErrorIf(IsRaverieQuoteCharacter(input.Back()) == false, "The ending character was not a Raverie quotation");
...
String ToLowerCamelCase(StringRange input)
{
  ...
  Rune firstCharacter = input.Front();
  firstCharacter = Raverie::UTF8::ToLower(firstCharacter);
  input.PopFront();
...
String ToUpperCamelCase(StringRange input)
{
  ...
  Rune firstCharacter = input.Front();
  firstCharacter = Raverie::UTF8::ToUpper(firstCharacter);
  input.PopFront();
  return BuildString(String(firstCharacter), input);
}
} // namespace Raverie
