...
namespace Raverie
{
Delegate::Delegate() : BoundFunction(nullptr)
{
  RaverieErrorIfNotStarted(Delegate);
}
...
bool Delegate::operator==(Raverie::NullPointerType) const
{
  return this->IsNull();
}
...
Any Delegate::Invoke(const Array<Any>& arguments)
{
  ...
  // RaverieOffsetOf with 0) to guarantee it will work
  ArrayClass<Any>* raverieArray = reinterpret_cast<ArrayClass<Any>*>(const_cast<Array<Any>*>(&arguments));
  return this->BoundFunction->Invoke(this->ThisHandle, raverieArray);
}
} // namespace Raverie
