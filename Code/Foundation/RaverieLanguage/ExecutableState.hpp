...
RaverieDeclareEvent(FatalError, FatalErrorEvent);

// Sent on ExecutableState when an exception occurs and isn't handled (can be
// user thrown, etc)
RaverieDeclareEvent(PreUnhandledException, ExceptionEvent);
RaverieDeclareEvent(ExceptionUnhandled, ExceptionEvent);

// Sent on ExecutableState when an exception occurs and is caught by the user
RaverieDeclareEvent(HandledException, ExceptionEvent);

// Debugger and profiler events
RaverieDeclareEvent(OpcodePreStep, OpcodeEvent);
RaverieDeclareEvent(OpcodePostStep, OpcodeEvent);
RaverieDeclareEvent(EnterFunction, OpcodeEvent);
RaverieDeclareEvent(ExitFunction, OpcodeEvent);

// Whenever an scripted object is leaked, it is reported when the executable
// state is torn down
RaverieDeclareEvent(MemoryLeak, MemoryLeakEvent);
} // namespace Events
...
{
public:
  RaverieDeclareType(MemoryLeakEvent, TypeCopyMode::ReferenceType);
...
{
public:
  RaverieDeclareType(OpcodeEvent, TypeCopyMode::ReferenceType);
...
{
public:
  RaverieDeclareType(ExceptionEvent, TypeCopyMode::ReferenceType);
  ExceptionEvent();
  ExecutableState* State;
  Exception* ThrownException;
};

// The types of fatal errors that can occur inside of Raverie (non recoverable
// errors)
...
{
public:
  RaverieDeclareType(FatalErrorEvent, TypeCopyMode::ReferenceType);
...
public:
  ...
  // ScriptTimer class) Every timeout is exclusive, which means while a timeout is
...
{
public:
  RaverieDeclareType(ExecutableState, TypeCopyMode::ReferenceType);
  ...
  // call to Raverie, and reset to the previous after the call)
                                         ...
                                         HeapFlags::Enum flags = HeapFlags::ReferenceCounted)
  {
    ...
    if (typeToAllocate == nullptr)
      typeToAllocate = RaverieTypeId(T);
  ...
  void Delete(T* instance)
  {
    ...
    Handle handle(this, RaverieTypeId(T), (const byte*)instance);
    handle.Delete();
  }
  ...
  T* GetHandleManager()
  {
    return (T*)HandleManagers::GetInstance().GetManager(RaverieManagerId(T), this);
  }
...
void InternalWriteValue(const T& value, byte* stackFrame)
{
  ...
  RaverieStaticType(T)::Write(TypeBinding::ReferenceCast<FromType, ToType>::Cast((FromType)value), stackFrame);
}
...
void InternalWriteRef(const T& value, byte* stackFrame, ExecutableState* state)
{
  ...
  RaverieStrip(T)* pointerToValue = RaverieToPointer(value);
  BoundType* type = RaverieVirtualTypeId(pointerToValue);
  ...
  if (RaverieStaticType(T)::DirectRead)
  {
  ...
  else
  {
    ...
    size_t size = sizeof(typename RaverieStaticType(T)::RepresentedType);
    ...
    typedef typename RaverieStaticType(T)::RepresentedType RepresentedType;
...
// function that is bound to Raverie
class Call
{
public:
  friend class ExecutableState;
  ...
  void SetHandle(size_t index, const T& value)
  {
    ...
    // Get the stack location and perform checks
    byte* stack = this->GetChecked(index, sizeof(Handle), valueType, CheckPrimitive::Handle, ScriptDirection::Set);
  ...
  {
    // Get the stack location and perform checks
    byte* stack = this->GetChecked(index, sizeof(Handle), RaverieTypeId(T), CheckPrimitive::Handle, ScriptDirection::Get);
  ...
  {
    // Get the stack location and perform checks
    return this->GetChecked(index, sizeof(Handle), RaverieTypeId(T), CheckPrimitive::Handle, ScriptDirection::Get);
  }
  ...
  byte* GetChecked(size_t index, size_t size, Type* userType, CheckPrimitive::Enum primitive, ScriptDirection::Enum io);
...
T CallHelper<T>::Get(Call& call, size_t index)
{
  ...
  if (RaverieTypeId(T)->CopyMode == TypeCopyMode::ReferenceType || index == Call::This)
  {
...
byte* CallHelper<T>::GetArgumentPointer(Call& call, size_t index)
{
  if (RaverieTypeId(T)->CopyMode == TypeCopyMode::ReferenceType || index == Call::This)
  {
    ...
    if (stackPointer == nullptr && !Raverie::is_pointer<T>::value)
...
template <>
RaverieCallHelperSpecialization(Any, const Any&);
template <>
RaverieCallHelperSpecialization(Any&, Any&);
template <>
RaverieCallHelperSpecialization(const Any*, const Any* const&);
template <>
RaverieCallHelperSpecialization(const Any&, const Any&);

template <>
RaverieCallHelperSpecialization(Handle*, Handle* const&);
template <>
RaverieCallHelperSpecialization(Handle, const Handle&);
template <>
RaverieCallHelperSpecialization(Handle&, Handle&);
template <>
RaverieCallHelperSpecialization(const Handle*, const Handle* const&);
template <>
RaverieCallHelperSpecialization(const Handle&, const Handle&);

template <>
RaverieCallHelperSpecialization(Delegate*, Delegate* const&);
template <>
RaverieCallHelperSpecialization(Delegate, const Delegate&);
template <>
RaverieCallHelperSpecialization(Delegate&, Delegate&);
template <>
RaverieCallHelperSpecialization(const Delegate*, const Delegate* const&);
template <>
RaverieCallHelperSpecialization(const Delegate&, const Delegate&);

template <typename T>
RaverieCallHelperTemplateSpecialization(HandleOf<T>*, HandleOf<T>* const&);
template <typename T>
RaverieCallHelperTemplateSpecialization(HandleOf<T>, const HandleOf<T>&);
template <typename T>
RaverieCallHelperTemplateSpecialization(HandleOf<T>&, HandleOf<T>&);
template <typename T>
RaverieCallHelperTemplateSpecialization(const HandleOf<T>*, const HandleOf<T>* const&);
template <typename T>
RaverieCallHelperTemplateSpecialization(const HandleOf<T>&, const HandleOf<T>&);
...
HandleOf<T> CallHelper<HandleOf<T>>::Get(Call& call, size_t index)
{
  return *CallHelper<HandleOf<T>*>::Get(call, index);
}
template <typename T>
HandleOf<T>& CallHelper<HandleOf<T>&>::Get(Call& call, size_t index)
{
...
template <typename T>
const HandleOf<T>* CallHelper<const HandleOf<T>*>::Get(Call& call, size_t index)
{
  return CallHelper<HandleOf<T>*>::Get(call, index);
}
template <typename T>
...
template <typename T>
void CallHelper<HandleOf<T>&>::Set(Call& call, size_t index, HandleOf<T>& value)
{
  CallHelper<const HandleOf<T>*>::Set(call, index, &value);
}
template <typename T>
