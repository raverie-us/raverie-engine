...
T& GetInvalid()
{
  ...
  typedef typename Raverie::Decay<T>::Type TNonReference;

  static MaxAlignmentType InvalidBuffer[RaverieAlignCount(sizeof(TNonReference))];
...
public:
  ...
  Any(Raverie::MoveReference<Any> rhs)
  {
  ...
  Any(const T& value, ExecutableState* state = nullptr, P_DISABLE_IF((Raverie::is_base_of<Type, typename Raverie::remove_pointer<typename Raverie::remove_const_and_volatile<T>::type>::type>::value)))
  {
    ...
    RaverieTypeId(T)->IsInitializedAssert();
  ...
  bool Is() const
  {
    BoundType* checkType = RaverieTypeId(T);
...
public:
  ...
  union
  {
    ...
    MaxAlignmentType DataAligned[RaverieAlignCount(sizeof(Delegate))];
  };
