...
namespace Raverie
{
RaverieDefineType(Function, builder, type)
{
  type->HandleManager = RaverieManagerId(PointerManager);
  RaverieFullBindField(builder, type, &Function::FunctionType, "Type", PropertyBinding::Get);
  RaverieFullBindMethod(builder, type, &Function::CreateDelegate, RaverieNoOverload, "CreateDelegate", "instance");
  RaverieFullBindMethod(builder, type, &Function::Invoke, RaverieNoOverload, "Invoke", "instance, arguments");
}
...
Any Function::Invoke(const Any& instance, ArrayClass<Any>* arguments)
{
  ...
  // Copy each argument from the any to the Raverie stack (as its actual value)
  for (size_t i = 0; i < argumentCount; ++i)
  {
