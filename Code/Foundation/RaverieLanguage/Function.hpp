...
public:
  ...
  // this will be set to the thunk function that actually invokes Raverie
  // from a native virtual call (or null if it's non-virtual)
  TypeBinding::VirtualTableFn Thunk;

  // In order to map a native function thunk back to it's Raverie function, we
...
public:
  ...
  RaverieDeclareType(Function, TypeCopyMode::ReferenceType);
