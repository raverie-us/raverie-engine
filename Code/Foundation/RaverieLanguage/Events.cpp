...
namespace Raverie
{
// For the moment we let the user use a special define to disable event safety,
// because right now events have issues
#if !defined(RaverieUnsafeEvents)
#  define RaverieUnsafeEvents false
#endif
...
RaverieDefineType(EventHandler, builder, type)
{
  RaverieFullBindDestructor(builder, type, EventHandler);
  RaverieFullBindConstructor(builder, type, EventHandler, RaverieNoNames);
  type->GetEventHandlerFunction = EventHandlerGetEventHandlerFunction;
}

RaverieDefineType(EventsClass, builder, type)
{
  RaverieFullBindMethod(builder, type, &EventsClass::Send, RaverieNoOverload, "Send", "sender, eventName, event");
  RaverieFullBindMethod(builder, type, &EventsClass::Connect, RaverieNoOverload, "Connect", "sender, eventName, callback");
}

RaverieDefineType(EventData, builder, type)
{
  RaverieFullBindDestructor(builder, type, EventData);
  RaverieFullBindConstructor(builder, type, EventData, RaverieNoNames);
}
...
EventDelegate::EventDelegate() : Type(nullptr)
{
}

EventDelegate::~EventDelegate()
{
  // Event delegates are always responsible for unlinking themselves from the
  // intrusive lists
...
int EventDelegateList::Send(EventData* event)
{
  ...
  // Because we want to ensure complete event safety, we have to make an entire
  // copy of all event delegates (and the container that holds them all) That
  ...
  RaverieForEach (EventDelegate& delegate, this->Outgoing)
  {
  ...
  // Now that we've made a copy of all delegates, go through and invoke them,
  // erasing them as we go
  for (size_t j = 0; j < totalOutgoing; ++j)
  {
    ...
    if (RaverieUnsafeEvents || Type::BoundIsA(sentEventType, delegate->Type))
    {
      // Invoke the delegate with the event
      successfulInvokes += delegate->Invoke(event);
    }

    // Because the delegate is a temporary copy, we can destruct it using the
    // virtual destructor
...
EventDelegateList* EventHandler::GetOrCreateOutgoingDelegateList(StringParam eventName)
{
  ...
  // Because the list is grabbed as a reference, this will automatically update
  // the list in the map
  if (list == nullptr)
  {
...
EventHandler::~EventHandler()
{
  ...
  // Destroy all outgoing delegate lists (which in turn destroys those
  // delegates)
...
int EventDisconnect(EventHandler* sender, EventHandler* receiver, StringParam eventName, void* thisPointerOrUniqueId)
{
  ...
  // Loop through all the event delegates and remove anyone that has the same
  // unique id/this pointer
  int removeCount = 0;
  ...
  while (delegate != outgoingDelegates->Outgoing.End())
  {
    ...
    {
      // We can directly remove the delegate since we do safe iteration here,
      // and we also perform copying when dispatching Note: Technically due to
      // reference counting, if the delegate is keeping an object alive and then
      // we destroy it, that can cause user code to run, which could then delete
      // the 'Next' delegate
...
{
  this->Type = RaverieTypeId(EventData);
}
...
int RaverieEventDelegate::Invoke(EventData* event)
{
  // If we have no executable state, then the event handler was most likely
  // invalidated
  if (this->State == nullptr)
    return 0;

  // If we the this handle is null, then we also skip this event handler
  // In general this should never happen (because this event handler would be
  // removed) However it is possible if we copied all the event handlers that we
  // were going to call and then the object was destroyed, then we would still
  // traverse this
  ...
  // For now we have a special toggle that allows the users to receive events
  // unsafely (as a stop gap...)
  if (RaverieUnsafeEvents)
  ...
  // If the function failed to completely run... then return that this was not a
  // successful invoke
  if (report.HasThrownExceptions())
    return 0;

  // Otherwise, we successfully invoked a Raverie callback!
  return 1;
}

void* RaverieEventDelegate::GetThisPointerOrUniqueId()
{
...
int EventsClass::Send(const Handle& sender, StringParam eventName, EventData* event)
{
  ...
  if (senderHandler == nullptr)
  {
    state->ThrowException(report,
                          "The sender was not a valid event handler and cannot "
...
void EventsClass::Connect(const Handle& sender, StringParam eventName, const Delegate& callback)
{
  ...
  if (Type::IsSame(signature->Return, RaverieTypeId(void)) == false)
  {
  ...
  if (Type::GenericIsA(parameters[0].ParameterType, RaverieTypeId(EventData)) == false)
  {
  ...
  {
    state->ThrowException(report,
                          "The callback's object (the receiver) was null, and "
                          "the function was not a static function");
    return;
  }
  ...
  {
    state->ThrowException(report,
                          "The receiver was not a valid event handler and "
                          "cannot send or receive events");
    return;
  }
  ...
  // Create a the event connection that will invoke the Raverie delegate
  RaverieEventDelegate* eventDelegate = new RaverieEventDelegate(callback, state);
