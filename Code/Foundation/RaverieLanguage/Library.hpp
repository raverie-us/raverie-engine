...
{
public:
  RaverieDeclareType(Library, TypeCopyMode::ReferenceType);
...
void ForEachExtensionGetterSetter(bool isStatic, const LibraryArray& libraries, BoundType* type, Functor functor)
{
  ...
  for (size_t i = 0; i < libraries.Size(); ++i)
  {
    ...
    while (baseIterator != nullptr)
    {
      ...
      {
        RaverieForEach (GetterSetter* property, properties->Values())
        {
  ...
  for (size_t i = 0; i < libraries.Size(); ++i)
  {
    ...
    while (baseIterator != nullptr)
    {
      ...
      if (functionMultiMap != nullptr)
      {
        ...
        RaverieForEach (FunctionArray& overloads, functionMultiMap->Values())
        {
          RaverieForEach (Function* function, overloads.All())
          {
...
namespace Raverie
{
template class Array<Raverie::LibraryRef>;
