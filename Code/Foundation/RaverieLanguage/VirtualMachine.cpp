...
template <typename T>
RaverieForceInline T& GetConstant(Function* function, OperandIndex constantOperand)
{
...
template <typename T>
RaverieForceInline T& GetLocal(byte* frame, OperandIndex localOperand)
{
...
template <typename T>
RaverieForceInline T& GetStatic(PerFrameData* stackFrame, PerFrameData* reportFrame, const Operand& operand)
{
...
template <typename T>
RaverieForceInline T& GetOperand(PerFrameData* stackFrame, PerFrameData* reportFrame, const Operand& operand)
{
...
RaverieForceInline void IfHandler(PerFrameData* stackFrame, const Opcode& opcode)
{
...
RaverieVirtualInstruction(BeginTimeout)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(EndTimeout)
{
  // Validate the timeout (this will throw an exception if we go beyond the time
  // we need to) This only really needs to be ran in jumps
...
RaverieVirtualInstruction(BeginScope)
{
...
RaverieVirtualInstruction(EndScope)
{
...
RaverieVirtualInstruction(ToHandle)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(CreateStaticDelegate)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(CreateInstanceDelegate)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(IfFalseRelativeGoTo)
{
  IfHandler<false>(ourFrame, opcode);
  return;
}

RaverieVirtualInstruction(IfTrueRelativeGoTo)
{
  IfHandler<true>(ourFrame, opcode);
  return;
}

RaverieVirtualInstruction(RelativeGoTo)
{
  // Validate the timeout (this will throw an exception if we go beyond the time
  // we need to) This only really needs to be ran in jumps
...
RaverieVirtualInstruction(Return)
{
}

RaverieVirtualInstruction(PrepForFunctionCall)
{
  // Validate the timeout (this will throw an exception if we go beyond the time
  // we need to) This only really needs to be ran in jumps
  if (state->ThrowExceptionOnTimeout(report))
...
RaverieVirtualInstruction(FunctionCall)
{
...
RaverieVirtualInstruction(LocalObject)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(NewObject)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(PropertyDelegate)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(BeginStringBuilder)
{
...
RaverieVirtualInstruction(EndStringBuilder)
{
  // Grab the rest of the data
  ...
  // Copy the handle to the stack
  Handle* handle = new (ourFrame->Frame + op.SaveStringHandleLocal) Handle((byte*)&result, RaverieTypeId(String));
...
RaverieVirtualInstruction(AddToStringBuilder)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(DeleteObject)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(ThrowException)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(TypeId)
{
  ...
  *new (handlePointer) Handle((byte*)virtualType, RaverieVirtualTypeId(virtualType));

  programCounter += sizeof(TypeIdOpcode);
  return;
}

RaverieVirtualInstruction(ConvertToAny)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(ConvertFromAny)
{
  // Grab the rest of the data
  const AnyConversionOpcode& op = (const AnyConversionOpcode&)opcode;
...
RaverieVirtualInstruction(ConvertDowncast)
{
  // Grab the rest of the data
...
RaverieVirtualInstruction(AnyDynamicMemberGet)
{
  Error("Not implemented");
}

RaverieVirtualInstruction(AnyDynamicMemberSet)
{
  Error("Not implemented");
}

RaverieVirtualInstruction(InvalidInstruction)
{
  Error("This case should never be hit");
}

RaverieVirtualInstruction(ConvertStringToStringRangeExtended)
{
  const ConversionOpcode& op = (const ConversionOpcode&)opcode;
...
RaverieVirtualInstruction(CopyValue)
{
  // Grab the rest of the data
  const CopyOpcode& op = (const CopyOpcode&)opcode;
...
RaverieVirtualInstruction(TestEqualityValue)
{
  const BinaryRValueOpcode& op = (const BinaryRValueOpcode&)opcode;
...
RaverieVirtualInstruction(TestInequalityValue)
{
  const BinaryRValueOpcode& op = (const BinaryRValueOpcode&)opcode;
  const byte* left = &GetOperand<byte>(ourFrame, ourFrame, op.Left);
  const byte* right = &GetOperand<byte>(ourFrame, ourFrame, op.Right);
...
RaverieCaseComplexCopy(Handle);
RaverieCaseComplexCopy(Delegate);
...
RaverieIntegralCases(Byte) RaverieScalarCases(Byte) RaverieIntegralCases(Integer) RaverieScalarCases(Integer)
    ...
    RaverieCaseUnaryRValue(Boolean, Boolean, LogicalNot, output = !operand);

RaverieCaseConversion(Byte, Real, output = (Real)value);
RaverieCaseConversion(Byte, Boolean, output = (value != 0));
RaverieCaseConversion(Byte, Integer, output = (Integer)value);
RaverieCaseConversion(Byte, DoubleInteger, output = (DoubleInteger)value);
RaverieCaseConversion(Byte, DoubleReal, output = (DoubleReal)value);
RaverieCaseConversion(Integer, Real, output = (Real)value);
RaverieCaseConversion(Integer, Boolean, output = (value != 0));
RaverieCaseConversion(Integer, Byte, output = (Byte)value);
RaverieCaseConversion(Integer, DoubleInteger, output = (DoubleInteger)value);
RaverieCaseConversion(Integer, DoubleReal, output = (DoubleReal)value);
RaverieCaseConversion(Real, Integer, output = (Integer)value);
RaverieCaseConversion(Real, Boolean, output = (value != 0));
RaverieCaseConversion(Real, Byte, output = (Byte)value);
RaverieCaseConversion(Real, DoubleInteger, output = (DoubleInteger)value);
RaverieCaseConversion(Real, DoubleReal, output = (DoubleReal)value);
RaverieCaseConversion(Boolean, Integer, output = (Integer)value);
RaverieCaseConversion(Boolean, Real, output = (Real)value);
RaverieCaseConversion(Boolean, Byte, output = (Byte)value);
RaverieCaseConversion(Boolean, DoubleInteger, output = (DoubleInteger)value);
RaverieCaseConversion(Boolean, DoubleReal, output = (DoubleReal)value);
RaverieCaseConversion(DoubleInteger, Real, output = (Real)value);
RaverieCaseConversion(DoubleInteger, Boolean, output = (value != 0));
RaverieCaseConversion(DoubleInteger, Byte, output = (Byte)value);
RaverieCaseConversion(DoubleInteger, Integer, output = (Integer)value);
RaverieCaseConversion(DoubleInteger, DoubleReal, output = (DoubleReal)value);
RaverieCaseConversion(DoubleReal, Real, output = (Real)value);
RaverieCaseConversion(DoubleReal, Boolean, output = (value != 0));
RaverieCaseConversion(DoubleReal, Byte, output = (Byte)value);
RaverieCaseConversion(DoubleReal, Integer, output = (Integer)value);
RaverieCaseConversion(DoubleReal, DoubleInteger, output = (DoubleInteger)value);

RaverieCaseConversion(Integer2, Real2, output = Real2((Real)value.x, (Real)value.y));
RaverieCaseConversion(Integer2, Boolean2, output = Boolean2(value.x != 0, value.y != 0));
RaverieCaseConversion(Real2, Integer2, output = Integer2((Integer)value.x, (Integer)value.y));
RaverieCaseConversion(Real2, Boolean2, output = Boolean2(value.x != 0.0f, value.y != 0.0f));
RaverieCaseConversion(Boolean2, Integer2, output = Integer2((Integer)value.x, (Integer)value.y));
RaverieCaseConversion(Boolean2, Real2, output = Real2((Real)value.x, (Real)value.y));

RaverieCaseConversion(Integer3, Real3, output = Real3((Real)value.x, (Real)value.y, (Real)value.z));
RaverieCaseConversion(Integer3, Boolean3, output = Boolean3(value.x != 0, value.y != 0, value.z != 0));
RaverieCaseConversion(Real3, Integer3, output = Integer3((Integer)value.x, (Integer)value.y, (Integer)value.z));
RaverieCaseConversion(Real3, Boolean3, output = Boolean3(value.x != 0.0f, value.y != 0.0f, value.z != 0.0f));
RaverieCaseConversion(Boolean3, Integer3, output = Integer3((Integer)value.x, (Integer)value.y, (Integer)value.z));
RaverieCaseConversion(Boolean3, Real3, output = Real3((Real)value.x, (Real)value.y, (Real)value.z));
...
{
#define RaverieEnumValue(Name) InstructionTable[Instruction::Name] = &Instruction##Name;
...
void VirtualMachine::ExecuteNext(Call& call, ExceptionReport& report)
{
  ...
  RaverieLastRunningOpcode = compactedOpcode;
  RaverieLastRunningFunction = ourFrame->CurrentFunction;
  RaverieLastRunningOpcodeLength = ourFrame->CurrentFunction->CompactedOpcode.Size();
...
byte* RaverieLastRunningOpcode = nullptr;
Raverie::Function* RaverieLastRunningFunction = nullptr;
size_t RaverieLastRunningOpcodeLength = 0;
