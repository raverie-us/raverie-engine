...
{
public:
  RaverieDeclareType(ScriptRune, TypeCopyMode::ValueType);

  ScriptRune();
  ScriptRune(int value);
  ScriptRune(const Rune rune);
  operator Rune() const;
...
// Runes are converted to and treated as Runes
RaverieDeclareDefineImplicitRedirectType(ScriptRune, Rune);

// String builder is a convenient way to concatenate strings
class StringBuilderExtended : public StringBuilder
{
public:
  RaverieDeclareType(StringBuilderExtended, TypeCopyMode::ReferenceType);
  ...
  void WriteLine(ScriptRune value);
...
{
public:
  RaverieDeclareType(RuneIterator, TypeCopyMode::ReferenceType);
...
// A string range bound to Raverie. This range will also keep the string that it's
// referencing alive
class StringRangeExtended
{
  RaverieDeclareType(StringRangeExtended, TypeCopyMode::ReferenceType);
  ...
  static void SetResultStringSplitRange(Call& call, ExceptionReport& report, StringParam strRef, const Raverie::StringSplitRange& result);
...
{
  RaverieDeclareType(StringSplitRangeExtended, TypeCopyMode::ReferenceType);
  ...
  // The original string this range was made from. Also keeps this string alive
  String mOriginalStringReference;
  Raverie::StringSplitRange mSplitRange;
};
} // namespace Raverie
