...
inline Bits SerializeUnknownBasicVariant(SerializeDirection::Enum direction, BitStream& bitStream, Variant& value)
{
  ...
  if (direction == SerializeDirection::Write)
  {
    ...
    if (!bitStream.SerializeQuantized(SerializeDirection::Write, nativeTypeId, cConstantNativeTypeIdMin,
  ...
  else
  {
    ...
    if (!bitStream.SerializeQuantized(SerializeDirection::Read, nativeTypeId, cConstantNativeTypeIdMin,
                                      cConstantNativeTypeIdMax)) // Unable?
    {
      Error("Serialization failed");
      return 0;
