...
class RaverieCompileEvent : public Event
{
public:
  RaverieDeclareType(RaverieCompileEvent, TypeCopyMode::ReferenceType);
  RaverieCompileEvent(HashSet<ResourceLibrary*>& modifiedLibraries);
...
class RaverieManager : public ExplicitSingleton<RaverieManager, EventObject>
{
public:
  typedef RaverieManager RaverieSelf;
  typedef ExplicitSingleton<RaverieManager, EventObject> RaverieBase;
  ...
  void OnDebuggerPause(Raverie::DebuggerEvent* event);

  // When the debugger resumes execution
  void OnDebuggerResume(Raverie::DebuggerEvent* event);

  // When the debugger is paused on a line and updates
  void OnDebuggerPauseUpdate(Raverie::DebuggerEvent* event);

  // When the debugger skips a breakpoint
  void OnDebuggerBreakNotAllowed(Raverie::DebuggerTextEvent* event);

  // The last library we properly built (set inside
  // CompileLoadedScriptsIntoLibrary) Once this library becomes in use by an
  // executable state, we CANNOT update it, or any RaverieMeta types
