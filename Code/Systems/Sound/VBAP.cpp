...
void VBAP::Initialize(unsigned channels)
{
  ...
  else if (channels == 2)
  {
    ...
    mSpeakers.PushBack(new SpeakerInfo(-FrontSpeakerRadians, rotationMatrix.Transform(normalVec), nullptr, 0));
    // Front right
    rotationMatrix.Rotate(FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[0], 1));
  }
  else if (channels == 3)
  {
    // Front left
    ...
    // Front right
    rotationMatrix.Rotate(FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[1], 1));
  }
  else if (channels == 4)
  {
    // Front left
    ...
    mSpeakers.PushBack(new SpeakerInfo(BackSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[1], 3));
    // Back left
    rotationMatrix.Rotate(-BackSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-BackSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[2], 2));
  }
  else if (channels == 5)
  {
    // Side left
    rotationMatrix.Rotate(-SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-SideSpeakerRadians, rotationMatrix.Transform(normalVec), nullptr, 3));
    // Front left
    rotationMatrix.Rotate(-FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[0], 0));
    // Center
    mSpeakers.PushBack(new SpeakerInfo(0, Math::Vec2(1.0f, 0.0f), mSpeakers[1], 2));
    // Front right
    rotationMatrix.Rotate(FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[2], 1));
    // Side right
    rotationMatrix.Rotate(SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[3], 4));
  }
  else if (channels == 6)
  {
    // Side left
    rotationMatrix.Rotate(-SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-SideSpeakerRadians, rotationMatrix.Transform(normalVec), nullptr, 4));
    // Front left
    rotationMatrix.Rotate(-FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[0], 0));
    // Center
    ...
    // Side right
    rotationMatrix.Rotate(SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[3], 5));
  }
  else if (channels == 7)
  {
    // Back left
    rotationMatrix.Rotate(-BackSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-BackSpeakerRadians, rotationMatrix.Transform(normalVec), nullptr, 5));
    // Side left
    rotationMatrix.Rotate(-SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[0], 3));
    // Front left
    rotationMatrix.Rotate(-FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[1], 0));
    // Center
    mSpeakers.PushBack(new SpeakerInfo(0, Math::Vec2(1.0f, 0.0f), mSpeakers[2], 2));
    // Front right
    rotationMatrix.Rotate(FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[3], 1));
    // Side right
    rotationMatrix.Rotate(SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[4], 4));
    // Back right
    rotationMatrix.Rotate(BackSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(BackSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[5], 6));
  }
  else if (channels == 8)
  {
    // Back left
    rotationMatrix.Rotate(-BackSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-BackSpeakerRadians, rotationMatrix.Transform(normalVec), nullptr, 6));
    // Side left
    rotationMatrix.Rotate(-SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[0], 4));
    // Front left
    rotationMatrix.Rotate(-FrontSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(-FrontSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[1], 0));
    // Center
    ...
    // Side right
    rotationMatrix.Rotate(SideSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(SideSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[4], 5));
    // Back right
    rotationMatrix.Rotate(BackSpeakerRadians);
    mSpeakers.PushBack(new SpeakerInfo(BackSpeakerRadians, rotationMatrix.Transform(normalVec), mSpeakers[5], 7));
  }
