...
public:
  ...
  /// The volume adjustment applied to all sounds played through this
  /// SoundEmitter. A value of 1 does nothing, 2 will double the volume, 0.5
  /// will halve it. The Volume property is linked to the Decibels property
  ...
  /// The volume adjustment, in decibels, applied to all sounds played through
  /// this SoundEmitter. A value of 0 does nothing, 6 will double the volume, -6
  ...
  /// This property affects both the pitch and speed of all sounds played
  /// through this SoundEmitter. A value of 0 will do nothing, 1 will raise the
  /// pitch by an octave and speed up the sound, -1 will lower the sound by an
  /// octave and slow it down. The Pitch property is linked to the Semitones
  /// property (changing one will change the other).
  float GetPitch();
  ...
  /// pitch and speed of all sounds played through this SoundEmitter.. A value
  /// of 0 will do nothing, 12 will raise the pitch by an octave and speed up
  /// the sound, -12 will lower the sound by an octave and slow it down. The
  /// Semitones property is linked to the Pitch property (changing one will
  /// change the other).
  float GetSemitones();
  ...
  /// Plays the SoundCue passed into the function and returns the resulting
  /// SoundInstance.
  ...
  /// This property will be true if there are SoundInstances currently
  /// associated with this SoundEmitter, even if they are paused or otherwise
  /// not audible.
  bool GetIsPlaying();
  /// When true, the audio output of the SoundEmitter will be limited by the
  /// EmitAngle, so that sound in front of the object will be louder than sound
  /// behind it. Within the EmitAngle the sound will be at full volume. The
  /// volume interpolates logarithmically until it reaches the RearVolume value
  /// directly behind the object.
  bool mDirectional;
  /// The angle of full volume sound for a directional SoundEmitter, from 1 to
  /// 360. An angle of 90, for example, will be centered at the object's front,
  /// extending 45 degrees to the left and right.
  float GetEmitAngle();
  void SetEmitAngle(float angleInDegrees);
  /// The volume of sound heard directly behind a directional SoundEmitter. It
  /// will only reach this value in a small area, since volume is interpolated
  /// from the edge of the EmitAngle. To make the volume as quiet as possible
  /// behind the object, use a small EmitAngle.
  float GetRearVolume();
  void SetRearVolume(float minimumVolume);
  /// If a SoundAttenuator resource other than DefaultNoAttenuation is selected
  /// it will be applied to SoundCues without their own SoundAttenuator
  /// resource. If a SoundCue has attenuation settings those will always be
  /// used. If neither has settings, the sound will not be attenuated.
  SoundAttenuator* GetAttenuator();
  void SetAttenuator(SoundAttenuator* attenuator);
  /// The SoundNode to use for attaching other nodes to the input of the
  /// SoundEmitter.
  ...
  /// The SoundNode to use for attaching other nodes to the output of the
  /// SoundEmitter.
