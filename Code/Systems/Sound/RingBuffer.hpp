...
public:
  ...
  // Writes the specified number of elements to the ring buffer. Returns the
  // number of elements that were actually written.
  unsigned Write(const void* data, int elementCount);
  // Reads the specified number of elements from the ring buffer into the
  // provided data buffer. Returns the number of elements that were actually
  // read.
...
private:
  ...
  // Used to wrap indexes when advancing and to handle WriteIndex - ReadIndex
  // calculation
