...
public:
  ...
  HandleOf<SoundInstance> GetSoundInstance()
  {
...
public:
  ...
  /// SoundCue's Volume property. A value of 1 does nothing, 2 will double the
  /// sound's volume, 0.5 will halve it. The Volume property is linked to the
  /// Decibels property (changing one will change the other).
  float GetVolume();
  void SetVolume(float volume);
  /// Interpolates the SoundInstance's Volume property from its current value
  /// to the value passed in as the first parameter, over the number of seconds
  /// passed in as the second parameter.
  void InterpolateVolume(float volume, float interpolationTime);
  /// The volume adjustment (in decibels) of the SoundInstance, initially set by
  /// the SoundCue's Decibels property. A value of 0 does nothing, 6 will double
  /// the sound's volume, -6 will halve it. The Decibels property is linked to
  /// the Volume property (changing one will change the other).
  float GetDecibels();
  ...
  /// The pitch adjustment of the SoundInstance, initially set by the SoundCue's
  /// Pitch property. A value of 0 will do nothing, 1 will raise the pitch by an
  ...
  /// Interpolates the SoundInstance's Pitch property from its current value to
  ...
  /// initially set by the SoundCue's Semitones property.  A value of 0 will do
  ...
  /// Interpolates the SoundInstance's Semitones property from its current value
  /// to the value passed in as the first parameter, over the number of seconds
  /// passed in as the second parameter.
  void InterpolateSemitones(float pitchSemitones, float interpolationTime);
  /// Setting this Property to true will pause a currently playing
  /// SoundInstance. Setting it to false will resume playback.
  bool GetPaused();
  void SetPaused(bool pause);
  /// Stops the playback of this SoundInstance. It cannot be re-started.
  void Stop();
  /// This Property will be true while the SoundInstance is playing, then will
  /// become false when its sound has stopped.
  bool GetIsPlaying();
  /// The SoundNode associated with this SoundInstance.
  HandleOf<SoundNode> GetSoundNode();
  /// When this Property is true the SoundInstance will loop indefinitely. If
  /// changed to false while a SoundInstance is looping the SoundInstance will
  /// continue playing to its EndTime and then stop.
  bool GetLooping();
  void SetLooping(bool loop);
  /// This property tells you to the current playback position, in seconds from
  /// the beginning of the file, and allows you to tell the instance to change
  /// its playback position to a different time. Be aware that the time will not
  /// be precisely accurate. If the Sound resource used to play the
  /// SoundInstance has Streamed selected, you cannot set the playback position.
  float GetTime();
  void SetTime(float seconds);
  /// The length of the entire audio file, in seconds.
  float GetFileLength();
  /// The time in seconds from the beginning of the file that the instance will
  /// stop.
  float GetEndTime();
  void SetEndTime(float seconds);
  /// The time in seconds from the beginning of the file that the instance will
  /// jump back to when it loops.
  float GetLoopStartTime();
  void SetLoopStartTime(float seconds);
  /// The time in seconds from the beginning of the file that the instance will
  /// stop and jump back when looping.
  float GetLoopEndTime();
  void SetLoopEndTime(float seconds);
  /// The time in seconds after looping that will fade out and play concurrently
  /// with the audio where the instance jumped back to.
  float GetLoopTailTime();
  void SetLoopTailTime(float seconds);
  /// If true, the loop tail will be cross-faded: the audio from where the
  /// instance jumped back to will fade in as the loop tail fades out. If false,
  /// the new audio will start at full volume.
  bool GetCrossFadeLoopTail();
  void SetCrossFadeLoopTail(bool crossFade);
  /// The time (in seconds from the beginning of the file) to get a
  /// MusicCustomTime event.
...
private:
  ...
  // Calls the disconnect function on the base class and then calls
  // FinishedCleanUp
  ...
  float mLoopTailTime;
  // If true, the loop tail will cross-fade (new audio will fade in as the tail
  // fades out).
