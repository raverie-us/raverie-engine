...
public:
  ...
  /// Returns the number of net objects in this space (but not including the net
  /// space itself).
  uint GetNetObjectCount() const;
  /// Returns the number of net users in this space.
  uint GetNetUserCount() const;

  /// [Server/Offline] Dispatches the network level started event and clones the
  /// current level state to clients.
  void PerformNetLevelStarted(bool isLevelTransition);

  /// [Client] Performs any delayed attachments now that the delayed parent
  /// exists locally. (Only used during net game cloning, missing attachment
  /// parents are an error otherwise)
  void FulfillDelayedAttachments(Cog* delayedParentObject);

  /// [Client] Adds a delayed attachment, to be performed once the delayed
  /// parent exists locally. If a pre-existing delayed attachment is already
  /// specified for this child it will be overwritten. (Only used during net
  /// game cloning, missing attachment parents are an error otherwise)
