...
/// <param name="Quadratic">Quadratically interpolate between values (uses
/// t^2).</param>
DeclareEnum2(PhysicsEffectInterpolationType, Linear, Quadratic);
/// Describes how interpolation is performed (if at all) outside of a physics
/// effect's max distance. <param name="ClampToMax">Clamps to the max force
/// value</param> <param name="ContinueFalloff">Continue the regular
/// interpolation method</param> <param name="NoEffect">Don't apply a force past
/// the max distance</param>
DeclareEnum3(PhysicsEffectEndCondition, ClampToMax, ContinueFalloff, NoEffect);

/// A common interface for all effects in physics. An effect is something that
/// typically applies a force and can be attached to a collider, rigid body,
/// region, or even a space. This effect is applied every frame according to the
/// rules of the object it is attached to.
...
public:
  ...
  /// If WakeUpOnChange is set, tell the associated object (RigidBody, Region,
  /// etc...) to wake up.
  void CheckWakeUp();
  ...
  /// Whether the object associated with this is woken up when any property is
  /// changed
