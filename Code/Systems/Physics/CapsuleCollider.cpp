...
RaverieDefineType(CapsuleCollider, builder, type)
{
  ...
  RaverieBindGetterSetterProperty(ScalingMode);

  RaverieBindGetter(WorldRadius);
  RaverieBindGetter(WorldCylinderHeight);
}
...
void CapsuleCollider::CacheWorldValues()
{
  ...
  if (mScalingMode == CapsuleScalingMode::PreserveScale)
  {
    ...
    // To preserve scale we need to compute the capsule's entire height (local
    // height + the two radius end caps) and then scale that up to world space.
    // Since the radius should only be affected by the radius' scale then the
    // height needs to account for the missing total height scale. This means we
    // can simply find the cylinder's height by removing the actual world radius
    // values.
...
void CapsuleCollider::ComputeWorldAabbInternal()
{
  ...
  // The easiest way to compute the tight-fit aabb of a capsule is to first look
  // at the capsule as a line segment. It's easy to compute the line segment of
  // the capsule as just the min/max on each axis for both points. After this we
  // can "add" in the radius of the capsule. The min will always shrink by the
  // radius while the max will grow by the radius.
...
void CapsuleCollider::ComputeWorldBoundingSphereInternal()
{
  ...
  // The minimum bounding sphere will always be at the mid-point of the two
  // sphere centers. The radius is then simply the radius of this segment plus
  // the sphere-cap radius.
...
void CapsuleCollider::ComputeLocalInverseInertiaTensor(real mass, Mat3Ref localInvInertia)
{
  ...
  // Unfortunately, we need to have piece-wise mass for the hemisphere and
  // cylinder so we can't use the passed in mass (currently this is just the
  // total mass so there's no real problem other than performance with
  // recomputing it)
  ...
  // Compute the unique hemisphere and cylinder inertia terms (the hemisphere is
  ...
  // The radius (the local 'x' and 'z' axes) axis gets a large amount of extra
...
void CapsuleCollider::ComputeWorldPoints(Vec3Ref pointA, Vec3Ref pointB) const
{
  ...
  // the half height so we can easily get the two sphere center points.
