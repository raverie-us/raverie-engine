...
void FlowEffect::DebugDraw()
{
  ...
  // Clamp the animating speed. Beyond a certain point it's too hard to
  // visualize.
  ...
  // Draw animating rings from the bottom of the cylinder to the top. Determine
  // how many rings to draw from the height of the cylinder. We can draw
  // anywhere from 1 to 5, depending on the height of the cylinder.
  ...
  for (size_t i = 0; i < ringCount; ++i)
  {
    ...
    // If the flow speed is negated, the "invert" the interpolation value so we
    ...
    if (GetAttractToFlowCenter())
    {
      ...
      {
        // Compute the angle along the circle and use that the get the position
        // on the ring
...
void FlowEffect::ApplyEffect(RigidBody* obj, real dt)
{
  ...
  // Get the inverse mass of the object, we need this to compute the
  // acceleration values as forces
  ...
  // (so we will always target a set speed but have a max acceleration with
  // which to get there)
  ...
  {
    // Compute the translation offset of the rigid body perpendicular to the
    // flow direction
    ...
    // If the object is in the center of the flow then we can't apply an
    // attraction force towards the center. In that case just compute a force to
