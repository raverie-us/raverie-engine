...
/// together. Welds should generally not be used to make two objects rigid as
/// they are computationally more expensive and less rigid than using
/// composites. The primary uses for welds are for quick connections or
/// connections that are desired to not be fully rigid. Also, while it can be
/// done with composites too, welds can be used to model object breaking beyond
/// some max force. Limits, motors and springs should most likely not be used on
/// this. Add on definitions: Limit: A limit will provide a min/max translation
/// on the x, y, and z axes. Motor: A motor will attempt to drive the rotation
/// on the x, y, and z axes. Spring: A spring will make the x, y, and z axis
/// springy.
