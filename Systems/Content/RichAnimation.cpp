...
//******************************************************************************
KeyFrame::KeyFrame(float time, AnyParam value, KeyFrameId id, TrackNode* parent)
  : Id(id)
{
  mTime = time;
  mValue = value;
...
void CreateSubTracks(TrackNode* parent, cstr names, RichAnimation* richAnim)
{
  ...
  for(uint i = 0; i < elements; ++i)
  {
    ...
    TrackNode* subTrack = new TrackNode(name, path, TrackType::SubProperty,
                                        nullptr, parent, richAnim);
  }
}

//******************************************************************************
TrackNode::TrackNode(StringParam name, StringParam path, TrackType::Enum type,
                     BoundType* targetMeta, TrackNode* parent,
...
{
  ReturnIf(Type == TrackType::Object || Type == TrackType::Component, Any(),
  ...
  case TrackType::Property:
    {
      ...
      if(prop == nullptr)
      {
        ...
        String message = String::Format("Failed to find Property '%s' on Component '%s'", 
      ...
      else if(prop->PropertyType != mPropertyTypeId)
      {
        ...
        String message = String::Format("Property type has changed from '%s' to '%s'.",
                                        storedTypeName, currTypeName);
        status.SetFailed(message);
      }
      break;
    }
...
//******************************************************************************
bool GetStartAndEndTimes(TrackNode* vectorTrack, float* startTime,
                         float* endTime, uint elements)
{
...
//******************************************************************************
void PushToAnimationRecursive(TrackNode* objectTrackInfo, Animation* animation,
                              uint& objectTrackCount, float& duration)
{
...
//   }
  ...
  for(uint i = 0; i < mRoot->Children.Size(); ++i)
  {
    ...
    PushToAnimationRecursive(track, animation, animation->mNumberOfTracks,
                             animation->mDuration);
  }
}
...
TrackNode* RichAnimation::GetObjectTrack(TrackNode* parent, 
                                         ...
                                         String currPath, bool createNew)
{
  ...
  if(objectTrack == nullptr)
  {
    ...
    objectTrack = new TrackNode(trackName, currPath, TrackType::Object,
                                nullptr, parent, this);
  }
...
TrackNode* RichAnimation::GetPropertyTrack(TrackNode* objectTrack,
                                           StringParam path,
                                           BoundType* targetMeta, bool createNew)
{
  // First we have to get the component track
  String componentName = ComponentNameFromPath(path);
  TrackNode* componentTrack = GetComponentTrack(objectTrack, componentName,
                                                targetMeta, createNew);
  ...
  if(propertyTrack == nullptr && createNew)
  {
    ...
    propertyTrack = new TrackNode(propertyName, path, TrackType::Property,
                                  targetMeta, componentTrack, this);
  }
                                            ...
                                            bool createNew)
{
  ...
  if(componentTrack == nullptr && createNew)
  {
    componentTrack = new TrackNode(component, component, TrackType::Component,
                                   targetMeta, objectTrack, this);
  }
...
//******************************************************************************
TrackNode* RichAnimation::GetPropertyTrack(Cog* object, Cog* animGraphObject,
             BoundType* componentType, StringParam propertyName, bool createNew)
{
  ...
  TrackNode* propertyTrack = GetPropertyTrack(objectTrack, propertyPath,
                                              virtualComponentType, createNew);
  return propertyTrack;
}
...
void RichAnimationBuilder::BuildContent(BuildOptions& buildOptions)
{
  ...
  if(!success)
  {
    ...
    buildOptions.Message = String::Format("Failed to bake rich animation file at %s to %s",
...
//------------------------------------------------------------------------ range
//******************************************************************************
RichAnimation::range::range(TrackNode* root)
  : mCurrent(root)
{
