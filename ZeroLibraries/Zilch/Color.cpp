...
namespace Zilch
{
  ...
  {
    ZilchFullBindMethod(builder, type, &ColorClass::FromBytes, ZilchStaticOverload(Real4, Integer, Integer, Integer),          "FromBytes", "r, g, b")
      ->Description = ZilchDocumentString("Converts RGB Bytes [0-255] into an RGBA Real4 (alpha defaults to 1.0). Integer is used in place of Byte for convenience.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromBytes, ZilchStaticOverload(Real4, Integer, Integer, Integer, Integer), "FromBytes", "r, g, b, a")
      ->Description = ZilchDocumentString("Converts RGBA Bytes [0-255] into an RGBA Real4. Integer is used in place of Byte for convenience.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromBytes, ZilchStaticOverload(Real4, Integer4Param),                      "FromBytes", "rgba")
      ->Description = ZilchDocumentString("Converts RGBA Bytes [0-255] into an RGBA Real4. Integer is used in place of Byte for convenience.");

    ZilchFullBindMethod(builder, type, &ColorClass::FromInteger, ZilchStaticOverload(Real4, Integer),          "FromInteger", "rgba")
      ->Description = ZilchDocumentString("Converts an RGBA packed Integer into an RGBA Real4. Endianness is handled so that 0x00FF00FF always means full green and full alpha.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromInteger, ZilchStaticOverload(Real4, Integer, Integer), "FromInteger", "rgb, a")
      ->Description = ZilchDocumentString("Converts an RGB packed Integer and an alpha Byte [0-255] into an RGBA Real4. Endianness is handled so that 0x00FF00FF always means full green and full alpha.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromInteger, ZilchStaticOverload(Real4, Integer, Real),    "FromInteger", "rgb, a")
      ->Description = ZilchDocumentString("Converts an RGB packed Integer and an alpha Real into an RGBA Real4. Endianness is handled so that 0x00FF00FF always means full green and full alpha.");

    ZilchFullBindMethod(builder, type, &ColorClass::FromHsva, ZilchStaticOverload(Real4, Real, Real, Real),       "FromHsva", "h, s, v")
      ->Description = ZilchDocumentString("Converts HSV Reals into an RGBA Real4 (alpha defaults to 1.0). The hue parameter will wrap if is beyond [0-1]. The saturation and value parameters may go beyond [0-1] to represent HDR values.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromHsva, ZilchStaticOverload(Real4, Real, Real, Real, Real), "FromHsva", "h, s, v, a")
      ->Description = ZilchDocumentString("Converts HSVA Reals into an RGBA Real4. The hue parameter will wrap if is beyond [0-1]. The saturation and value parameters may go beyond [0-1] to represent HDR values.");
    ZilchFullBindMethod(builder, type, &ColorClass::FromHsva, ZilchStaticOverload(Real4, Real4Param),             "FromHsva", "hsva")
      ->Description = ZilchDocumentString("Converts an HSVA Real4 into an RGBA Real4. The hue parameter will wrap if is beyond [0-1]. The saturation and value parameters may go beyond [0-1] to represent HDR values.");

    ZilchFullBindMethod(builder, type, &ColorClass::FromHexString, ZilchNoOverload, "FromHexString", "value")
      ->Description = ZilchDocumentString("Converts a hex String into an RGBA Real4. Must be a 3, 4, 6, or 8 digit RGB[A] representation with an optional preceding '#' or '0x' (case insensitive). E.g. #f00, #F00F, ff0000, 0x00FF00FF.");

    ZilchFullBindMethod(builder, type, &ColorClass::ToBytes, ZilchStaticOverload(Integer4, Real4Param),             "ToBytes", "rgba")
      ->Description = ZilchDocumentString("Converts an RGBA Real4 into an RGBA Integer4 [0-255]. Integer is used in place of Byte for convenience.");
    ZilchFullBindMethod(builder, type, &ColorClass::ToBytes, ZilchStaticOverload(Integer4, Real, Real, Real),       "ToBytes", "r, g, b")
      ->Description = ZilchDocumentString("Converts RGB Reals into an RGBA Integer4 [0-255] (alpha defaults to 255). Integer is used in place of Byte for convenience.");
    ZilchFullBindMethod(builder, type, &ColorClass::ToBytes, ZilchStaticOverload(Integer4, Real, Real, Real, Real), "ToBytes", "r, g, b, a")
      ->Description = ZilchDocumentString("Converts RGBA Reals into an RGBA Integer4 [0-255]. Integer is used in place of Byte for convenience.");
    
    ZilchFullBindMethod(builder, type, &ColorClass::ToInteger, ZilchStaticOverload(Integer, Real4Param),             "ToInteger", "rgba")
      ->Description = ZilchDocumentString("Converts an RGBA Real4 into an RGBA packed Integer. Endianness is handled so that 0x00FF00FF always means full green and full alpha.");
    ZilchFullBindMethod(builder, type, &ColorClass::ToInteger, ZilchStaticOverload(Integer, Real, Real, Real),       "ToInteger", "r, g, b")
      ->Description = ZilchDocumentString("Converts RGB Reals into an RGBA packed Integer (alpha defaults to 255). Endianness is handled so that 0x00FF00FF always means full green and full alpha.");
    ZilchFullBindMethod(builder, type, &ColorClass::ToInteger, ZilchStaticOverload(Integer, Real, Real, Real, Real), "ToInteger", "r, g, b, a")
      ->Description = ZilchDocumentString("Converts RGBA Reals into an RGBA packed Integer. Endianness is handled so that 0x00FF00FF always means full green and full alpha.");
    ...
    ZilchFullBindMethod(builder, type, &ColorClass::ToHsva, ZilchStaticOverload(Real4, Real, Real, Real),       "ToHsva", "r, g, b")
      ->Description = ZilchDocumentString("Converts RGB Reals into an HSVA Real4 (alpha defaults to 1.0).");
    ZilchFullBindMethod(builder, type, &ColorClass::ToHsva, ZilchStaticOverload(Real4, Real, Real, Real, Real), "ToHsva", "r, g, b, a")
      ->Description = ZilchDocumentString("Converts RGBA Reals into an HSVA Real4.");

    ZilchFullBindMethod(builder, type, &ColorClass::ToHexString, ZilchStaticOverload(String, Real4Param),             "ToHexString", "rgba")
      ->Description = ZilchDocumentString("Converts an RGBA Real4 into the 8 digit hex format RRGGBBAA.");
    ZilchFullBindMethod(builder, type, &ColorClass::ToHexString, ZilchStaticOverload(String, Real, Real, Real),       "ToHexString", "r, g, b")
      ->Description = ZilchDocumentString("Converts RGB Reals into the 8 digit hex format RRGGBBAA (alpha defaults to 1.0 so the end will always be FF).");
    ZilchFullBindMethod(builder, type, &ColorClass::ToHexString, ZilchStaticOverload(String, Real, Real, Real, Real), "ToHexString", "r, g, b, a")
  ...
  {
    return Math::Vec4(
      float(((color)&CC::RedMask) >> CC::RedOffset)*CC::InvFactor,
  ...
  Real4 ColorClass::FromHexString(StringParam value)
  {
    ...
    const char* sizeError = "The hex string must be a 3, 4, 6, or 8 digit RGB[A] representation with an optional preceding '#' or '0x' (case insensitive). E.g. #f00, #F00F, ff0000, 0x00FF00FF.";
    ...
    {
      ExecutableState::CallingState->ThrowException("A string that starts with '0x' must be the 8 digit RGBA representation. E.g. 0xFF000000.");
      return Real4::cZero;
    }
  ...
  Integer4 ColorClass::ToBytes(Real4Param rgba)
  {
    ...
    return Integer4(
      int(Math::Round(rgbaClamped.x * 255.0f)),
