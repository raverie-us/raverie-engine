...
bool FileDialog(OsHandle windowHandle, FileDialogSetup& config, bool opening)
{
  ...
  if(success)
  {
       ...
       config.Flags & FileDialogFlags::MultiSelect)
    {
      ...
      while(*current != '\0')
      {
        ...
        else
        {
          ...
          files.PushBack(FilePath::Combine(directoryName, Narrow(currentFile)));
        }
    ...
    {
      files.PushBack(Narrow(fileDialog.lpstrFile).c_str());
    }
  }
  ...
  bool succeeded = (success != FALSE) && !files.Empty();
  config.mCallback(files, succeeded, config.mUserData);
