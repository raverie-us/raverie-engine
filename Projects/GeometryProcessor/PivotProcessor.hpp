...
public:
  ...
  void CollapsePivots();
  // Take a node and collapse all pivots between the first pivot and the non-pivot node or animated pivot
  String CollapsePivotHierarchy(HierarchyData& startNode, bool preAnimationCorrection);
  // Helper function that corrects skeleton paths for after collapsing a hierarchy of pivots
  void CorrectSkeletonPath(HierarchyData& node);
  // Takes an animation with single track values and builds a local transform to correct a later nodes animations
  ...
  // Array that contains the name of each top level pivot in an objects hierarchy
